...
#In local, public url contains with authentication details. In order to make it work in local while running against dev- we might need to remove authentication details for comparison purpose

*** Test Cases ***

Parse and Store public url without auth details
    ${PUBLIC_URL_WITHOUT_AUTH}=    normalize url    %{PUBLIC_URL}    
...
Verify that absolute paths with trailing slashes are redirected without them
    ...
    user checks url and normalize    ${PUBLIC_URL_WITHOUT_AUTH}/data-catalogue
    ...
    user checks url and normalize    ${PUBLIC_URL_WITHOUT_AUTH}/glossary?someRandomUrlParameter=123
