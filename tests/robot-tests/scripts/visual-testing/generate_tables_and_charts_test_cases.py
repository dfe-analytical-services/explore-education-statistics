...
robot_tests_folder = '../../tests/visual_testing'

with open(f'{robot_tests_folder}/visually_check_tables_and_charts.template.robot', 'r', encoding='utf-8-sig') as robot_template_file:
    ...
    generated_test_script += footer.replace('{{datablocks_csv_filename}}',
                                            f'tests/visual_testing/{target_robot_test_csv_filename}')

    warning(generated_test_script)

    with open(f'{robot_tests_folder}/{target_robot_test_filename}', 'w', encoding='utf-8-sig') as generated_robot_file:
