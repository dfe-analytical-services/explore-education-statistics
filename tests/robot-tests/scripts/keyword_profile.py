...
def _print_results(times, shown_keywords, limit):
    ...
    kwrows = []
    for kt in ktlist:
       kwrows.append([kt.elapsed, kt.calls, kt.average_time, kt.median_time, kt.standard_deviation, kt.stdev_per_avgtime, kt.name])
    ...
    for k in kwrows:
        ...
        if limit is not None and k.stdev_per_avgtime > limit:
            continue
        shown += 1
        print(str(k[0]).rjust(14)+' | '+str(k[1]).rjust(7)+' | '+str(k[2]).rjust(12)+' | '+str(k[3]).rjust(15)+' | '+str(k[4]).rjust(9)+' | '+str(k[5]).rjust(16)+' | '+str(k[6]))
...
def _write_results(times, shown_keywords, limit, path):
    ...
    for k in kwrows:
        ...
        if limit is not None and k.stdev_per_avgtime > limit:
            continue
        shown += 1
        fd.write(str(k[0]).rjust(14)+' | '+str(k[1]).rjust(7)+' | '+str(k[2]).rjust(12)+' | '+str(k[3]).rjust(15)+' | '+str(k[4]).rjust(9)+' | '+str(k[5]).rjust(16)+' | '+str(k[6])+'\n')
