...
2. Run the following command:
   ```bash
   cd tests/robot-tests/tests/visual_testing
   pipenv run python generate_tables_and_charts_test_cases.py --file /path/to/tests/robot-tests/datablocks-dev.csv --target visually_check_tables_and_charts.dev.robot 
   ```
   This will generate a test suite for the Data Blocks (for the Dev environment in this example). In this suite will be an individual test case for each Release with 
   published Data Blocks for that environment.

#### Capture "before" and "after" images
Now that we have a generate test script specific to an environment and the data on it, we can run the tests to capture "before" images on that environment.
...
This will run through the Releases on the environment, capturing images of all of the tables and charts per Release, saving them into a
...
```bash
cd tests/robot-tests/tests/visual_testing
pipenv run python generate_permalink_test_cases.py --file /path/to/tests/robot-tests/permalinks-dev.csv --target visually_check_permalinks.dev.robot 
```
