...
const performTest = () => {
  ...
  const requestBody = {
    subjectId: subjectIds[randomSubjectIdIndex],
    indicators: null,
    filters: null,
  ...
  if (
    ...
    console.log(`'SUCCESS! SubjectId: ${subjectId} LocationId: ${locationId}`);
    filterSubjectMetaSuccessCount.add(1);
    filterSubjectMetaRequestDuration.add(Date.now() - startTime);
  } else {
    console.log(
      `FAILURE! SubjectId: ${subjectId} LocationId: ${locationId} Got ${response.status} response code - ${JSON.stringify(
        response.body,
      )}`,
    );
    filterSubjectMetaFailureCount.add(1);
