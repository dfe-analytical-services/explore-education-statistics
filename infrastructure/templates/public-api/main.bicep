    ...
    appSettings: [
      {
        ...
        value: replace(replace(postgreSqlServerModule.outputs.managedIdentityConnectionStringTemplate, '[database_name]', 'public_api'), '[managed_identity_client_id]', apiContainerAppManagedIdentity.id)
      }
    ]
    tagValues: tagValues
