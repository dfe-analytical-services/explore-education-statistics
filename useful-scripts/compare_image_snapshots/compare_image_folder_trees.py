...
def compare_images_in_folder_trees(
        first_folderpath,
        second_folderpath,
        diff_folderpath):
    ...
    for folder, _, original_files in os.walk(first_folderpath_absolute):
        print('Found directory: %s' % folder)
        for original_filename in original_files:
            print('\t%s' % original_filename)
            ...
            if not os.path.exists(second_filepath):
                warning(f'Comparison file does not exist at {second_filepath}')
                continue

            diff_folderpath = f'{diff_folderpath_absolute}/{original_folder_relative_path}'
            compare_images(f'{folder}/{original_filename}', second_filepath, f'{diff_folderpath}/{original_filename}')


ap = argparse.ArgumentParser()
ap.add_argument("-f", "--first", required=True,
                help="first folder tree")
ap.add_argument("-s", "--second", required=True,
                help="second folder tree")
ap.add_argument("-d", "--diff", required=True,
                help="output path for diff folder tree")
args = vars(ap.parse_args())

compare_images_in_folder_trees(
    args['first'],
    args['second'],
    args['diff'])
