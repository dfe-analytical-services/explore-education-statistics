...
) : IParquetService
{
    ...
    )
    {
        ...
        logger.LogDebug(
            "Writing data files to data set version directory '{VersionDir}'",
            versionDir
        );
        ...
        var newLines = (await File.ReadAllLinesAsync(loadSqlFilePath, cancellationToken)).Select(
            line => line.Replace(absolutePathToReplace, "")
        );
