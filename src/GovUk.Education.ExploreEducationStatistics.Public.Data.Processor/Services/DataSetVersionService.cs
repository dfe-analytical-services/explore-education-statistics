...
) : IDataSetVersionService
{
            ...
            .OnSuccess(dataSet =>
                (
                    ...
                    && dataSetVersionToReplaceId is not null
                        ...
                        : (DataSetVersion?)null
                ).OnSuccess(previousDataSetVersionToReplace =>
                    (dataSet, previousDataSetVersionToReplace)
                )
            )
        ...
        return await publicDataDbContext.RequireTransaction(() =>
            GetDataSetVersion(dataSetVersionId, cancellationToken)
                .OnSuccessDo(dataSetVersion =>
                    CheckCanDeleteDataSetVersion(
                        dataSetVersion: dataSetVersion,
                        forceDeleteAll: false
                    )
                )
                .OnSuccessDo(async dataSetVersion =>
                    await UpdateReleaseFiles(dataSetVersion, cancellationToken)
                )
                .OnSuccessDo(async dataSetVersion =>
                    await DeleteDataSetVersion(dataSetVersion, cancellationToken)
                )
    ...
    private async Task DeleteDataSetVersion(
        DataSetVersion dataSetVersion,
        CancellationToken cancellationToken
    )
    {
    ...
    {
        var previousVersion = dataSet.Versions.FirstOrDefault(dv =>
            dataSetVersionToReplaceId == dv.Id
        );
    ...
    )
    {
        ...
        var previousReleaseFileIds = dataSet
            .Versions.Select(version => version.Release.ReleaseFileId)
            .ToList();
        ...
        var selectedReleaseFileReleaseId = (
            await GetReleaseIdsForReleaseFiles(
                contentDbContext,
                [releaseFile.Id],
                cancellationToken
            )
    ...
    )
    {
        var nextVersion =
            featureFlags.Value.EnableReplacementOfPublicApiDataSets
            && previousVersionToPatch is not null
        ...
        var dataSetVersion = new DataSetVersion
        {
            ...
            {
                DataSetFileId =
                    releaseFile.File.DataSetFileId
                    ?? throw new NullReferenceException("DataSetFileId cannot be null"),
                ReleaseFileId = releaseFile.Id,
