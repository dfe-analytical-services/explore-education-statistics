...
) : IDataSetVersionMappingService
{
    ...
    )
    {
        var mapping = await publicDataDbContext
            ...
            .SingleAsync(mapping => mapping.TargetDataSetVersionId == nextDataSetVersionId, cancellationToken);
        ...
        mapping.LocationMappingsComplete = !mapping
            ...
            .Any(level =>
                level.Value.Mappings.Any(optionMapping => IncompleteMappingTypes.Contains(optionMapping.Value.Type))
            );
    ...
    private static bool IsMajorVersionUpdate(DataSetVersionMapping mapping)
    {
        ...
        var hasUnmappedLocationOptions = mapping.LocationMappingPlan.Levels.Any(level =>
            level.Value.Mappings.Any(optionMapping => NoMappingTypes.Contains(optionMapping.Value.Type))
        );
    ...
    )
    {
        return GetNextDataSetVersionInMappingStatus(request, cancellationToken)
            ...
            .OnSuccessCombineWith(nextDataSetVersion => GetImportInManualMappingStage(request, nextDataSetVersion));
    }
    ...
    )
    {
        var importToContinue = nextDataSetVersion.Imports.SingleOrDefault(import =>
            import.DataSetVersionId == nextDataSetVersion.Id && import.Stage == DataSetVersionImportStage.ManualMapping
        );
    ...
    )
    {
        var nextVersion = await publicDataDbContext
            ...
            .SingleOrDefaultAsync(dataSetVersion => dataSetVersion.Id == request.DataSetVersionId, cancellationToken);
        ...
        where TOptionMapping : Mapping<TMappableOption>
    {
        ...
        if (autoMapCandidate is not null)
        {
            ...
            parentMapping.OptionMappings.ForEach(optionMapping =>
                AutoMapElement(sourceKey: optionMapping.Key, mapping: optionMapping.Value, candidates: candidateOptions)
            );
        }
        else
        {
    ...
    )
    {
        ...
        var sourceMappingsByLevel = sourceLocationMeta.ToDictionary(
            ...
            {
                var candidatesForLevel = targetLocationMeta.Any(entry => entry.Key.Level == level.Level)
        ...
        {
            Levels = sourceMappingsByLevel.Concat(onlyTargetMappingsByLevel).ToDictionary(e => e.Key, e => e.Value),
        };
    }
    ...
    private async Task<List<LocationMeta>> GetLocationMeta(Guid dataSetVersionId, CancellationToken cancellationToken)
    {
