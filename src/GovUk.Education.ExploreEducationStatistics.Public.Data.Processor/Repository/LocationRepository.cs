    ...
    IDataSetVersionPathResolver dataSetVersionPathResolver) : ILocationRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var metas = levels
            ...
            {
                DataSetVersionId = dataSetVersion.Id, Level = level,
            })
            .ToList();
        ...
        foreach (var meta in metas)
        {
            ...
            while (current < options.Count)
            {
                ...
                var links = await optionTable
                    ...
                    {
                        MetaId = meta.Id, OptionId = option.Id
                    })
                    .ToListAsync(token: cancellationToken);
        ...
        GeographicLevel level)
    {
        ...
        return level switch
        {
            ...
            {
                PublicId = string.Empty, Label = label, Ukprn = row[ProviderCsvColumns.Ukprn]
            },
            GeographicLevel.RscRegion => new LocationRscRegionOptionMeta
            {
                PublicId = string.Empty, Label = label
            },
            _ => new LocationCodedOptionMeta
            {
                PublicId = string.Empty, Label = label, Code = row[cols.Codes.First()]
            }
        };
    }
}
