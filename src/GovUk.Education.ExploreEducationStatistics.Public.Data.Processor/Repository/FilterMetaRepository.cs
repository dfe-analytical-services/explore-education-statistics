    ...
    IDataSetVersionPathResolver dataSetVersionPathResolver) : IFilterMetaRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var id = currentMetaId++;
            var meta = new FilterMeta
            {
                Id = id,
                PublicId = publicIdMappings.Filters.GetValueOrDefault(metaRow.ColName, SqidEncoder.Encode(id)),
            ...
            var defaultOption = options.FirstOrDefault(option => metaRow.FilterDefault != null
    ...
    private async Task<IEnumerable<MetaFileRow>> GetFilterMetaRows(
        IDuckDbConnection duckDbConnection,
        DataSetVersion dataSetVersion,
        IReadOnlySet<string> allowedColumns,
        CancellationToken cancellationToken)
