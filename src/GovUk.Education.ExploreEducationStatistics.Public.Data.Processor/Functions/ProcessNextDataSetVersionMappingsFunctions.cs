...
) : BaseProcessDataSetVersionFunction(publicDataDbContext)
{
    ...
    public async Task CreateMappings([ActivityTrigger] Guid instanceId, CancellationToken cancellationToken)
    {
        ...
        await mappingService.CreateMappings(
            dataSetVersionImport.DataSetVersionId,
            dataSetVersionImport.DataSetVersionToReplaceId,
            cancellationToken
        );
    }

    [Function(ActivityNames.ApplyAutoMappings)]
    public async Task ApplyAutoMappings([ActivityTrigger] Guid instanceId, CancellationToken cancellationToken)
    {
        var dataSetVersionImport = await GetDataSetVersionImport(instanceId, cancellationToken);
