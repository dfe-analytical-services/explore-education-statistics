...
public static class PrivateBlobStorageServiceExtensions
{
    ...
    {
        return async () =>
            (await service.GetDownloadStream(PrivateReleaseFiles, import.File.Path())).Right;
    }
    ...
    {
        return async () =>
            (await service.GetDownloadStream(PrivateReleaseFiles, import.MetaFile.Path())).Right;
    }
}
