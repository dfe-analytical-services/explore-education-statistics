...
public class ValidatorService : IValidatorService
{
    ...
    private async Task<Either<List<DataImportError>, (List<string> columnHeaders, int totalRows)>> ValidateMetadataFile(
        File file,
        Func<Task<Stream>> fileStreamProvider
    )
    {
        ...
        {
            errors.Add(
                new DataImportError("The filter_default column cannot be used in conjunction with filter hierarchies")
            );
        }
    ...
    private static Task<Either<List<DataImportError>, Unit>> ValidateObservationHeaders(List<string> cols)
    {
        ...
        return Task.FromResult<Either<List<DataImportError>, Unit>>(errors.Count > 0 ? errors : Unit.Instance);
    }

    private async Task<Either<List<DataImportError>, ProcessorStatistics>> ValidateAndCountObservations(
        DataImport import,
