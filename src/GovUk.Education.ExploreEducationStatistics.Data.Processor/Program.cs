...
async Task ClearQueues()
{
    ...
    var importsPendingQueueClient = new QueueClient(
        connectionString,
        queueName: ImportsPendingQueue
    );
    await importsPendingQueueClient.CreateIfNotExistsAsync();
    await importsPendingQueueClient.ClearMessagesAsync();

    var restartImportsQueueClient = new QueueClient(
        connectionString,
        queueName: RestartImportsQueue
    );
...
async Task RestartImports()
{
    ...
    await restartImportsQueueClient.SendMessageAsync(
        JsonSerializer.Serialize(new RestartImportsMessage())
    );
}
