    ...
    public class MethodologyVersionRepositoryTests
    {
        ...
        public async Task CreateMethodologyForPublication()
        {
            ...
            await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
            {
                ...
                Assert.Null(methodologyVersion.Methodology.LatestPublishedVersionId);
        ...
        [Fact]
        public async Task IsPubliclyAccessible_PublishedImmediatelyApprovedAndLatestPublishedIsAccessible()
        {
            ...
            await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
            {
                ...
                var result = await service.IsPubliclyAccessible(methodologyVersion);
                Assert.True(result);
            }
        }
        ...
        public async Task PublicationTitleChanged_MethodologyAlreadyPubliclyAvailable()
        {
            ...
            await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
            {
                ...
                // The Methodology has LatestPublishedVersionId set - i.e. is published - and so its Slug does not update.
                Assert.Equal("New Title", publicationMethodology.Methodology.OwningPublicationTitle);
                Assert.Equal("original-slug", publicationMethodology.Methodology.Slug);
            }
        }
