...
public static class ReleaseGeneratorExtensions
{
        ...
        int? year = null)
    {
        ...
        return fixture.Generator<Release>()
            ...
            .WithVersions(fixture.DefaultReleaseVersion()
                .ForInstance(s => s
                    .Set(p => p.Version,
                        (_, _, context) => context.Index))
                .ForRange(..publishedVersions,
                    s => s
                        ...
                        .Set(rv => rv.PublishScheduled, (_, rv) => rv.Published!.Value.AsStartOfDayUtcForTimeZone()))
                .ForRange(1.., s => s
    ...
    public static Generator<Release> WithDefaults(
    ...
    public static InstanceSetters<Release> SetPublication(
        ...
        => setters
            ...
            .Set((_, release, context) =>
            {
                publication.ReleaseSeries.Add(context.Fixture
            ...
            .Set((_, _) =>
            {
                ...
                {
                    publication.LatestPublishedReleaseVersion = publishedVersions.Count == 1
                        ? publishedVersions[0]
                        : publishedVersions
