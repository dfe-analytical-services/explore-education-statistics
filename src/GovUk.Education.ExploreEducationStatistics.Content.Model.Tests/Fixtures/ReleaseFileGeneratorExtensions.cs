...
public static class ReleaseFileGeneratorExtensions
{
    ...
    public static Generator<ReleaseFile> WithId(this Generator<ReleaseFile> generator, Guid id) =>
        generator.ForInstance(s => s.SetId(id));

    public static Generator<ReleaseFile> WithFile(this Generator<ReleaseFile> generator, File file) =>
        generator.ForInstance(s => s.SetFile(file));

    public static Generator<ReleaseFile> WithFile(this Generator<ReleaseFile> generator, Func<File> file) =>
        generator.ForInstance(s => s.SetFile(file));

    public static Generator<ReleaseFile> WithFileId(this Generator<ReleaseFile> generator, Guid fileId) =>
        generator.ForInstance(s => s.SetFileId(fileId));

    public static Generator<ReleaseFile> WithFiles(this Generator<ReleaseFile> generator, IEnumerable<File> files)
    {
    ...
    public static Generator<ReleaseFile> WithName(this Generator<ReleaseFile> generator, string name) =>
        generator.ForInstance(s => s.SetName(name));

    public static Generator<ReleaseFile> WithOrder(this Generator<ReleaseFile> generator, int order) =>
    ...
    public static Generator<ReleaseFile> WithReleaseVersions(
        this Generator<ReleaseFile> generator,
    ...
    public static InstanceSetters<ReleaseFile> SetId(this InstanceSetters<ReleaseFile> setters, Guid id) =>
        setters.Set(rf => rf.Id, id);

    public static InstanceSetters<ReleaseFile> SetFile(this InstanceSetters<ReleaseFile> setters, File file) =>
        setters.SetFile(() => file);

    public static InstanceSetters<ReleaseFile> SetFile(this InstanceSetters<ReleaseFile> setters, Func<File> file) =>
        setters.Set(rf => rf.File, file).Set(rf => rf.FileId, (_, rf) => rf.File.Id);

    public static InstanceSetters<ReleaseFile> SetFileId(this InstanceSetters<ReleaseFile> setters, Guid fileId) =>
        setters.Set(rf => rf.FileId, fileId);

    public static InstanceSetters<ReleaseFile> SetName(this InstanceSetters<ReleaseFile> setters, string name) =>
        setters.Set(rf => rf.Name, name);

    public static InstanceSetters<ReleaseFile> SetOrder(this InstanceSetters<ReleaseFile> setters, int order) =>
    ...
    public static InstanceSetters<ReleaseFile> SetReleaseVersion(
        ...
        ReleaseVersion releaseVersion
    ) => setters.Set(rf => rf.ReleaseVersion, releaseVersion).SetReleaseVersionId(releaseVersion.Id);
    ...
    public static InstanceSetters<ReleaseFile> SetPublicApiDataSetVersion(
        ...
        int minor,
        int patch = 0
    ) => setters.Set(rf => rf.PublicApiDataSetVersion, new SemVersion(major: major, minor: minor, patch: patch));
