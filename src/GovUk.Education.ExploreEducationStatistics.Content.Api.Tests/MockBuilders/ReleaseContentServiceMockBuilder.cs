...
public class ReleaseContentServiceMockBuilder
{
    ...
    private static readonly Expression<Func<IReleaseContentService,
        Task<Either<ActionResult, ReleaseContentDto>>>> GetReleaseContent =
        m => m.GetReleaseContent(
            It.IsAny<string>(),
            It.IsAny<string>(),
    ...
    public ReleaseContentServiceMockBuilder WhereGetReleaseContentReturnsNotFound(
        ...
        string releaseSlug)
    {
        _mock.Setup(m => m.GetReleaseContent(
                publicationSlug,
                releaseSlug,
                It.IsAny<CancellationToken>()))
    ...
    {
        public void GetReleaseContentWasCalled(
            string? publicationSlug = null,
            string? releaseSlug = null)
        {
            mock.Verify(m => m.GetReleaseContent(
