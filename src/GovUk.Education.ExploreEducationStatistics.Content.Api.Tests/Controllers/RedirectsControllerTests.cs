...
public abstract class RedirectsControllerTests(TestApplicationFactory testApp)
    : IntegrationTestFixture(testApp)
{
    public abstract class ListTests(TestApplicationFactory testApp)
        : RedirectsControllerTests(testApp)
    {
        ...
        public class GeneralTests(TestApplicationFactory testApp) : ListTests(testApp)
        {
            ...
            public async Task RedirectsExistInCache_Returns200WithRedirects()
            {
                ...
                [
                    ...
                    new(
                        FromSlug: "original-methodology-slug-2",
                        ToSlug: "updated-methodology-slug-2"
                    ),
                ];
                ...
                [
                    ...
                    new(
                        FromSlug: "original-publication-slug-2",
                        ToSlug: "updated-publication-slug-2"
                    ),
                ];

                var cachedReleaseRedirectsByPublicationSlug = new Dictionary<
                    string,
                    List<RedirectViewModel>
                >
                {
                    {
                        ...
                        new List<RedirectViewModel>()
                        {
                            ...
                            new(
                                FromSlug: "original-release-slug-2",
                                ToSlug: "updated-release-slug-2"
                            ),
                        }
                    },
                };
                ...
                Assert.Equal(
                    cachedMethodologyRedirects.Count,
                    viewModel.MethodologyRedirects.Count
                );
                Assert.All(
                ...
                Assert.Equal(
                    cachedPublicationRedirects.Count,
                    viewModel.PublicationRedirects.Count
                );
                Assert.All(
                ...
                var cachedReleaseRedirectsForPublication = Assert.Single(
                    cachedReleaseRedirectsByPublicationSlug
                );
                Assert.Equal(
                ...
                Assert.Equal(
                    "updated-publication-slug-1",
                    cachedReleaseRedirectsForPublication.Key
                );
                Assert.All(
                    cachedReleaseRedirectsForPublication.Value,
                    crr =>
                        Assert.Contains(
                            viewModel.ReleaseRedirectsByPublicationSlug[
                                "updated-publication-slug-1"
                            ],
        ...
        public class MethodologyRedirectsTests(TestApplicationFactory testApp) : ListTests(testApp)
        {
            ...
            public async Task RedirectsExist_Returns200WithRedirects()
            {
                Methodology methodology = DataFixture
                    ...
                    .WithMethodologyVersions(
                        DataFixture
                            ...
                            .ForIndex(
                                0,
                                s => s.SetRedirects([DataFixture.DefaultMethodologyRedirect()])
                            )
                            .ForIndex(
                                1,
                                s =>
                ...
                var redirects = methodology
                    .Versions.SelectMany(mv => mv.MethodologyRedirects)
                    .ToList();
        ...
        public class PublicationRedirectsTests(TestApplicationFactory testApp) : ListTests(testApp)
        {
            ...
            public async Task RedirectsExist_RedirectsAreCached()
            {
                ...
                Assert.Equal(
                    publicationRedirects.Count,
                    cachedRedirectsViewModel.PublicationRedirects.Count
                );
                Assert.All(
                    publicationRedirects,
                    pr =>
        ...
        public class ReleaseRedirectsTests(TestApplicationFactory testApp) : ListTests(testApp)
        {
            ...
            public async Task ReleaseRedirectDoesNotExistForPublicationWithRedirect_Returns200WithRedirects()
            {
                ...
                Assert.Equal(
                    publicationRedirect.Publication.Slug,
                    publicationRedirectViewModel.ToSlug
                );
            ...
            public async Task ReleaseRedirectExistsForPublicationWithRedirect_Returns200WithRedirects()
            {
                ...
                Assert.Contains(
                    ...
                    r =>
                        r.FromSlug == "first-release-slug-1" && r.ToSlug == "updated-release-slug-1"
                );
                Assert.Contains(
                    releaseRedirectsForPublication.Value,
                    r =>
                        r.FromSlug == "second-release-slug-1"
                        && r.ToSlug == "updated-release-slug-1"
                );
            }
