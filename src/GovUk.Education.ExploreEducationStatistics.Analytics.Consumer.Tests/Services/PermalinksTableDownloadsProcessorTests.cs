...
public abstract class PermalinksTableDownloadsProcessorTests : ProcessorTestsBase
{
    ...
    {
        return new PermalinksTableDownloadsProcessor(pathResolver: PathResolver, workflow: Workflow);
    }

    private async Task AssertReportRow(CapturePermaLinkTableDownloadLine row, string jsonFileName, int numRequests)
    {
