...
public class AnalyticsPathResolverMockBuilder
{
    ...
    {
        _mock.Setup(m => m.BuildOutputDirectory(It.IsAny<string[]>())).Returns(() => _outputDirectoryPathReturned);
    ...
    public class Asserter(Mock<IAnalyticsPathResolver> mock)
    {
        ...
        {
            mock.Verify(
                m => m.BuildOutputDirectory(It.Is<string[]>(actual => actual.SequenceEqual(expectedSubPaths))),
                Times.Once
            );
        }
    }
