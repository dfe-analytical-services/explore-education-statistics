...
public static class FootnoteGeneratorExtensions
{
    ...
    public static Generator<Footnote> WithFilterGroups(
        this Generator<Footnote> generator,
        IEnumerable<FilterGroup> filterGroups
    ) => generator.ForInstance(s => s.SetFilterGroups(filterGroups));

    public static Generator<Footnote> WithFilterItems(
        this Generator<Footnote> generator,
        IEnumerable<FilterItem> filterItems
    ) => generator.ForInstance(s => s.SetFilterItems(filterItems));

    public static Generator<Footnote> WithIndicators(
        this Generator<Footnote> generator,
    ...
    public static InstanceSetters<Footnote> SetDefaults(this InstanceSetters<Footnote> setters) =>
        setters
            ...
            .Set(f => f.Updated, (f, footnote) => f.Date.Soon(14, footnote.Created));
    ...
    {
        instanceSetter.Set(
            footnote => footnote.Subjects,
            (_, footnote, context) =>
            {
                return subjects
                    .Invoke(context)
                    .Select(subject => new SubjectFootnote { Footnote = footnote, Subject = subject })
                    .ToList();
            }
        );
    ...
    {
        instanceSetter.Set(
            footnote => footnote.Filters,
            (_, footnote, context) =>
            {
                return filters
                    .Invoke(context)
                    .Select(filter => new FilterFootnote { Footnote = footnote, Filter = filter })
                    .ToList();
            }
        );
    ...
    {
        instanceSetter.Set(
            footnote => footnote.FilterGroups,
            (_, footnote, context) =>
            {
                return filterGroups
                    .Invoke(context)
                    .Select(filterGroup => new FilterGroupFootnote { Footnote = footnote, FilterGroup = filterGroup })
                    .ToList();
            }
        );
    ...
    {
        instanceSetter.Set(
            footnote => footnote.FilterItems,
            (_, footnote, context) =>
            {
                return filterItems
                    .Invoke(context)
                    .Select(filterItem => new FilterItemFootnote { Footnote = footnote, FilterItem = filterItem })
                    .ToList();
            }
        );
    ...
    {
        instanceSetter.Set(
            footnote => footnote.Indicators,
            (_, footnote, context) =>
            {
                return indicators
                    .Invoke(context)
                    .Select(indicator => new IndicatorFootnote { Footnote = footnote, Indicator = indicator })
                    .ToList();
            }
        );
    ...
    public static InstanceSetters<Footnote> SetContent(this InstanceSetters<Footnote> instanceSetter, string content) =>
        instanceSetter.Set(f => f.Content, content);

    public static InstanceSetters<Footnote> SetOrder(this InstanceSetters<Footnote> instanceSetter, int order) =>
