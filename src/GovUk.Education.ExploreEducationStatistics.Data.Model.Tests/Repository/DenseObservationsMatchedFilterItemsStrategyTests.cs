...
public abstract class DenseObservationsMatchedFilterItemsStrategyTests
{
    public class GetFilterItemsFromMatchedObservationIdsTests : DenseObservationsMatchedFilterItemsStrategyTests
    {
        ...
        public async Task TempTablesCreated_QueriesRan_Successfully()
        {
            ...
            filterItemIds
                .SetupGet(t => t.Name)
                .Returns("#CandidateFilterItem");
            
            tempTableCreator
                .Setup(t => t.CreateAnonymousTemporaryTableAndPopulate(
                    context,
            ...
            sqlExecutor
                .Setup(s => s.ExecuteSqlRaw(
                    context,
                    It.Is<string>(sql =>
                        NormaliseSqlFormatting(sql) == NormaliseSqlFormatting(expectedQuerySql)),
                    default))
            ...
            sqlExecutor
                .Setup(s => s.ExecuteSqlRaw(
                    context,
                    It.Is<string>(sql => 
                        expectedIndexSqlRegex.IsMatch(NormaliseSqlFormatting(sql))),
                    default))
                .Returns(Task.CompletedTask);
