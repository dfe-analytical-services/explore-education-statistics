...
public class CancellationTokenTimeoutAttribute : Attribute
{
    ...
    public CancellationTokenTimeoutAttribute(string configurationKey)
    {
        ...
        var timeoutConfig = _timeoutConfiguration.GetSection(configurationKey).Value ?? throw new ArgumentException($"Could not find timeout configuration setting for " +
