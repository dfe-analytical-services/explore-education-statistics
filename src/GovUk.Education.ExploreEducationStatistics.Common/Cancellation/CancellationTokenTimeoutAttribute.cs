    ...
    public CancellationTokenTimeoutAttribute(string configurationKey)
    {
        ...
        if (_timeoutConfiguration == null)
        {
            throw new ArgumentException(
                "Timeout configuration section cannot be null when using the "
        ...
        var timeoutConfig =
            _timeoutConfiguration.GetSection(configurationKey).Value
            ?? throw new ArgumentException(
                $"Could not find timeout configuration setting for " + $"key {configurationKey}"
            );

        if (!int.TryParse(timeoutConfig, out var timeoutValue))
        {
            throw new ArgumentException(
                $"Timeout configuration setting for " + $"key {configurationKey} must be an integer"
            );
        }
