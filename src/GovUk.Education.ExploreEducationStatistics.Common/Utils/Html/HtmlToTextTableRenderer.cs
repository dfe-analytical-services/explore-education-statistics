...
internal class HtmlToTextTableRenderer
{
    ...
    {
        _headerRows.Add(GenerateTableCells(rowElement: rowElement, previousRowCells: _headerRows.LastOrDefault()));
    }

    public void AddBodyRow(IElement rowElement)
    {
        _bodyRows.Add(GenerateTableCells(rowElement: rowElement, previousRowCells: _bodyRows.LastOrDefault()));
    }
    ...
    private List<TableCell> GenerateTableCells(IElement rowElement, List<TableCell>? previousRowCells)
    {
        ...
        cellElements.ForEach(child =>
        {
            ...
            {
                var newColumns = (_columnWidths.Count - 1 - cellIndex).ToEnumerable().Select(_ => 0);
