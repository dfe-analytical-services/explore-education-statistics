            ...
            RuleFor(request => request.MetaFile)
                ...
                .Must(
                    (request, file, context) =>
                    {
                        context.MessageFormatter.AppendArgument(
                            "FileName",
                            request.MetaFile.FileName
                        );
                        return file
                            .FileName.ToLower()
                            .EndsWith(Constants.DataSet.MetaFileExtension);
                    }
                )
                .WithMessage(
    ...
    public class Validator : AbstractValidator<UploadDataSetAsZipRequest>
    {
        public Validator()
        {
            ...
            RuleFor(request => request.Title)
                ...
                .WithMessage(
                    ValidationMessages.DataSetTitleShouldNotContainSpecialCharacters,
                    "{PropertyValue}"
                );
