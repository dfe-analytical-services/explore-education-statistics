    ...
    public MappingProfiles()
    {
        ...
        CreateMap<ReleaseVersion, ReleaseVersionViewModel>()
            ...
            .ForMember(
                ...
                m =>
                    m.MapFrom(rv => rv.Release.Publication.LatestPublishedReleaseVersionId == rv.Id)
            )
            .ForMember(
...
#pragma warning restore 612
        ...
        CreateMap<MethodologyVersion, ManageMethodologyContentViewModel>()
            .ForMember(
                ...
                m =>
                    m.MapFrom(methodologyVersion =>
                        methodologyVersion.MethodologyContent.Content.OrderBy(contentSection =>
            ...
            .ForMember(
                ...
                m =>
                    m.MapFrom(methodologyVersion =>
                        methodologyVersion.MethodologyContent.Annexes.OrderBy(annexSection =>
                            annexSection.Order
                        )
                    )
            )
        ...
        CreateMap<DataSetVersion, DataSetVersionInfoViewModel>()
            .ForMember(
                dest => dest.Version,
                m => m.MapFrom(dataSetVersion => dataSetVersion.PublicVersion)
            )
            .ForMember(
                dest => dest.Type,
                m => m.MapFrom(dataSetVersion => dataSetVersion.VersionType)
            );

        CreateMap<DataSetUpload, DataSetUploadViewModel>()
            .ForMember(
                dest => dest.Status,
                m => m.MapFrom(upload => GetDataSetUploadStatus(upload.ScreenerResult))
            )
            .ForMember(
        ...
        CreateMap<DataSetUpload, DataSetScreenerRequest>()
            .BeforeMap(
                (_, d) => d.StorageContainerName = Constants.ContainerNames.PrivateReleaseTempFiles
            );
    }
    ...
    private static string GetDataSetUploadStatus(DataSetScreenerResponse screenerResult)
    {
        ...
        var hasIndividualTestFailures =
            screenerResult is null
            || screenerResult.TestResults.Any(test => test.Result == TestResult.FAIL);
        ...
        {
            ScreenerResult.Passed => screenerResult.TestResults.Any(test =>
    ...
    private void CreateContentBlockMap()
    {
        ...
        CreateMap<EmbedBlockLink, EmbedBlockLinkViewModel>()
            .ForMember(
                dest => dest.Title,
                m => m.MapFrom(embedBlockLink => embedBlockLink.EmbedBlock.Title)
            )
            .ForMember(
                dest => dest.Url,
                m => m.MapFrom(embedBlockLink => embedBlockLink.EmbedBlock.Url)
            );
