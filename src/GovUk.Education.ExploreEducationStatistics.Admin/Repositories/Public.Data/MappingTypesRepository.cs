...
public class MappingTypesRepository(PublicDataDbContext context) : IMappingTypesRepository
{
    ...
    {
        var targetDataSetVersionIdParam = new NpgsqlParameter(
        ...
        return await _context
            .Database.SqlQueryRaw<LocationMappingTypes>(
                $$$"""
                SELECT DISTINCT
                    CASE WHEN Level.value -> '{{{nameof(
                    LocationLevelMappings.Candidates
                )}}}' = '{{}}' 
                ...
                FROM
                    ...
                    jsonb_each(Level.value -> '{{{nameof(
                    LocationLevelMappings.Mappings
                )}}}') OptionMapping,
                    jsonb_extract_path_text(OptionMapping.value, '{{{nameof(
                    LocationOptionMapping.Type
                )}}}') OptionMappingType
                WHERE "{{{nameof(
                    DataSetVersionMapping.TargetDataSetVersionId
                )}}}" = @targetDataSetVersionId
                """,
    ...
    public async Task<List<FilterMappingTypes>> GetFilterOptionMappingTypes(
        Guid targetDataSetVersionId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        return await _context
            .Database.SqlQueryRaw<FilterMappingTypes>(
                ...
                FROM 
                    ...
                    jsonb_each(FilterMapping.value -> '{nameof(
                    FilterMapping.OptionMappings
                )}') OptionMapping,
                    jsonb_extract_path_text(FilterMapping.value, '{nameof(
                    FilterMapping.Type
                )}') FilterMappingType,
                    jsonb_extract_path_text(OptionMapping.value, '{nameof(
                    FilterOptionMapping.Type
                )}') OptionMappingType
                WHERE "{nameof(
                    DataSetVersionMapping.TargetDataSetVersionId
                )}" = @targetDataSetVersionId
