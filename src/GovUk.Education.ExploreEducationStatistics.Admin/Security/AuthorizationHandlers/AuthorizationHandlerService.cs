...
)
{
    ...
    )
    {
        ...
        var usersReleaseRoles = await userReleaseRoleRepository.GetAllRolesByUserAndPublication(
            userId,
            publicationId
        );
    ...
    public async Task<bool> IsReleaseVersionViewableByUser(
    ...
    )
    {
        ...
        {
            var windowStatus = preReleaseService.GetPreReleaseWindowStatus(
                releaseVersion,
                DateTime.UtcNow
            );
