...
) : AuthorizationHandler<ViewSpecificMethodologyRequirement, MethodologyVersion>
{
    ...
    )
    {
        ...
        var owningPublication = await methodologyRepository.GetOwningPublication(
        ...
        {
            var publicationIds = await methodologyRepository.GetAllPublicationIds(
                methodologyVersion.MethodologyId
            );

            foreach (var publicationId in publicationIds)
            {
                var latestReleaseVersion = await releaseVersionRepository.GetLatestReleaseVersion(
                    publicationId
                );
