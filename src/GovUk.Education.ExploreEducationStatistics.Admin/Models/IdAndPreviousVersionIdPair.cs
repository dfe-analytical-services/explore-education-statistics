...
public class IdAndPreviousVersionIdPair<TId>
    where TId : class, IComparable
{
    ...
    public bool DirectDescendantOf(IdAndPreviousVersionIdPair<TId> entityIds) =>
        entityIds.Id.Equals(PreviousVersionId);
...
{
    public static List<IdAndPreviousVersionIdPair<TId>> Sort<TId>(
        List<IdAndPreviousVersionIdPair<TId>> entities
    )
        where TId : class, IComparable
    {
        var sorted = new List<IdAndPreviousVersionIdPair<TId>>();
        var unsorted = entities.Where(entity => entity != null).ToList();
        ...
        while (unsorted.Count > 0)
        {
            ...
            {
                var directAncestor = sorted.FirstOrDefault(ancestor =>
                    descendant.DirectDescendantOf(ancestor)
                );
