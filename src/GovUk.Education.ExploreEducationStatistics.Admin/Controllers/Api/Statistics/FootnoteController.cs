...
public class FootnoteController : ControllerBase
{
    ...
    public async Task<ActionResult<FootnoteViewModel>> CreateFootnote(
    ...
    public async Task<ActionResult> DeleteFootnote(Guid releaseVersionId, Guid footnoteId)
    {
        return await _footnoteService
            .DeleteFootnote(releaseVersionId: releaseVersionId, footnoteId: footnoteId)
            .HandleFailuresOrNoContent();
    }

    [HttpGet("releases/{releaseVersionId:guid}/footnotes/{footnoteId:guid}")]
    public async Task<ActionResult<FootnoteViewModel>> GetFootnote(
        Guid releaseVersionId,
        Guid footnoteId
    )
    {
        return await _footnoteService
            .GetFootnote(releaseVersionId: releaseVersionId, footnoteId: footnoteId)
            .OnSuccess(GatherAndBuildFootnoteViewModel)
            .HandleFailuresOrOk();
    }

    [HttpGet("releases/{releaseVersionId:guid}/footnotes")]
    public async Task<ActionResult<IEnumerable<FootnoteViewModel>>> GetFootnotes(
        Guid releaseVersionId
    )
    {
        return await _footnoteService
    ...
    public async Task<ActionResult<FootnoteViewModel>> UpdateFootnote(
        Guid releaseVersionId,
        Guid footnoteId,
    ...
    public async Task<ActionResult<FootnotesMetaViewModel>> GetFootnotesMeta(Guid releaseVersionId)
    {
        return await _releaseService
            ...
            .OnSuccess(async subjects =>
            {
                ...
                {
                    Subjects = subjectMetaViewModels.ToDictionary(
    ...
    {
        return await _footnoteService
            .UpdateFootnotes(releaseVersionId, request)
            .HandleFailuresOrOk();
    }

    private async Task<Dictionary<Guid, FootnotesIndicatorsMetaViewModel>> GetIndicators(
        Guid subjectId
    )
    {
        return (await _indicatorGroupRepository.GetIndicatorGroups(subjectId))
            ...
            .ToDictionary(
                ...
                group => new FootnotesIndicatorsMetaViewModel
                {
                    Label = group.Label,
                    Options = group
                        .Indicators.OrderBy(indicator => indicator.Label, LabelComparer)
    ...
    private async Task<Dictionary<Guid, FootnotesFilterMetaViewModel>> GetFilters(Guid subjectId)
    {
        return (await _filterRepository.GetFiltersIncludingItems(subjectId)).ToDictionary(
            ...
            filter => new FootnotesFilterMetaViewModel
            {
                ...
                Legend = filter.Label,
                Options = filter
                    .FilterGroups.OrderBy(items => items.Label, LabelComparer)
                    .ToDictionary(
                        filterGroup => filterGroup.Id,
                        filterGroup =>
                            BuildFilterItemsMetaViewModel(filterGroup, filterGroup.FilterItems)
                    ),
            }
        );
    }

    private static FootnotesFilterGroupsMetaViewModel BuildFilterItemsMetaViewModel(
    ...
    {
        var filterItemsGroupByFilterGroup = footnote
            .FilterItems.GroupBy(
        ...
        var filterItemsGroupByFilter = footnote
            .FilterItems.GroupBy(
        ...
        var filterGroupsGroupByFilter = footnote
        ...
        var filterItemsByFilterGroup = filterItemsGroupByFilterGroup.ToDictionary(
            grouping => grouping.Key.Id,
            grouping => grouping.ToList()
        );

        var filterGroupsByFilter = filterGroupsGroupByFilter.ToDictionary(
            grouping => grouping.Key.Id,
            grouping => grouping.ToList()
        );

        // There can be more than one indicator belonging to the same subject in each group so we add Distinct here.
        var indicatorGroupsBySubject = footnote
            .Indicators.GroupBy(
                indicatorFootnote => indicatorFootnote.Indicator.IndicatorGroup.SubjectId,
        ...
        var indicatorsByIndicatorGroup = footnote
            .Indicators.GroupBy(
                indicatorFootnote => indicatorFootnote.Indicator.IndicatorGroupId,
        ...
        AppendFilterGroups(
            filterItemsGroupByFilterGroup.Select(group => group.Key),
        ...
        var selectedFilterGroups = footnote.FilterGroups.Select(groupFootnote =>
            groupFootnote.FilterGroupId
        );
    ...
    private static void AppendFilterGroups(
    ...
    private static FootnoteViewModel BuildFootnoteViewModel(
        Footnote footnote,
                                        ...
                                        filterGroupsByFilter
                                            ...
                                            ?.ToDictionary(
                                                ...
                                                filterGroupId => new FootnoteFilterGroupViewModel
                                                {
                                                    ...
                                                    Selected = selectedFilterGroups.Contains(
                                                        filterGroupId
                                                    ),
                                                }
                                            )
