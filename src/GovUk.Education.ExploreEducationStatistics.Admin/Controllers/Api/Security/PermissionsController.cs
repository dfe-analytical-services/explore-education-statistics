...
) : ControllerBase
{
    ...
    {
        return CheckPolicyAgainstEntity<Theme>(
            themeId,
            userService.CheckCanCreatePublicationForTheme
        );
    }
    ...
    public Task<ActionResult<bool>> CanUpdateRelease(Guid releaseVersionId)
    {
        return CheckPolicyAgainstEntity<ReleaseVersion>(
            releaseVersionId,
            userService.CheckCanUpdateReleaseVersion
        );
    }
    ...
    )
    {
        return await persistenceHelper
            ...
            {
                var canMarkDraft = await userService.CheckCanMarkReleaseVersionAsDraft(
                    releaseVersion
                );
                var canMarkHigherReview =
                    await userService.CheckCanSubmitReleaseVersionForHigherReview(releaseVersion);
                var canMarkApproved = await userService.CheckCanApproveReleaseVersion(
                    releaseVersion
                );
    ...
    public async Task<ActionResult<DataFilePermissions>> GetDataFilePermissions(
        Guid releaseVersionId,
        Guid fileId
    )
    {
    ...
    public async Task<ActionResult<PreReleaseWindowStatus>> GetPreReleaseWindowStatus(
        Guid releaseVersionId
    )
    {
        return await persistenceHelper
            ...
            .OnSuccess(releaseVersion =>
                preReleaseService.GetPreReleaseWindowStatus(releaseVersion, UtcNow)
            )
            .HandleFailuresOrOk();
    }
    ...
    public async Task<
        ActionResult<MethodologyApprovalStatusPermissions>
    > GetMethodologyApprovalPermissions(Guid methodologyVersionId)
    {
        return await persistenceHelper
            .CheckEntityExists<MethodologyVersion>(methodologyVersionId)
            .OnSuccess(async methodologyVersion =>
            {
                var canMarkDraft = await userService.CheckCanMarkMethodologyVersionAsDraft(
                    methodologyVersion
                );
                var canMarkHigherLevelReview =
                    await userService.CheckCanSubmitMethodologyForHigherReview(methodologyVersion);
                var canMarkApproved = await userService.CheckCanApproveMethodologyVersion(
                    methodologyVersion
                );
    ...
    {
        return (
            await userReleaseRoleRepository.GetDistinctRolesByUser(userService.GetUserId())
    ...
    {
        return (
            await userPublicationRoleRepository.GetDistinctRolesByUser(userService.GetUserId())
