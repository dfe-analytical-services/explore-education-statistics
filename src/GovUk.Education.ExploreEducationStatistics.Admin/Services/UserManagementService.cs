...
namespace GovUk.Education.ExploreEducationStatistics.Admin.Services
{
    public class UserManagementService : IUserManagementService
    {
        ...
        public async Task<Either<ActionResult, List<UserViewModel>>> ListPendingInvitesAsync()
        {
            return await _userService
                .CheckCanManageAllUsers()
                .OnSuccess(async () =>
                    await _usersAndRolesDbContext.UserInvites.Where(u => u.Accepted == false)
        ...
        public async Task<Either<ActionResult, UserInvite>> InviteUserAsync(string email, string inviteCreatedByUser,
            string roleId)
        {
            return await _userService
                .CheckCanManageAllUsers()
        ...
        public async Task<Either<ActionResult, UserInvite>> CancelInviteAsync(string email)
        {
            return await _userService
                .CheckCanManageAllUsers()
                .OnSuccess<ActionResult, Unit, UserInvite>(async () =>
                {
                    var invite = await _usersAndRolesDbContext.UserInvites.FirstAsync(i => i.Email == email);
                    if (invite == null)
                    {
                        return ValidationActionResult(UnableToCancelInvite);
                    }
            ...
            EnumExtensions.EnumValue role)
        {
            ...
            var email = _usersAndRolesDbContext.Users.FirstOrDefault(x =>
                x.Id == userId.ToString())?.Email;
