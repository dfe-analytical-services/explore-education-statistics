    ...
    public class PublicationService : IPublicationService
    {
            ...
            PublicationSaveViewModel publication)
        {
            return await ValidateSelectedTopic(publication.TopicId)
                ...
                .OnSuccess(async _ =>
                {
                    if (_contentDbContext.Publications.Any(p => p.Slug == publication.Slug))
                    {
                        return ValidationActionResult(SlugNotUnique);
                    }

                    var contact = await _contentDbContext.Contacts.AddAsync(new Contact
                    {
                    ...
                    var saved = await _contentDbContext.Publications.AddAsync(new Publication
                    {
            ...
            PublicationSaveViewModel updatedPublication)
        {
            return await _persistenceHelper
                ...
                { 
                    ...
                    _contentDbContext.Publications.Update(publication);
                    await _contentDbContext.SaveChangesAsync();
        ...
        private async Task<Either<ActionResult, Unit>> ValidateSelectedTopic(
            Guid? topicId)
        {
            var topic = await _contentDbContext.Topics.FindAsync(topicId);
            ...
            return await _persistenceHelper
                ...
                .OnSuccess(async publication =>
                {
                    publication.LegacyReleases.ForEach(legacyRelease =>
                    {
                        ...
                        _contentDbContext.Update(legacyRelease);
                    });
        ...
        {
            if (await _contentDbContext.Publications.AnyAsync(publication => publication.Slug == slug && publication.Id != id))
            {
                return ValidationActionResult(SlugNotUnique);
            }
