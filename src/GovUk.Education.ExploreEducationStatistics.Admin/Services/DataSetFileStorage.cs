...
) : IDataSetFileStorage
{
    ...
    )
    {
        ...
        {
            replacedReleaseDataFile = await GetReplacedReleaseFile(
                releaseVersionId,
                dataSet.ReplacingFile.Id
            );
        ...
        var releaseDataFileOrder = await GetNextDataFileOrder(
            releaseVersionId,
            replacedReleaseDataFile?.File.Id
        );
        ...
        await UploadDataSetToReleaseStorage(
            releaseVersionId,
            dataFile.Id,
            metaFile.Id,
            dataSet,
        ...
        {
            await CreateDraftDataSetVersion(
                dataReleaseFile.Id,
    ...
    )
    {
        await dataSetVersionService
            ...
            {
                var errorMessage =
                    $"Failed whilst deleting the draft version for the data set id: {dataSetId}.";
                logger.LogError(errorMessage);
                throw new InvalidOperationException(
    ...
    )
    {
        await dataSetVersionService
            ...
            .OnFailureVoid(_ =>
            {
                ...
                logger.LogError(errorMessage);
                throw new InvalidOperationException(
                    "Failed whilst deleting initial the draft version."
                );
            });
    }
    ...
    )
    {
        ...
        {
            await dataSetUploadRepository.Delete(
                releaseVersionId,
                existingUpload.Id,
                cancellationToken
            );
    ...
    )
    {
        ...
        {
            var subjectId =
                await releaseVersionRepository.CreateStatisticsDbReleaseAndSubjectHierarchy(
                    releaseVersionId
                );
            ...
            var sourceDataFilePath = FileExtensions.Path(
                releaseVersionId,
                FileType.Data,
                dataSetUpload.DataFileId
            );
            var destinationDataFilePath = FileExtensions.Path(
                releaseVersionId,
                FileType.Data,
                dataFile.Id
            ); // Same path, but a new ID has been generated by the creation step above
            ...
            var sourceMetaFilePath = FileExtensions.Path(
                releaseVersionId,
                FileType.Metadata,
                dataSetUpload.MetaFileId
            );
            var destinationMetaFilePath = FileExtensions.Path(
                releaseVersionId,
                FileType.Metadata,
                metaFile.Id
            ); // Same path, but a new ID has been generated by the creation step above
    ...
    private async Task<ReleaseFile?> GetReplacedReleaseFile(
        Guid releaseVersionId,
        Guid fileToBeReplacedId
    )
    {
    ...
    private static Either<ActionResult, Unit> ValidateFileTypeForTemporaryDataSetUploadFile(
        FileType fileType
    )
    {
        return fileType is FileType.Data or FileType.Metadata
            ...
            : ValidationUtils.ValidationResult(
                ...
                {
                    Message =
                        $"Invalid {nameof(fileType)} value {fileType} for temporary data set upload file type",
