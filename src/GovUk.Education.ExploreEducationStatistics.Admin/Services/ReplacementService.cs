...
) : IReplacementService
{
    ...
    )
    {
        chart
            ...
            ?.DataSets.ForEach(dataSet =>
            {
                ...
                if (
                    dataSet.Indicator.HasValue
                    && indicatorTargets.TryGetValue(
                        dataSet.Indicator.Value,
                        out var targetIndicatorId
                    )
                )
                {
                    dataSet.Indicator = targetIndicatorId;
                ...
                {
                    if (
                        locationTargets.TryGetValue(
                            dataSet.Location.Value,
                            out var targetLocationId
                        )
                    )
                    {
                        dataSet.Location.Value = targetLocationId;
    ...
    private async Task ReplaceFootnoteSubject(
        Guid footnoteId,
        Guid originalSubjectId,
        Guid replacementSubjectId
    )
    ...
    private async Task ReplaceFootnoteFilterGroup(
        Guid footnoteId,
        TargetableReplacementViewModel plan
    )
    {
    ...
    private async Task ReplaceFootnoteFilterItem(
        Guid footnoteId,
        TargetableReplacementViewModel plan
    )
    {
    ...
    private async Task ReplaceIndicatorFootnote(
        Guid footnoteId,
        TargetableReplacementViewModel plan
    )
    {
