...
) : IDataSetService
{
    ...
    {
        var previousReleaseIds = releaseFilesByDataSetVersionId.Select(r => r.Value.ReleaseVersion.ReleaseId).ToList();
    ...
    private async Task<DataSetViewModel> MapDataSet(DataSet dataSet, CancellationToken cancellationToken)
    {
        ...
        var versionNumberBeforePatch =
            dataSet.LatestDraftVersion?.VersionPatch > 0
                ? dataSet.LatestDraftVersion?.SemVersion().WithPatch(dataSet.LatestDraftVersion.VersionPatch - 1)
                : null;
        ...
        var latestLiveVersion = dataSet.LatestLiveVersion is null
            ? null
            : MapLiveVersion(dataSet.LatestLiveVersion, releaseFilesByDataSetVersionId[dataSet.LatestLiveVersion.Id]);

        var allReleaseIds = releaseFilesByDataSetVersionId.Select(r => r.Value.ReleaseVersion.ReleaseId).ToList();
    ...
    {
        var dataSetVersionsByReleaseFileId = dataSet.Versions.ToDictionary(dsv => dsv.Release.ReleaseFileId);
    ...
    private static DataSetLiveVersionViewModel MapLiveVersion(DataSetVersion dataSetVersion, ReleaseFile releaseFile)
    {
