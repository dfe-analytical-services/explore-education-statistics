...
{
    public async Task<
        Either<ActionResult, PaginatedListViewModel<DataSetSummaryViewModel>>
    > ListDataSets(
        int page,
        int pageSize,
        Guid publicationId,
            ...
            .OnSuccess(async () =>
            {
                ...
                var dataSets = await dataSetsQueryable
                    .OrderByDescending(ds =>
                        ds.LatestLiveVersion != null
                            ? ds.LatestLiveVersion.Published
                ...
                var dataSetReleaseVersions = await GetDataSetReleaseVersions(
                    dataSets,
                    cancellationToken
                );
                ...
                var results = dataSets
                    .Select(ds =>
                        MapDataSetSummary(
                            ds,
                            dataSetReleaseVersions[ds.Id].ReleaseFilesByDataSetVersionId
                        )
                    )
                    .ToList();
                ...
                return new PaginatedListViewModel<DataSetSummaryViewModel>(
                    results,
                    totalResults: await dataSetsQueryable.CountAsync(
    ...
    )
    {
        return await userService
            .CheckIsBauUser()
            .OnSuccess(async _ =>
                await processorClient.CreateDataSet(
                    releaseFileId: releaseFileId,
                    cancellationToken: cancellationToken
                )
            )
            .OnSuccess(async processorResponse =>
                await QueryDataSet(processorResponse.DataSetId).SingleAsync(cancellationToken)
            )
            .OnSuccess(async dataSet => await MapDataSet(dataSet, cancellationToken));
    }
    ...
    )
    {
        ...
        return new DataSetSummaryViewModel
        {
            ...
            DraftVersion = MapDraftSummaryVersion(
                ...
                releaseFilesByDataSetVersionId
            ),
    ...
    )
    {
        return dataSetVersion != null
            ? new DataSetVersionSummaryViewModel
            {
                ...
                ReleaseVersion = MapReleaseVersion(
                    releaseFilesByDataSetVersionId[dataSetVersion.Id].ReleaseVersion
                ),
    ...
    private async Task<DataSetViewModel> MapDataSet(
        DataSet dataSet,
        CancellationToken cancellationToken
    )
    {
        var releaseFilesByDataSetVersionId = await GetReleaseFilesByDataSetVersionId(
            dataSet,
            cancellationToken
        );
        ...
        var versionNumberBeforePatch =
            dataSet.LatestDraftVersion?.VersionPatch > 0
                ? dataSet
                    .LatestDraftVersion?.SemVersion()
    ...
    )
    {
        ...
        return releaseFiles
            ...
            .ToDictionary(
                ...
                {
                    ReleaseFilesByDataSetVersionId = group.ToDictionary(
    ...
    {
        var dataSetVersionsByReleaseFileId = dataSet.Versions.ToDictionary(dsv =>
            dsv.Release.ReleaseFileId
        );
    ...
    )
    {
        return new DataSetDraftVersionViewModel
        {
            ...
            GeographicLevels =
                dataSetVersion.MetaSummary?.GeographicLevels.Select(l => l.GetEnumLabel()).ToList()
                ?? null,
    ...
    )
    {
        return new DataSetLiveVersionViewModel
        {
            ...
            Notes = dataSetVersion.Notes,
            ReleaseVersion = MapReleaseVersion(releaseFile.ReleaseVersion),
            GeographicLevels = dataSetVersion
                .MetaSummary!.GeographicLevels.Select(l => l.GetEnumLabel())
                .ToList(),
            TimePeriods = TimePeriodRangeViewModel.Create(
                dataSetVersion.MetaSummary.TimePeriodRange
            ),
    ...
    private record DataSetReleaseVersions
    {
        public IReadOnlyDictionary<
            Guid,
            ReleaseFile
        > ReleaseFilesByDataSetVersionId { get; init; } = new Dictionary<Guid, ReleaseFile>();
    }
}
