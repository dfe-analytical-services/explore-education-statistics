    ...
    : IDataSetVersionMappingService
{
        ...
        CancellationToken cancellationToken)
    {
        return await request
            ...
            .SelectAwait(async (updateRequest, index) => await UpdateMapping
                <LocationOptionMapping, MappableLocationOption, LocationMappingUpdateRequest, LocationMappingUpdateResponse>(
                    nextDataSetVersionId: nextDataSetVersionId,
                    ...
                    createSuccessfulResponseFn: mappingUpdate => new LocationMappingUpdateResponse
                    {
    ...
    /// </summary>
    private async Task<List<Either<ErrorViewModel, FilterOptionMappingUpdateResponse>>> UpdateFilterOptionMappingsBatch(
        Guid nextDataSetVersionId,
        BatchFilterOptionMappingUpdatesRequest request,
        CancellationToken cancellationToken)
    {
        return await request
            ...
            .ToAsyncEnumerable()
            .SelectAwait(async (updateRequest, index) => await UpdateMapping
                <FilterOptionMapping, MappableFilterOption, FilterOptionMappingUpdateRequest, FilterOptionMappingUpdateResponse>(
                    nextDataSetVersionId: nextDataSetVersionId,
                    updateRequest: updateRequest,
                    index: index,
                    ...
                    createSuccessfulResponseFn: mappingUpdate => new FilterOptionMappingUpdateResponse
                    {
        ...
        where TMappingUpdateResponse: MappingUpdateResponse<TMapping, TMappableElement>
    {
        ...
        return await postgreSqlRepository
            .UpdateJsonbAtPath(
                publicDataDbContext,
                updateJsonRequest,
                (TMapping mapping) => mapping is not null
                    ? mapping with
                    {
