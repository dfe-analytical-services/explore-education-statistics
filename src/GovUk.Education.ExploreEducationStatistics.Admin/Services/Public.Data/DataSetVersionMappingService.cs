...
) : IDataSetVersionMappingService
{
    ...
    )
    {
        ...
        return await publicDataDbContext
            .DataSetVersionMappings.Where(mapping => mapping.TargetDataSetVersionId == dataSetVersionId)
    ...
    private async Task<Either<ActionResult, TBatchMappingUpdatesResponseViewModel>> ApplyBatchMappingUpdates<
        ...
        return await publicDataDbContext.RequireTransaction(async () =>
            await userService
                ...
                .OnSuccess(validationResults =>
                {
                    ...
                    return aggregatedResult.OnFailure<ActionResult>(errors => ValidationUtils.ValidationResult(errors));
                })
    ...
    )
    {
        var sourceDataSetVersion = await publicDataDbContext
            .DataSetVersionMappings.Where(mapping => mapping.TargetDataSetVersionId == nextDataSetVersionId)
        ...
        var isReplacingApiDataFile = await publicDataDbContext.DataSetVersionImports.AnyAsync(
            i => i.DataSetVersionToReplaceId != null && i.DataSetVersionToReplaceId == sourceDataSetVersion.Id,
            cancellationToken
        );
