...
) : IDataSetVersionMappingService
{
    ...
    )
    {
        return ApplyBatchMappingUpdates(
            ...
            applyUpdatesFn: () =>
                UpdateLocationOptionMappingsBatch(nextDataSetVersionId, request, cancellationToken),
            createViewModelFn: updates => new BatchLocationMappingUpdatesResponseViewModel
            {
                Updates = updates,
            },
    ...
    )
    {
        ...
        return await publicDataDbContext
            .DataSetVersionMappings.Where(mapping =>
                mapping.TargetDataSetVersionId == dataSetVersionId
            )
    ...
    )
    {
        return ApplyBatchMappingUpdates(
            ...
            applyUpdatesFn: () =>
                UpdateFilterOptionMappingsBatch(nextDataSetVersionId, request, cancellationToken),
            createViewModelFn: updates => new BatchFilterOptionMappingUpdatesResponseViewModel
            {
                Updates = updates,
            },
            cancellationToken: cancellationToken
    ...
    private async Task<
        Either<ActionResult, TBatchMappingUpdatesResponseViewModel>
    > ApplyBatchMappingUpdates<
        TMappingUpdateResponse,
        TBatchMappingUpdatesResponseViewModel,
        TMappingCandidate
    >(
        Guid nextDataSetVersionId,
        ...
        return await publicDataDbContext.RequireTransaction(async () =>
            await userService
                ...
                .OnSuccess(validationResults =>
                {
                    ...
                    return aggregatedResult.OnFailure<ActionResult>(errors =>
                ...
                .OnSuccess(updateSuccessesAndFailures =>
                {
                    ...
                    var aggregatedResult =
                        updateSuccessesAndFailures.AggregateSuccessesAndFailures();
                ...
                .OnSuccessDo(() =>
                    UpdateMappingsCompleteAndVersion(nextDataSetVersionId, cancellationToken)
                )
                .OnSuccess(createViewModelFn)
        );
    }

    private async Task UpdateMappingsCompleteAndVersion(
        Guid nextDataSetVersionId,
        CancellationToken cancellationToken
    )
    {
    ...
    )
    {
        var sourceDataSetVersion = await publicDataDbContext
            .DataSetVersionMappings.Where(mapping =>
                mapping.TargetDataSetVersionId == nextDataSetVersionId
            )
        ...
        var isReplacingApiDataFile = await publicDataDbContext.DataSetVersionImports.AnyAsync(
            i =>
                i.DataSetVersionToReplaceId != null
        ...
        var releaseFile = await contentDbContext
            .ReleaseFiles.Where(rf => rf.Id == targetDataSetVersion.Release.ReleaseFileId)
            .SingleAsync(cancellationToken);
    ...
    )
    {
        var majorChangesStatus = new MajorChangesStatus
        {
            ...
            LocationsHaveMajorChange = locationMappingTypes.Any(types =>
                NoMappingTypes.Contains(types.LocationLevel)
    ...
    )
    {
        return await publicDataDbContext
            .DataSetVersionMappings.Where(mapping =>
                mapping.TargetDataSetVersionId == targetDataSetVersionId
            )
            .Select(mapping =>
    ...
    )
    {
        ...
        await publicDataDbContext
            ...
            .ExecuteUpdateAsync(
                setPropertyCalls: s =>
                    s.SetProperty(
                            mapping => mapping.LocationMappingsComplete,
                            locationMappingsComplete
                        )
                        .SetProperty(
                            mapping => mapping.FilterMappingsComplete,
                            filterMappingsComplete
                        ),
                cancellationToken: cancellationToken
            );
    }
    ...
    /// </summary>
    private async Task<
        List<Either<ErrorViewModel, LocationMappingUpdateResponseViewModel>>
    ...
    {
        var filterKeys = request.Updates.Select(update => update.FilterKey).Distinct();
        ...
        return await filterKeys
            ...
            .ToDictionaryAwaitAsync(
                ...
                elementSelector: async filterKey =>
                {
                    ...
                    return await postgreSqlRepository.GetJsonbFromPath<
                        PublicDataDbContext,
                        Guid,
                        string
                    >(
    ...
    /// </summary>
    private async Task<
        List<Either<ErrorViewModel, FilterOptionMappingUpdateResponseViewModel>>
            ...
            .SelectAwait(
                async (updateRequest, index) =>
                    await UpdateMapping<
                        ...
                        FilterOptionMappingUpdateRequest,
    ...
    /// </summary>
    private async Task<Either<ErrorViewModel, Unit>> ValidateMappingCandidate(
        Guid nextDataSetVersionId,
        int index,
    ...
    )
    {
        ...
        var candidateExists = await postgreSqlRepository.KeyExistsAtJsonbPath(
            publicDataDbContext,
            jsonRequest,
            candidateKey,
            cancellationToken: cancellationToken
    ...
    /// Given a mapping update request, this method will return either a success or failure response.
    /// </summary>
    private async Task<Either<ErrorViewModel, TMappingUpdateResponse>> UpdateMapping<
        TMapping,
        TMappableElement,
        TMappingUpdateRequest,
        TMappingUpdateResponse
    >(
        Guid nextDataSetVersionId,
                ...
                (TMapping? mapping) =>
                    Task.FromResult(
                        mapping is not null
                            ...
                            : new Either<ErrorViewModel, TMapping?>(
                                new ErrorViewModel
                                {
                                    Code = ValidationMessages
                                        .DataSetVersionMappingSourcePathDoesNotExist
                                        .Code,
