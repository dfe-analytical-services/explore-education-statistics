...
) : IDataSetVersionService
{
    ...
    )
    {
        return await userService
            ...
            .OnSuccess(async dataSet =>
            {
                ...
                var releasesVersionsByDataSetVersionId = await GetReleaseFilesByDataSetVersionId(
                    dataSetVersions,
                    cancellationToken
                );
                ...
                var results = dataSetVersions
                    .Select(dsv =>
                        MapLiveVersionSummary(dsv, releasesVersionsByDataSetVersionId[dsv.Id])
                    )
                    .ToList();

                return new PaginatedListViewModel<DataSetLiveVersionSummaryViewModel>(
                    results,
                    totalResults: await dataSetVersionsQueryable.CountAsync(
    ...
    )
    {
        ...
        return await publicDataDbContext
            .DataSetVersions.Where(dataSetVersion =>
                releaseFileIds.Contains(dataSetVersion.Release.ReleaseFileId)
            )
    ...
    )
    {
        return await userService
            ...
            .OnSuccess(async dataSetVersion =>
                await MapDraftVersionSummary(dataSetVersion, cancellationToken)
            );
    }

    public async Task<
        Either<ActionResult, DataSetVersionSummaryViewModel>
    ...
    )
    {
        return await userService
            ...
            .OnSuccess(async dataSetVersion =>
                await MapDraftVersion(dataSetVersion, cancellationToken)
            );
    }
    ...
    {
        var dataSetVersionsByReleaseFileId = dataSetVersions.ToDictionary(dsv =>
