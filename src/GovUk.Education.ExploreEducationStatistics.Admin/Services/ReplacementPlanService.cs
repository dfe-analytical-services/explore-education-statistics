...
) : IReplacementPlanService
{
    ...
    )
    {
        ...
        var isPatch =
            DataSetVersionNumber.TryParse(apiDataSetVersionPlan.Version, out var number)
            && number.Patch > 0;
        ...
        return apiDataSetVersionPlan with
        {
            MappingStatus =
                mappingStatus
                ?? (apiDataSetVersionPlan.ReadyToPublish ? completeStatusResult : null), // If no mapping is found, this data set version was deleted and recreated (& no mapping was necessary)
            Valid =
                (
                    isPatch
                        ? mappingStatus is { IsMajorVersionUpdate: false }
        ...
        return await GetLinkedDataSetVersion(
            ...
            .OnSuccess(async replacementApiDataSetVersion =>
            {
                ...
                var apiDataSetVersionPlan = replacementApiDataSetVersion is null
                    ? null
                    : await GetApiVersionPlanViewModel(
                        replacementApiDataSetVersion,
                        cancellationToken
                    );
    ...
    )
    {
        ...
        var newlyIntroducedFilters = FindNewlyIntroducedFiltersForDataBlock(
            dataBlock,
            replacementSubjectMeta
        );
        var indicatorGroups = ValidateIndicatorGroupsForDataBlock(
            dataBlock,
            replacementSubjectMeta
        );
    ...
    private static Dictionary<
        Guid,
        IndicatorGroupReplacementViewModel
    > ValidateIndicatorGroupsForFootnote(
        Footnote footnote,
        ReplacementSubjectMeta replacementSubjectMeta
    )
    {
    ...
    private static FilterReplacementViewModel CreateNewlyIntroducedFilterReplacementViewModel(
        Filter filter
    )
    {
        var filterGroupReplacementViewModels = filter
            .FilterGroups.Select(fg => new FilterGroupReplacementViewModel(
                fg.Id,
                fg.Label,
                fg.FilterItems.Select(fi => new FilterItemReplacementViewModel(
                    fi.Id,
                    fi.Label,
                    null
                ))
                                ...
                                group =>
                                    ValidateFilterGroupForReplacement(
                                        new FilterGroup
                                        {
                                            ...
                                            Label = group.Key.Label,
                                            FilterItems = group
                                                .Key.FilterItems.Intersect(filter)
                                                .ToList(),
                                        },
                                        replacementSubjectMeta
                                    )
    ...
    private Dictionary<
        Guid,
        IndicatorGroupReplacementViewModel
    > ValidateIndicatorGroupsForDataBlock(
        DataBlock dataBlock,
        ReplacementSubjectMeta replacementSubjectMeta
    )
    {
    ...
    )
    {
        return new FilterGroupReplacementViewModel(
            ...
            label: filterGroup.Label,
            filters: filterGroup
                .FilterItems.Select(item =>
                    ValidateFilterItemForReplacement(item, replacementSubjectMeta)
                )
    ...
    private static Filter? FindReplacementFilter(
        ReplacementSubjectMeta replacementSubjectMeta,
        string filterName
    )
    {
    ...
    {
        var replacementFilterGroup = FindReplacementFilterGroup(
            replacementSubjectMeta,
            filterName,
            filterGroupLabel
        );
    ...
    private static Guid? FindReplacementIndicator(
        ReplacementSubjectMeta replacementSubjectMeta,
