...
{
    public async Task<Either<ActionResult, ReleaseVersionViewModel>> CreateRelease(
    ...
    )
    {
        return await ReleaseCreateRequestValidator
            .Validate(request)
            .OnSuccess(async () =>
                await context.Publications.SingleOrNotFoundAsync(p => p.Id == request.PublicationId)
            )
            ...
            .OnSuccess(async publicationAndPublishingOrganisations =>
            {
                ...
                {
                    await CreateGenericContentFromTemplate(
                        request.TemplateReleaseId.Value,
                        newReleaseVersion
                    );
                }
                else
                ...
                publication.ReleaseSeries.Insert(
                    0,
    ...
    )
    {
        return await GetRelease(releaseId)
            ...
            .OnSuccess(async release =>
                await ValidateUpdateRequest(
                    release: release,
                    request: request,
                    cancellationToken: cancellationToken
                )
            )
            .OnSuccessDo(async releaseAndSlugs =>
                await UpdateRelease(
            ...
            .OnSuccess(async releaseAndSlugs =>
            {
                ...
                var slugHasChanged =
                    releaseAndSlugs.oldReleaseSlug != releaseAndSlugs.newReleaseSlug;
    ...
    private async Task<
        Either<ActionResult, (Release release, string oldReleaseSlug, string newReleaseSlug)>
    ...
    )
    {
        ...
        return await releaseSlugValidator
            .ValidateNewSlug(
            ...
            .OnSuccess(async _ =>
                await ValidateReleaseIsNotUndergoingPublishing(release.Id, cancellationToken)
            )
    ...
    {
        var latestUnpublishedReleaseVersion = await context
            .ReleaseVersions.LatestReleaseVersion(releaseId: releaseId, publishedOnly: false)
        ...
        var releaseVersionPublishingStartedStatuses =
            await releasePublishingStatusRepository.GetAllByOverallStage(
    ...
    {
        var trimmedNewLabel = string.IsNullOrWhiteSpace(releaseLabel) ? null : releaseLabel.Trim();
    ...
    )
    {
        return await context
            .ReleaseVersions.LatestReleaseVersions(
    ...
    )
    {
        ...
        await context.ReleaseRedirects.AddAsync(
            newReleaseRedirect,
            cancellationToken: cancellationToken
        );
        await context.SaveChangesAsync(cancellationToken: cancellationToken);
