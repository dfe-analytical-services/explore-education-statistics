    ...
    )
    {
        return await _persistenceHelper
            ...
            .OnSuccess(async _ =>
                await fileIds
                    .Select(fileId =>
                        CheckFileExists(releaseVersionId: releaseVersionId, fileId: fileId, DeletableFileTypes)
                    )
                    .OnSuccessAll()
            )
            .OnSuccessVoid(async files =>
            {
                ...
                {
                    await _releaseFileRepository.Delete(releaseVersionId: releaseVersionId, fileId: file.Id);
                    if (
                    ...
                    {
                        await _privateBlobStorageService.DeleteBlob(PrivateReleaseFiles, file.Path());
        ...
        return _persistenceHelper
            ...
            .OnSuccess(_ =>
                _persistenceHelper.CheckEntityExists<ReleaseFile>(q =>
                    q.Where(rf =>
                        rf.ReleaseVersionId == releaseVersionId && rf.FileId == fileId && rf.File.Type == FileType.Data
                    )
                )
            )
            .OnSuccessVoid(async releaseFile =>
            {
                ...
                var releaseFileDataHasChanged =
                    !string.IsNullOrWhiteSpace(update.Title) || !string.IsNullOrWhiteSpace(update.Summary);
        ...
        return _persistenceHelper
            ...
            .OnSuccess(async () =>
            {
                ...
                await _privateBlobStorageService.UploadFile(
                    containerName: PrivateReleaseFiles,
                    path: FileExtensions.Path(rootPath: releaseVersionId, type: Ancillary, fileId: newFileId),
                    file: upload.File
                );
        ...
        return _persistenceHelper
            ...
            .OnSuccess(_ =>
                _persistenceHelper.CheckEntityExists<ReleaseFile>(query =>
                    query
                        .Include(rf => rf.File)
                        .Where(rf =>
                            rf.FileId == fileId && rf.ReleaseVersionId == releaseVersionId && rf.File.Type == Ancillary
                        )
                )
            )
            .OnSuccessDo(async _ =>
            ...
            .OnSuccess(async releaseFile =>
            {
                if (request.File != null)
                {
                    ...
                    {
                        await _privateBlobStorageService.DeleteBlob(PrivateReleaseFiles, oldFile.Path());
