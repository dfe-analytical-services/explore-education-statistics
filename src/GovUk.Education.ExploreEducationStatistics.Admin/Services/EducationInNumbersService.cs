...
public class EducationInNumbersService(ContentDbContext contentDbContext, IUserService userService)
    : IEducationInNumbersService
{
    ...
    public async Task<
        Either<ActionResult, List<EinSummaryWithPrevVersionViewModel>>
    > ListLatestPages()
    {
        var uniqueSlugs = await contentDbContext
            .EducationInNumbersPages.Select(p => p.Slug)
            .Distinct()
            .ToListAsync();
    ...
    )
    {
        ...
        var pageWithSlugAlreadyExists = contentDbContext.EducationInNumbersPages.Any(page =>
            page.Slug == slug
        );
        ...
        var currentMaxOrder = contentDbContext
            .EducationInNumbersPages.Select(page => page.Order)
            .Max();
    ...
    public async Task<Either<ActionResult, EinSummaryViewModel>> CreateAmendment(Guid id)
    {
        return await contentDbContext
            ...
            .OnSuccess(async page =>
            {
                ...
                var amendmentAlreadyExists = contentDbContext.EducationInNumbersPages.Any(
                    amendment =>
                ...
                var amendmentContent = page
                    .Content.Select(section => section.Clone(amendment.Id))
                    .ToList();
    ...
    )
    {
        return await contentDbContext
            ...
            .OnSuccess(async page =>
            {
                ...
                {
                    var pageWithTitleAlreadyExists = contentDbContext.EducationInNumbersPages.Any(
                        p => p.Title == request.Title
                    );
                    if (pageWithTitleAlreadyExists)
                    {
                        return new Either<ActionResult, EinSummaryViewModel>(
        ...
        newOrder.ForEach(
            (pageId, order) =>
            {
                ...
                if (matchingPage is { Version: > 0, Published: null })
                {
                    ...
                    var previousPageVersion = contentDbContext.EducationInNumbersPages.Single(
                        page =>
                            page.Slug == matchingPage.Slug
