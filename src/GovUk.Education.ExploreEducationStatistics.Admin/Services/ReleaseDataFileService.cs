...
) : IReleaseDataFileService
{
    ...
    )
    {
        return await persistenceHelper
            ...
            .OnSuccess(async _ =>
                await fileIds
                    .Select(fileId => releaseFileService.CheckFileExists(releaseVersionId, fileId, FileType.Data))
                    .OnSuccessAll()
            )
                    ...
                    else
                    {
                        ...
                        var mappings = await contentDbContext
                            .DataSetMappings.Where(map =>
                                map.OriginalDataFileId == file.Id || map.ReplacementDataFileId == file.Id
                            )
                            .ToListAsync();
    ...
    )
    {
        return await persistenceHelper
            ...
            .OnSuccess(async releaseFile =>
            {
                var dataBlocks = (await dataBlockService.ListDataBlocks(releaseFile.ReleaseVersionId))
                ...
                return new DataSetAccoutrementsViewModel
                {
                    DataBlocks = dataBlocks
                        .Select(db => new DataBlockAccoutrementViewModel { Id = db.Id, Name = db.Name })
                        .ToList(),
                    Footnotes = footnotes
                        .Select(fn => new FootnoteAccoutrementViewModel { Id = fn.Id, Content = fn.Content })
                        .ToList(),
                };
            });
    }
        ...
        return await persistenceHelper
            ...
            .OnSuccess(async () =>
            {
                ...
                var inProgressReplacementsInCurrentReleaseVersion = releaseFiles
                    ...
                    .Select(rf =>
                        releaseFilesByFileId.TryGetValue(rf.File.ReplacedById!.Value, out var replacementReleaseFile)
            ...
            .OnSuccess(async _ =>
            {
                ...
                fileIds.ForEach(
                    (fileId, order) =>
                    {
                        ...
                        {
                            var replacingReleaseFile = contentDbContext.ReleaseFiles.Single(releaseFile =>
    ...
    )
    {
        return await persistenceHelper
            ...
            .OnSuccess(dataSetUploads => dataSetUploads.ToList());
    }
    ...
    )
    {
        ...
        {
            await dataSetValidator.ValidateDataSet(dataSetDto).OnSuccessDo(dataSets.Add).OnFailureDo(errors.AddRange);
        }

        return errors.Count > 0 ? ValidationUtils.ValidationResult(errors) : dataSets;
    }

    private static List<DataSetDto> BuildDataSetsFromBulkZip(DataSetIndex dataSetIndex, List<FileDto> dataSetFiles)
    {
    ...
    private static FileDto BuildDataSetFile(Func<Stream> fileStreamProvider, string fileName, long fileSize)
    {
        return new FileDto
        {
            FileName = fileName,
            ...
            .OnSuccess(_ =>
                dataSetUploadIds
                    .Select(dataSetUploadId =>
                        ValidateDataSetUploadExistence(releaseVersionId, dataSetUploadId, cancellationToken)
                    )
                    .OnSuccessAll()
            )
            .OnSuccess(dataSetUploads => dataSetUploads.Select(ValidateDataSetCanBeImported).OnSuccessAll())
    ...
    {
        var files = releaseFiles.Concat(inProgressReplacementsInCurrentReleaseVersion).Select(rf => rf.File).ToList();
