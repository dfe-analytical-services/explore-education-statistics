...
namespace GovUk.Education.ExploreEducationStatistics.Admin.Services
{
    public class ReleaseDataFileService : IReleaseDataFileService
    {
            ...
            return await _persistenceHelper
                ...
                .OnSuccess(async _ =>
                {
                    ...
                    foreach (var archiveDataSetFile in archiveDataSetFiles)
                    {
                        ...
                        var sourceDataFilePath = FileStoragePathUtils.FilesPath(releaseVersionId, FileType.Data, archiveDataSetFile.DataFileId);
                        var destinationDataFilePath = FileStoragePathUtils.FilesPath(releaseVersionId, FileType.Data, dataFile.Id); // Same path, but a new ID has been generated by the creation step above
                        ...
                        var sourceMetaFilePath = FileStoragePathUtils.FilesPath(releaseVersionId, FileType.Metadata, archiveDataSetFile.MetaFileId);
                        var destinationMetaFilePath = FileStoragePathUtils.FilesPath(releaseVersionId, FileType.Metadata, metaFile.Id); // Same path, but a new ID has been generated by the creation step above
        ...
        {
            var dataBlobExists = await _privateBlobStorageService.CheckBlobExists(PrivateReleaseTempFiles, $"{FileStoragePathUtils.FilesPath(releaseVersionId, FileType.Data, fileImportRequest.DataFileId)}");
            var metaBlobExists = await _privateBlobStorageService.CheckBlobExists(PrivateReleaseTempFiles, $"{FileStoragePathUtils.FilesPath(releaseVersionId, FileType.Metadata, fileImportRequest.MetaFileId)}");
