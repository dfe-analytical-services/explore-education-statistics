            ...
            CancellationToken cancellationToken)
        {
            return await _persistenceHelper
                ...
                .OnSuccess(async dataSetFiles =>
                {
                    ...
                    {
                        var dataFileId = await UploadFileToTempStorage(
                            releaseVersionId,
                            archive.GetEntry(file.DataFilename)!,
                            FileType.Data,
                            cancellationToken);
                        var metaFileId = await UploadFileToTempStorage(
                            releaseVersionId,
                            archive.GetEntry(file.MetaFilename)!,
                            FileType.Metadata,
                            cancellationToken);
            ...
            return await _persistenceHelper
                ...
                .OnSuccess(async _ =>
                {
                    ...
                    foreach (var archiveDataSetFile in archiveDataSetFiles)
                    {
                        ...
                        var dataFile = await _releaseDataFileRepository.Create(
                            fileId: archiveDataSetFile.DataFileId,
                            releaseVersionId: releaseVersionId,
                            subjectId: subjectId,
                        ...
                        var metaFile = await _releaseDataFileRepository.Create(
                            fileId: archiveDataSetFile.MetaFileId,
                            releaseVersionId: releaseVersionId,
                            subjectId: subjectId,
