...
public class ClaimsPrincipalTransformationService : IClaimsTransformation
{
    ...
    private static void TransferUnsupportedClaims(
    ...
    )
    {
        ...
        {
            var unsupportedScopeClaim = principal.FindFirst(claim =>
                claim.Type == EesClaimTypes.KeycloakScope
            );
            if (unsupportedScopeClaim != null)
            {
                localIdentity.AddClaim(
                    new Claim(EesClaimTypes.SupportedMsalScope, unsupportedScopeClaim.Value)
                );
            }
        }
    }
