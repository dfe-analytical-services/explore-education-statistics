...
public class DataBlockService : IDataBlockService
{
    ...
    public Task<Either<ActionResult, Unit>> DeleteDataBlocks(
        DeleteDataBlockPlanViewModel deletePlan
    )
    {
        return InvalidateDataBlockCaches(deletePlan)
            .OnSuccessVoid(async () =>
            {
                ...
                var keyStats = _context.KeyStatisticsDataBlock.Where(ks =>
                    dataBlockIds.Contains(ks.DataBlockId)
                );
    ...
    public async Task<Either<ActionResult, Unit>> RemoveChartFile(
        Guid releaseVersionId,
        Guid fileId
    )
    {
        return await RemoveInfographicChartFromDataBlock(
                releaseVersionId: releaseVersionId,
                fileId: fileId
            )
            .OnSuccess(async () =>
    ...
    public async Task<Either<ActionResult, DataBlockViewModel>> Get(Guid dataBlockVersionId)
    {
        return await GetDataBlockVersion(dataBlockVersionId)
            .OnSuccessDo(dataBlockVersion =>
                _userService.CheckCanViewReleaseVersion(dataBlockVersion.ReleaseVersion)
            )
    ...
    public async Task<Either<ActionResult, List<DataBlockSummaryViewModel>>> List(
        Guid releaseVersionId
    )
    {
        return await _persistenceHelper
            ...
            .OnSuccess(async releaseVersion =>
            {
                ...
                return dataBlocks
                    .Select(block =>
                    {
                        var featuredTable = featuredTables.SingleOrDefault(ft =>
                            ft.DataBlockId == block.Id
                        );

                        var inContent =
                            block.ContentSectionId != null
    ...
    {
        return await GetDataBlockVersion(dataBlockVersionId)
            .OnSuccessDo(dataBlock =>
                _userService.CheckCanUpdateReleaseVersion(dataBlock.ReleaseVersion)
            )
            .OnSuccessDo(async dataBlockVersion =>
            {
                // Remove old infographic file if using a new file
                var infographicChart = dataBlockVersion
                    .Charts.OfType<InfographicChart>()
                    .FirstOrDefault();
                var updatedInfographicChart = updateRequest
                    .Charts.OfType<InfographicChart>()
                    .FirstOrDefault();
            ...
            .OnSuccessDo(dataBlock =>
                InvalidateCachedDataBlock(dataBlock.ReleaseVersionId, dataBlock.Id)
            )
    ...
    )
    {
        return await _persistenceHelper
            ...
            .OnSuccessDo(dataBlockVersion =>
                _userService.CheckCanUpdateReleaseVersion(dataBlockVersion.ReleaseVersion)
            )
    ...
    public async Task<DeleteDataBlockPlanViewModel> GetDeletePlan(
        Guid releaseVersionId,
        Subject? subject
    )
    {
    ...
    private async Task<DependentDataBlock> CreateDependentDataBlock(
    ...
    private async Task<Either<ActionResult, Unit>> DeleteDependentDataBlocks(
        DeleteDataBlockPlanViewModel deletePlan
    )
    {
    ...
    private List<DataBlockVersion> GetDataBlockVersions(
        Guid releaseVersionId,
        Guid? subjectId = null
    )
    {
        return _context
            ...
            .ToList()
            .Where(dataBlockVersion =>
                subjectId == null || dataBlockVersion.Query.SubjectId == subjectId
            )
            .ToList();
    }
    ...
    private Task<Either<ActionResult, Unit>> InvalidateDataBlockCaches(
        DeleteDataBlockPlanViewModel deletePlan
    )
    {
        return deletePlan
    ...
    private Task<Either<ActionResult, Unit>> InvalidateCachedDataBlock(
        Guid releaseVersionId,
        Guid dataBlockId
    )
    {
        return _cacheKeyService
            .CreateCacheKeyForDataBlock(
                releaseVersionId: releaseVersionId,
    ...
    public async Task<Either<ActionResult, List<DataBlockViewModel>>> GetUnattachedDataBlocks(
        Guid releaseVersionId
    )
    {
        return await _persistenceHelper
    ...
    public async Task<bool> IsUnattachedDataBlock(
        Guid releaseVersionId,
        DataBlockVersion dataBlockVersion
    )
    {
