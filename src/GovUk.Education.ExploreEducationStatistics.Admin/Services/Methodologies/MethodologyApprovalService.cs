...
public class MethodologyApprovalService : IMethodologyApprovalService
{
    ...
    )
    {
        ...
        return await _userService
            ...
            .OnSuccessDo(methodologyVersion =>
                CheckMethodologyCanDependOnRelease(methodologyVersion, request)
            )
            ...
            .OnSuccess(async methodologyVersion =>
            {
                ...
                var isToBePublished = await _methodologyVersionRepository.IsToBePublished(
                    methodologyVersion
                );
    ...
    {
        var owningPublicationId = await _context
            .PublicationMethodologies.Where(pm =>
                pm.MethodologyId == methodologyVersion.MethodologyId && pm.Owner
            )
        ...
        var userPublicationRoles = await _context
            ...
            .Where(upr =>
                owningPublicationId == upr.PublicationId && upr.Role == PublicationRole.Allower
            )
            .ToListAsync();
        ...
        if (notifyHigherReviewers)
        {
            userReleaseRoles
                ...
                .ForEach(email =>
                {
                    _emailTemplateService.SendMethodologyHigherReviewEmail(
                        email,
    ...
    )
    {
        ...
        return await _persistenceHelper
            ...
            .OnSuccess<ActionResult, ReleaseVersion, Unit>(async release =>
            {
                ...
                await _context
                    .Entry(methodologyVersion.Methodology)
    ...
    private async Task<Either<ActionResult, Unit>> RemoveUnusedImages(
        MethodologyVersion methodologyVersion
    )
    {
        return await _methodologyContentService
            .GetContentBlocks<HtmlBlock>(methodologyVersion.Id)
            .OnSuccess(async contentBlocks =>
            {
                var contentImageIds = contentBlocks
                    .SelectMany(contentBlock =>
                        HtmlImageUtil.GetMethodologyImages(contentBlock.Body)
                    )
                    .Distinct();

                var imageFiles = await _methodologyFileRepository.GetByFileType(
                    methodologyVersion.Id,
                    Image
                );
