...
public class MethodologyService : IMethodologyService
{
    ...
    public async Task<Either<ActionResult, Unit>> AdoptMethodology(
        Guid publicationId,
        Guid methodologyId
    )
    {
        return await _persistenceHelper
            ...
            .OnSuccessCombineWith(_ =>
                _persistenceHelper.CheckEntityExists<Methodology>(methodologyId)
            )
    ...
    public Task<Either<ActionResult, MethodologyVersionViewModel>> CreateMethodology(
        Guid publicationId
    )
    {
        return _persistenceHelper
            ...
            .OnSuccess(_ =>
                _methodologyVersionRepository.CreateMethodologyForPublication(
                    publicationId,
    ...
    public async Task<Either<ActionResult, Unit>> DropMethodology(
        Guid publicationId,
        Guid methodologyId
    )
    {
        return await _persistenceHelper
            .CheckEntityExists<PublicationMethodology>(q =>
                q.Where(link =>
                    link.PublicationId == publicationId && link.MethodologyId == methodologyId
                )
            )
    ...
    > GetAdoptableMethodologies(Guid publicationId)
    {
        return await _persistenceHelper
            .CheckEntityExists<Publication>(publicationId)
            .OnSuccess(_userService.CheckCanAdoptMethodologyForPublication)
            .OnSuccess(async publication =>
            {
                var publishedMethodologies =
                    await _methodologyRepository.GetPublishedMethodologiesUnrelatedToPublication(
                        publication.Id
                    );
                var latestPublishedVersions = publishedMethodologies
                    .ToAsyncEnumerable()
                    .SelectAwait(async methodology =>
                        await _methodologyVersionRepository.GetLatestPublishedVersion(
                            methodology.Id
                        )
                    )
                    .WhereNotNull();
            ...
            .OnSuccess(async publication =>
            {
                return await publication
                    ...
                    .SelectAwait(async publicationMethodology =>
                    {
                        ...
                        if (
                            isPrerelease
                            && methodologyVersion.Status != MethodologyApprovalStatus.Approved
                        )
                        {
    ...
    public async Task<
        Either<ActionResult, List<IdTitleViewModel>>
    > GetUnpublishedReleasesUsingMethodology(Guid methodologyVersionId)
    {
        return await _persistenceHelper
            .CheckEntityExists<MethodologyVersion>(
                methodologyVersionId,
                queryable =>
                    queryable.Include(m => m.Methodology).ThenInclude(mp => mp.Publications)
            )
            .OnSuccess(methodologyVersion =>
                _userService
                    .CheckCanApproveMethodologyVersion(methodologyVersion)
                    .OrElse(() =>
                        _userService.CheckCanMarkMethodologyVersionAsDraft(methodologyVersion)
                    )
            )
            .OnSuccess(async methodologyVersion =>
            {
                // Get all Publications using the Methodology including adopting Publications
                var publicationIds = methodologyVersion.Methodology.Publications.Select(pm =>
                    pm.PublicationId
                );
                ...
                return releaseVersions
                    ...
                    .Select(rv => new IdTitleViewModel(
                        rv.Id,
                        $"{rv.Release.Publication.Title} - {rv.Release.Title}"
                    ))
                    .ToList();
            });
    }
    ...
    )
    {
        ...
        viewModel.InternalReleaseNote = await GetLatestInternalReleaseNote(
            loadedMethodologyVersion.Id
        );
    ...
    )
    {
        ...
        {
            throw new ArgumentException(
                "Should not be updating details of an approved methodology"
            );
        }
        ...
        return _persistenceHelper
            ...
            .OnSuccess(async methodologyVersion =>
            {
                var methodologyVersionIds = await _context
                    .MethodologyVersions.Where(mv =>
                        mv.MethodologyId == methodologyVersion.MethodologyId
                    )
                    .Select(mv => mv.Id)
                    .ToListAsync();
    ...
    > ListUsersMethodologyVersionsForApproval()
    {
        ...
        return (
            await methodologiesToApprove.SelectAsync(BuildMethodologyVersionViewModel)
        ).ToList();
    }
    ...
    )
    {
        ...
        if (slugChanged)
        {
            ...
            if (latestPublishedVersion is { AlternativeSlug: null })
            {
                ...
                if (
                    latestVersion.Id != latestPublishedVersion.Id
                ...
                )
                {
                    ...
                    if (unpublishedAmendmentRedirectsToRemove.Count > 0)
                    {
                        _context.MethodologyRedirects.RemoveRange(
                            unpublishedAmendmentRedirectsToRemove
                        );
                    }
    ...
    private static PublicationSummaryViewModel BuildPublicationViewModel(
    ...
    )
    {
        ...
        var methodologyVersion =
            await _methodologyVersionRepository.GetLatestPublishedVersionBySlug(newSlug);
