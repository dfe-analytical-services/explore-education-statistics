...
public static class FileNameValidators
{
    ...
    public static bool ContainsSpaces(string fileName) => fileName.Contains(' ', StringComparison.Ordinal);

    public static bool ContainsSpecialChars(string fileName) =>
        fileName.Contains('&', StringComparison.Ordinal) || fileName.IndexOfAny(Path.GetInvalidFileNameChars()) != -1;
}
