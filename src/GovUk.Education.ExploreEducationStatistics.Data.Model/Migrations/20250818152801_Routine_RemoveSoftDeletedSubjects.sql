...
BEGIN
    ...
    IF EXISTS(SELECT 1 from #SoftDeletedSubjects)
        BEGIN
            ...
            SET @batchRowsDeleted = 1;
            SET @totalRowsDeleted = 0;
            WHILE @batchRowsDeleted <> 0
                BEGIN
                    BEGIN TRANSACTION
                    ...
                    SET @totalRowsDeleted += @batchRowsDeleted;
                    SET @currentTime = FORMAT(GETUTCDATE(),'o');
                    RAISERROR('%s - %d ObservationFilterItem rows deleted so far', 0, 1, @currentTime, @totalRowsDeleted) WITH NOWAIT;
                END;
            ...
            WHILE @batchRowsDeleted <> 0
                BEGIN
                    ...
                    SET @totalRowsDeleted += @batchRowsDeleted;
                    SET @currentTime = FORMAT(GETUTCDATE(),'o');
                    RAISERROR('%s - %d Observation rows deleted so far', 0, 1, @currentTime, @totalRowsDeleted) WITH NOWAIT;
                END;
