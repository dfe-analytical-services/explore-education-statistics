    ...
    IF (@observationalUnitExists = 1)
        BEGIN
            ...
            IF (EXISTS(SELECT TOP 1 1 FROM @providersList))
                SET @sqlString = @sqlString + N'(l.Provider_Code IN (SELECT id from @providersList) AND (@geographicLevel IS NOT NULL OR o.GeographicLevel = ''PRO'')) OR '
            ...
            IF (EXISTS(SELECT TOP 1 1 FROM @schoolsList))
                SET @sqlString = @sqlString + N'(l.School_Code IN (SELECT id from @schoolsList) AND (@geographicLevel IS NOT NULL OR o.GeographicLevel = ''SCH'')) OR '
