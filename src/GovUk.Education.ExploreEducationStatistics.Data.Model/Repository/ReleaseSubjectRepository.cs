...
public class ReleaseSubjectRepository : IReleaseSubjectRepository
{
    ...
    {
        var query = _statisticsDbContext.ReleaseSubject.Where(rs =>
            rs.ReleaseVersionId == releaseVersionId
        );
    ...
    {
        await DeleteReleaseSubjectIfExists(
            releaseVersionId: releaseVersionId,
            subjectId: subjectId
        );
        await _footnoteRepository.DeleteFootnotesBySubject(
            releaseVersionId: releaseVersionId,
            subjectId: subjectId
        );
    ...
    private async Task DeleteSubjectIfOrphaned(Guid subjectId, bool softDelete)
    {
        ...
        if (softDelete)
        {
            var subject = await _statisticsDbContext.Subject.FirstOrDefaultAsync(s =>
                s.Id == subjectId
            );
            if (subject != null)
            {
