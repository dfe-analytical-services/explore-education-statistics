...
public class FootnoteRepository : IFootnoteRepository
{
    ...
    public async Task<Footnote> CreateFootnote(
    ...
    )
    {
        ...
        {
            Releases = new List<ReleaseFootnote> { new() { ReleaseVersionId = releaseVersionId } },
            Content = content,
            Order = order,
            Filters = filterIds.Select(filterId => new FilterFootnote { FilterId = filterId }).ToList(),
            FilterGroups = filterGroupIds
                .Select(filterGroupId => new FilterGroupFootnote { FilterGroupId = filterGroupId })
                .ToList(),
            FilterItems = filterItemIds
                .Select(filterItemId => new FilterItemFootnote { FilterItemId = filterItemId })
                .ToList(),
            Indicators = indicatorIds
                .Select(indicatorId => new IndicatorFootnote { IndicatorId = indicatorId })
                .ToList(),
            Subjects = subjectIds.Select(subjectId => new SubjectFootnote { SubjectId = subjectId }).ToList(),
        };
    ...
    )
    {
        ...
        var filterItemIdsResult = await _context
            .FilterItem.Where(filterItem =>
                filterItemIds.Contains(filterItem.Id) && filterItem.FilterGroup.Filter.SubjectId == subjectId
            )
        ...
        var filterItemIdsSanitised = filterItemIdsResult.Select(filter => filter.FilterItemId).ToList();

        var filterGroupsIdsSanitised = filterItemIdsResult.Select(filter => filter.FilterGroupId).Distinct().ToList();

        var filterIdsSanitised = filterItemIdsResult.Select(filter => filter.FilterId).Distinct().ToList();

        // Sanitise the indicator id's to make sure they belong to the subject
        var indicatorIdsSanitised = await _context
            .Indicator.Where(indicator =>
                indicatorIds.Contains(indicator.Id) && indicator.IndicatorGroup.SubjectId == subjectId
            )
            .Select(indicator => indicator.Id)
            .ToListAsync();

        // A footnote must be linked to the release version id
        var footnotesQueryable = _context.Footnote.Where(footnote =>
            footnote.Releases.Any(releaseFootnote => releaseFootnote.ReleaseVersionId == releaseVersionId)
        );
        ...
        footnotesQueryable = footnotesQueryable.Where(footnote =>
            ...
            (
                ...
                (
                    ...
                    || footnote.Filters.Any(filterFootnote => filterFootnote.Filter.SubjectId == subjectId)
                ...
                (
                    ...
                    footnote.Filters.Any(filterFootnote => filterIdsSanitised.Contains(filterFootnote.FilterId))
                )
                &&
    ...
    public async Task DeleteFootnotesBySubject(Guid releaseVersionId, Guid subjectId)
    {
        var footnotes = await GetFootnotes(releaseVersionId, subjectId);
        ...
        await footnotes
            ...
            .ForEachAwaitAsync(async footnote =>
            {
                if (await IsFootnoteLinkedToAnotherSubject(footnote.Id, subjectId))
                {
                    ...
                    var filterItemFootnoteLinks = _context.FilterItemFootnote.Where(filterItemFootnote =>
                        filterItemFootnote.FootnoteId == footnote.Id
                        && filterItemFootnote.FilterItem.FilterGroup.Filter.SubjectId == subjectId
                    );
                    _context.FilterItemFootnote.RemoveRange(filterItemFootnoteLinks);

                    var filterGroupFootnoteLinks = _context.FilterGroupFootnote.Where(filterGroupFootnote =>
                        filterGroupFootnote.FootnoteId == footnote.Id
                        && filterGroupFootnote.FilterGroup.Filter.SubjectId == subjectId
                    );
                    _context.FilterGroupFootnote.RemoveRange(filterGroupFootnoteLinks);

                    var filterFootnoteLinks = _context.FilterFootnote.Where(filterFootnote =>
                        filterFootnote.FootnoteId == footnote.Id && filterFootnote.Filter.SubjectId == subjectId
                    );
                    _context.FilterFootnote.RemoveRange(filterFootnoteLinks);

                    var indicatorFootnoteLinks = _context.IndicatorFootnote.Where(indicatorFootnote =>
                        indicatorFootnote.FootnoteId == footnote.Id
                        && indicatorFootnote.Indicator.IndicatorGroup.SubjectId == subjectId
                    );
                    _context.IndicatorFootnote.RemoveRange(indicatorFootnoteLinks);

                    var subjectFootnoteLinks = _context.SubjectFootnote.Where(subjectFootnote =>
                        subjectFootnote.FootnoteId == footnote.Id && subjectFootnote.SubjectId == subjectId
                    );
                    _context.SubjectFootnote.RemoveRange(subjectFootnoteLinks);

                    if (
                        !await IsFootnoteLinkedToAnotherSubjectForThisRelease(
                    ...
                    )
                    {
                        ...
                        await DeleteReleaseFootnoteLink(releaseVersionId: releaseVersionId, footnoteId: footnote.Id);
                    }
                ...
                else
                {
                    ...
                    await DeleteFootnote(releaseVersionId: releaseVersionId, footnoteId: footnote.Id);
                }
            });
    }
    ...
    public async Task DeleteFootnote(Guid releaseVersionId, Guid footnoteId)
    {
        ...
        {
            var footnoteToRemove = await _context
                .Footnote
            ...
            var footnotesToReorder = await _context
                .ReleaseFootnote.Include(rf => rf.Footnote)
                .Where(rf => rf.ReleaseVersionId == releaseVersionId && rf.Footnote.Order > footnoteToRemove.Order)
        ...
        else
        {
            ...
            await DeleteReleaseFootnoteLink(releaseVersionId: releaseVersionId, footnoteId: footnoteId);
            await _context.SaveChangesAsync();
    ...
    public async Task<bool> IsFootnoteExclusiveToRelease(Guid releaseVersionId, Guid footnoteId)
    {
        // TODO EES-2979 Remove this method
        var otherFootnoteReferences = await _context.ReleaseFootnote.CountAsync(rf =>
            rf.FootnoteId == footnoteId && rf.ReleaseVersionId != releaseVersionId
        );
    ...
    public async Task<List<Footnote>> GetFootnotes(Guid releaseVersionId, Guid? subjectId = null)
    {
        if (subjectId.HasValue)
        {
            return await GetFootnotes(releaseVersionId, new List<Guid> { subjectId.Value });
        }

        var subjectsIdsForReleaseVersion = await _context
            .ReleaseSubject.Where(rs => rs.ReleaseVersionId == releaseVersionId)
            .Select(rs => rs.SubjectId)
            .ToListAsync();

        var footnotes = await GetFootnotes(releaseVersionId, subjectsIdsForReleaseVersion);

        // Remove the below check as part of EES-2971
        var releaseFootnotes = _context.ReleaseFootnote.Where(rf => rf.ReleaseVersionId == releaseVersionId).ToList();
        if (footnotes.Count != releaseFootnotes.Count)
        {
            throw new Exception(
                $"Number of footnotes different from number of ReleaseFootnotes for ReleaseVersion {releaseVersionId}"
            );
        }
    ...
    private async Task<List<Footnote>> GetFootnotes(Guid releaseVersionId, IEnumerable<Guid> subjectIds)
    {
        return (
            await GetBaseFootnoteQuery()
                .Where(footnote =>
                    footnote.Releases.Any(releaseFootnote => releaseFootnote.ReleaseVersionId == releaseVersionId)
                )
                .Select(f => new Footnote
                {
                    ...
                    Order = f.Order,
                    Filters = f
                        .Filters.Where(filterFootnote => subjectIds.Contains(filterFootnote.Filter.SubjectId))
                        .ToList(),
                    Indicators = f
                        .Indicators.Where(indicatorFootnote =>
                            subjectIds.Contains(indicatorFootnote.Indicator.IndicatorGroup.SubjectId)
                        )
                        .ToList(),
                    Releases = f
                        .Releases.Where(releaseFootnote => releaseFootnote.ReleaseVersionId == releaseVersionId)
                        .ToList(),
                    FilterGroups = f
                        .FilterGroups.Where(filterGroupFootnote =>
                            subjectIds.Contains(filterGroupFootnote.FilterGroup.Filter.SubjectId)
                        )
                        .ToList(),
                    FilterItems = f
                        .FilterItems.Where(filterItemFootnote =>
                            subjectIds.Contains(filterItemFootnote.FilterItem.FilterGroup.Filter.SubjectId)
                        )
                        .ToList(),
                    Subjects = f
                        .Subjects.Where(subjectFootnote => subjectIds.Contains(subjectFootnote.SubjectId))
                        .ToList(),
                })
                .ToListAsync()
        ).Where(f => f.Filters.Any() || f.Indicators.Any() || f.FilterGroups.Any() || f.FilterItems.Any() || f.Subjects.Any()).OrderBy(f => f.Order).ThenBy(f => f.Id).ToList();
    }
    ...
    public async Task<IList<Subject>> GetSubjectsWithNoFootnotes(Guid releaseVersionId)
    {
        return await _context
            .ReleaseSubject.Where(releaseSubject => releaseSubject.ReleaseVersionId == releaseVersionId)
            .Where(releaseSubject =>
                ...
                && !releaseSubject
                    .Subject.Filters.SelectMany(filter => filter.FilterGroups)
                ...
                && !releaseSubject
                    ...
                    .Any()
                && !releaseSubject
                    .Subject.IndicatorGroups.SelectMany(indicatorGroup => indicatorGroup.Indicators)
    ...
    public async Task DeleteReleaseFootnoteLink(Guid releaseVersionId, Guid footnoteId)
    {
        // TODO EES-2979 Remove this method
        var releaseFootnote = await _context.ReleaseFootnote.FirstAsync(rf =>
            rf.ReleaseVersionId == releaseVersionId && rf.FootnoteId == footnoteId
        );
    ...
    private async Task<bool> IsFootnoteLinkedToAnotherSubjectForThisRelease(
        Guid footnoteId,
        Guid releaseVersionId,
        Guid subjectId
    )
    {
        var otherSubjectIds = await _context
            .ReleaseSubject.Where(rs => rs.ReleaseVersionId == releaseVersionId && rs.SubjectId != subjectId)
            .Select(rs => rs.SubjectId)
            .ToListAsync();
        ...
        return footnote.Subjects.Any(subject => otherSubjectIds.Contains(subject.SubjectId))
            ...
            || footnote.Indicators.Any(indicator =>
                otherSubjectIds.Contains(indicator.Indicator.IndicatorGroup.SubjectId)
            )
            || footnote.FilterGroups.Any(filterGroup =>
                otherSubjectIds.Contains(filterGroup.FilterGroup.Filter.SubjectId)
            )
            || footnote.FilterItems.Any(filterItem =>
                otherSubjectIds.Contains(filterItem.FilterItem.FilterGroup.Filter.SubjectId)
            );
    }
}
