...
public class FootnoteRepository : IFootnoteRepository
{
    ...
    )
    {
        var footnote = new Footnote
        {
            ...
            Order = order,
            Filters = filterIds
                .Select(filterId => new FilterFootnote { FilterId = filterId })
                .ToList(),
            ...
            Indicators = indicatorIds
                ...
                .ToList(),
            Subjects = subjectIds
                .Select(subjectId => new SubjectFootnote { SubjectId = subjectId })
                .ToList(),
        };
    ...
    )
    {
        ...
        var indicatorIdsSanitised = await _context
            .Indicator.Where(indicator =>
                indicatorIds.Contains(indicator.Id)
        ...
        var footnotesQueryable = _context.Footnote.Where(footnote =>
            footnote.Releases.Any(releaseFootnote =>
                releaseFootnote.ReleaseVersionId == releaseVersionId
            )
        );
        ...
        await footnotes
            ...
            .ForEachAwaitAsync(async footnote =>
            {
                if (await IsFootnoteLinkedToAnotherSubject(footnote.Id, subjectId))
                {
                    ...
                    var filterItemFootnoteLinks = _context.FilterItemFootnote.Where(
                        filterItemFootnote =>
                            filterItemFootnote.FootnoteId == footnote.Id
                            && filterItemFootnote.FilterItem.FilterGroup.Filter.SubjectId
                                == subjectId
                    );
                    _context.FilterItemFootnote.RemoveRange(filterItemFootnoteLinks);

                    var filterGroupFootnoteLinks = _context.FilterGroupFootnote.Where(
                        filterGroupFootnote =>
                    ...
                    var indicatorFootnoteLinks = _context.IndicatorFootnote.Where(
                        indicatorFootnote =>
                    ...
                    )
                    {
                        ...
                        await DeleteReleaseFootnoteLink(
                            releaseVersionId: releaseVersionId,
                            footnoteId: footnote.Id
                        );
                    }
    ...
    {
        if (
            await IsFootnoteExclusiveToRelease(
                releaseVersionId: releaseVersionId,
    ...
    private async Task<List<Footnote>> GetFootnotes(
        Guid releaseVersionId,
                ...
                .Select(f => new Footnote
                {
                    ...
                    Indicators = f
                        .Indicators.Where(indicatorFootnote =>
                            subjectIds.Contains(
                                indicatorFootnote.Indicator.IndicatorGroup.SubjectId
                            )
                        )
                        .ToList(),
                    Releases = f
                        .Releases.Where(releaseFootnote =>
                            releaseFootnote.ReleaseVersionId == releaseVersionId
                        )
                        .ToList(),
                    FilterGroups = f
                    ...
                    FilterItems = f
                        ...
                        .ToList(),
                    Subjects = f
                        .Subjects.Where(subjectFootnote =>
                            subjectIds.Contains(subjectFootnote.SubjectId)
                        )
                        .ToList(),
                })
                .ToListAsync()
    ...
    public async Task<IList<Subject>> GetSubjectsWithNoFootnotes(Guid releaseVersionId)
    {
        return await _context
            .ReleaseSubject.Where(releaseSubject =>
                releaseSubject.ReleaseVersionId == releaseVersionId
            )
    ...
    private async Task<bool> IsFootnoteLinkedToAnotherSubject(Guid footnoteId, Guid subjectId)
    {
        ...
        return footnote.Subjects.Any(subject => subject.SubjectId != subjectId)
            ...
            || footnote.Indicators.Any(indicator =>
                indicator.Indicator.IndicatorGroup.SubjectId != subjectId
            )
            || footnote.FilterGroups.Any(filterGroup =>
                filterGroup.FilterGroup.Filter.SubjectId != subjectId
            )
            || footnote.FilterItems.Any(filterItem =>
                filterItem.FilterItem.FilterGroup.Filter.SubjectId != subjectId
            );
    }
