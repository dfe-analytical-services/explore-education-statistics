...
public class DenseObservationsMatchedFilterItemsStrategy(
    ...
    ILogger<DenseObservationsMatchedFilterItemsStrategy> logger) : IDenseObservationsMatchedFilterItemsStrategy
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var candidateFilterItemIdTable = await temporaryTableCreator
            .CreateAnonymousTemporaryTableAndPopulate(
                context,
                allFilterItemIds.Select(fi => new IdTempTable(fi)),
                cancellationToken);

        var matchedFilterItemIdTable = await temporaryTableCreator
