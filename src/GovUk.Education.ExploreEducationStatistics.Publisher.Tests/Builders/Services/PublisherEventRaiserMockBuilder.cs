...
public class PublisherEventRaiserMockBuilder
{
    ...
    {
        _mock
            .Setup(m => m.RaiseReleaseVersionPublishedEvents(It.IsAny<IList<PublishedReleaseVersionInfo>>()))
            .Returns(Task.CompletedTask);
    }

    public class Asserter(Mock<IPublisherEventRaiser> mock)
    {
        public void EventWasRaised(Func<PublishedReleaseVersionInfo, bool> expected)
        {
            mock.Verify(m => m.RaiseReleaseVersionPublishedEvents(
                It.Is<IList<PublishedReleaseVersionInfo>>(
