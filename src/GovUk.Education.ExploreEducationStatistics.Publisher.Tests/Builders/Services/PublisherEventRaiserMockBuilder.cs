...
public class PublisherEventRaiserMockBuilder
{
    ...
    {
        _mock
            .Setup(m => m.OnPublicationArchived(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<Guid>()))
            .Returns(Task.CompletedTask);

        _mock
            .Setup(m => m.OnReleaseVersionsPublished(It.IsAny<IReadOnlyList<PublishedPublicationInfo>>()))
            .Returns(Task.CompletedTask);
    }
    ...
    public class Asserter(Mock<IPublisherEventRaiser> mock)
    {
        ...
        {
            mock.Verify(
                m => m.OnPublicationArchived(publication.Id, publication.Slug, publication.SupersededById!.Value),
                Times.Once()
            );
        }
        ...
        {
            mock.Verify(
                m => m.OnPublicationArchived(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<Guid>()),
                Times.Never()
            );
        }
        ...
        {
            mock.Verify(m =>
                m.OnReleaseVersionsPublished(
                    It.Is<IReadOnlyList<PublishedPublicationInfo>>(actual => actual.Any(expected))
                )
            );
        }
    }
