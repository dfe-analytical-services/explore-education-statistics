...
public class PublisherEventRaiserMockBuilder
{
    ...
    public PublisherEventRaiserMockBuilder()
    {
        ...
        _mock
            .Setup(m =>
                m.OnReleaseVersionsPublished(It.IsAny<IReadOnlyList<PublishedPublicationInfo>>())
            )
            .Returns(Task.CompletedTask);
    }
    ...
    public class Asserter(Mock<IPublisherEventRaiser> mock)
    {
        ...
        public void ReleaseVersionPublishedEventWasRaised(
            Func<PublishedPublicationInfo, bool> expected
        )
        {
            mock.Verify(m =>
