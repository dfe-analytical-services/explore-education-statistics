...
public class ContentDbContextMockBuilder
{
    ...
    public ContentDbContextMockBuilder WherePublication(
        Guid publicationId,
        Action<Publication> modifier
    )
    {
        var publication = _inMemoryContentDbContext.Publications.SingleOrDefault(p =>
            p.Id == publicationId
        );
        if (publication == null)
    ...
    public class Asserter(ContentDbContext contentDbContext)
    {
        ...
        {
            var publication = contentDbContext.Publications.SingleOrDefault(p =>
                p.Id == publicationId
            );
            if (publication == null)
                Xunit.Assert.Fail($"No publication was found with id {publicationId}");
