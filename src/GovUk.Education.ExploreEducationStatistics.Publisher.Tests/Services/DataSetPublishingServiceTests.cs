        ...
        : DataSetPublishingServiceTests(fixture)
    {
        ...
        public async Task FirstDataSetVersionPublished()
        {
            ...
            DataSetVersion dataSetVersion = DataFixture
                ...
                .WithRelease(
                    DataFixture.DefaultDataSetVersionRelease().WithReleaseFileId(releaseDataFile.Id)
                );

            await AddTestData<ContentDbContext>(context =>
                context.ReleaseFiles.Add(releaseDataFile)
            );
            ...
            var versionedFolderPath = dataSetVersionPathResolver.DirectoryPath(
                publishedDataSet.LatestLiveVersion
            );
            Assert.False(Directory.Exists(draftFolderPath));
            Assert.True(Directory.Exists(versionedFolderPath));
            Assert.EndsWith(
                $"v{publishedDataSet.LatestLiveVersion.SemVersion()}",
                versionedFolderPath
            );
        }
        ...
        public async Task SecondDataSetVersionPublished()
        {
            ...
            var publishedDataSet = await publicDataDbContext
                .DataSets.Include(ds => ds.LatestDraftVersion)
        ...
        public async Task NoDraftDataSetVersionsToPublish()
        {
            ...
            Assert.Equal(
                dataSet.LatestLiveVersion!.Status,
                retrievedDataSet.LatestLiveVersion!.Status
            );
            Assert.Equal(
        ...
        {
            ReleaseVersion originalReleaseVersion = DataFixture.DefaultReleaseVersion();

            ReleaseVersion amendmentReleaseVersion = DataFixture
                .DefaultReleaseVersion()
                .WithPreviousVersionId(originalReleaseVersion.Id);

            File dataFile = DataFixture.DefaultFile(FileType.Data);
            ...
            Assert.Equal(
                amendmentReleaseDataFile.Id,
                publishedDataSet.LatestLiveVersion.Release.ReleaseFileId
            );
        }
        ...
        public async Task ReleaseAmendmentPublishedWithNewDataSetVersion_CorrectReleaseFileIds(
            bool amendmentResultedInPatchReplacement
        )
        {
            ReleaseVersion originalReleaseVersion = DataFixture.DefaultReleaseVersion();
            ...
            var (dataFile, newDataFile) = DataFixture.DefaultFile(FileType.Data).GenerateTuple2();
            ...
            Assert.Equal(
                dataSetVersion1.Published.TruncateNanoseconds(),
                savedDataSetVersion1.Published
            );
            ...
            if (amendmentResultedInPatchReplacement)
            {
                ...
                Assert.Equal(
                    originalReleaseDataFile.Id,
                    savedDataSetVersion1.Release.ReleaseFileId
                );
            }
            else
            {
