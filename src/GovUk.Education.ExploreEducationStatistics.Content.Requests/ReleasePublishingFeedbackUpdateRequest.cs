...
public record ReleasePublishingFeedbackUpdateRequest(
    [JsonConverter(typeof(StringEnumConverter))] ReleasePublishingFeedbackResponse Response,
    string EmailToken,
...
)
{
    public class Validator : AbstractValidator<ReleasePublishingFeedbackUpdateRequest>
    {
        public Validator()
        {
            RuleFor(request => request.EmailToken).NotEmpty().MaximumLength(55);

            RuleFor(request => request.Response).IsInEnum();

            RuleFor(request => request.AdditionalFeedback).MaximumLength(2000);
        }
    }
}
