...
) : IPublishingCompletionService
{
    ...
    )
    {
        ...
        await releaseVersionIdsToUpdate
            .ToAsyncEnumerable()
            .ForEachAwaitAsync(releaseId =>
                releaseService.CompletePublishing(releaseId, DateTime.UtcNow)
            );
        ...
        var publishedReleaseVersionInfos = await BuildPublishedReleaseVersionInfos(
            releaseVersionIdsToUpdate
        );
    ...
    )
    {
        ...
        var latestPublishedReleaseVersion = await releaseService.GetLatestPublishedReleaseVersion(
            publicationId
        );
    ...
    )
    {
        ...
        var archivedPublications = await contentDbContext
            .Publications.Where(p =>
                ...
                && newlyPublishedPublicationIds.Contains(p.SupersededById.Value)
            )
            .ToListAsync();
