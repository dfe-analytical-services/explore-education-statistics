...
public class ProcessorStage3Tests
{
    ...
    public ProcessorStage3Tests()
    {
        ...
        _locations = _fixture
            ...
            .ForIndex(
                0,
                s => s.SetLocalAuthority(new LocalAuthority("E08000025", "330", "Birmingham"))
            )
            .ForIndex(
                1,
                s => s.SetLocalAuthority(new LocalAuthority("E09000011", "203", "Greenwich"))
            )
            .ForIndex(
                2,
                s => s.SetLocalAuthority(new LocalAuthority("E08000016", "370", "Barnsley"))
            )
    ...
    public async Task ProcessStage3()
    {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            ...
            var observations = await statisticsDbContext
                .Observation.Include(o => o.FilterItems)
                .ToListAsync();

            Assert.Equal(16, observations.Count);

            var expectedIndicatorIds = _subject
                .IndicatorGroups.SelectMany(ig => ig.Indicators)
                .Select(i => i.Id);
            ...
            Assert.Equal(
                "1",
                firstObservation.Measures[_subject.IndicatorGroups[0].Indicators[0].Id]
            );
            Assert.Equal(
                "2",
                firstObservation.Measures[_subject.IndicatorGroups[0].Indicators[1].Id]
            );
            ...
            Assert.Equal(
                "16",
                lastObservation.Measures[_subject.IndicatorGroups[0].Indicators[0].Id]
            );
            Assert.Equal(
                "32",
                lastObservation.Measures[_subject.IndicatorGroups[0].Indicators[1].Id]
            );
            ...
            file.DataSetFileMeta.Filters.Select(f => (f.Label, f.Hint, f.ColumnName))
                .ToList()
                .AssertDeepEqualTo(
                    [("Filter one", "Hint 1", "filter_one"), ("Filter two", null, "filter_two")]
                );

            file.DataSetFileMeta.Indicators.Select(i => (i.Label, i.ColumnName))
                .ToList()
                .AssertDeepEqualTo(
                    [("Indicator one", "indicator_one"), ("Indicator two", "indicator_two")]
                );
        }
    }
    ...
    public async Task ProcessStage3_IgnoredRows()
    {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            ...
            Enumerable
                .Range(0, 8)
                .ForEach(i => Assert.Equal((i * 2) + 2, observations[i].CsvRow));
        }
    }
        ...
        var subject = _fixture
            ...
            .WithIndicatorGroups(
                _fixture
                    .DefaultIndicatorGroup()
                    .WithIndicators(
                        _fixture.DefaultIndicator().WithLabel("Indicator one").GenerateList(1)
                    )
                    .Generate(1)
            )
            .Generate();
