...
public class ProcessorStage3Tests
{
            ...
            .WithFilters(
                _fixture
                    ...
                    .ForIndex(
                        0,
                        s =>
                            s.SetLabel("Filter one")
                                .SetHint("Hint 1")
                                .SetFilterGroups(
                                    _fixture
                                        .DefaultFilterGroup()
                                        .WithFilterItems(_fixture.DefaultFilterItem().WithLabel("Total").Generate(1))
                                        .Generate(1)
                                )
                    )
                    .ForIndex(
    ...
    public async Task ProcessStage3()
    {
        ...
        var importerMetaService = new ImporterMetaService(guidGenerator, databaseHelper);

        var observationBatchImporter = new TestObservationBatchImporter();

        var importerService = new ImporterService(
            new AppOptions { RowsPerBatch = 5000 }.ToOptionsWrapper(),
            guidGenerator,
        ...
        var function = BuildFunction(processorService: processorService, dataImportService: dataImportService);
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            var dataImport = await contentDbContext.DataImports.SingleAsync();
            ...
            var observations = await statisticsDbContext.Observation.Include(o => o.FilterItems).ToListAsync();

            Assert.Equal(16, observations.Count);

            var expectedIndicatorIds = _subject.IndicatorGroups.SelectMany(ig => ig.Indicators).Select(i => i.Id);

            var expectedFilterItemIds = _subject.Filters.Select(f => f.Id);
            ...
            observations.ForEach(observation =>
            {
                ...
                var filterItemIds = observation.FilterItems.Select(ofi => ofi.FilterId!.Value);
            ...
            // of 2.
            Enumerable.Range(0, 16).ForEach(i => Assert.Equal(i + 2, observations[i].CsvRow));
            ...
            Assert.Equal(TimeIdentifier.CalendarYear, file.DataSetFileMeta.TimePeriodRange.Start.TimeIdentifier);
            Assert.Equal(TimeIdentifier.CalendarYear, file.DataSetFileMeta.TimePeriodRange.End.TimeIdentifier);
            ...
            file.DataSetFileMeta.Filters.Select(f => (f.Label, f.Hint, f.ColumnName))
                .ToList()
                .AssertDeepEqualTo([("Filter one", "Hint 1", "filter_one"), ("Filter two", null, "filter_two")]);

            file.DataSetFileMeta.Indicators.Select(i => (i.Label, i.ColumnName))
                .ToList()
                .AssertDeepEqualTo([("Indicator one", "indicator_one"), ("Indicator two", "indicator_two")]);
        }
    }
    ...
    public async Task ProcessStage3_PartiallyImportedAlready()
    {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            ...
            var observations = await statisticsDbContext.Observation.ToListAsync();
    ...
    public async Task ProcessStage3_PartiallyImportedAlready_BatchSizeChanged()
    {
        ...
        var importerService = new ImporterService(
            new AppOptions { RowsPerBatch = 3 }.ToOptionsWrapper(),
            guidGenerator,
            new ImporterLocationService(
                guidGenerator,
                importerLocationCache,
    ...
    public async Task ProcessStage3_IgnoredRows()
    {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            ...
            statisticsDbContext
                .Observation.Include(o => o.Location)
    ...
    public async Task ProcessStage3_IgnoredRows_PartiallyImported()
    {
        ...
        var alreadyImportedObservations = _fixture
            .DefaultObservation()
            .WithLocation(_fixture.DefaultLocation().WithGeographicLevel(GeographicLevel.LocalAuthority).Generate())
            .WithSubject(_subject)
            .ForInstance(s => s.Set(o => o.CsvRow, (_, _, context) => (context.Index * 2) + 2))
            .Generate(4);
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            ...
            var observations = await statisticsDbContext.Observation.Include(o => o.Location).ToListAsync();

            Assert.Equal(8, observations.Count);

            // Verify that only the LA-level data was imported and the School-level data ignored.
            observations.ForEach(o => Assert.Equal(GeographicLevel.LocalAuthority, o.Location.GeographicLevel));
    ...
    public async Task ProcessStage3_Cancelling()
    {
        ...
        observationBatchImporterMock
            .Setup(s => s.ImportObservationBatch(It.IsAny<StatisticsDbContext>(), It.IsAny<IEnumerable<Observation>>()))
    ...
    public async Task ProcessStage3_AdditionalFiltersAndIndicators()
    {
        var additionalFilter = _fixture
            ...
            .WithFilterGroups(
                _fixture
                    .DefaultFilterGroup()
                    .WithFilterItems(
                        _fixture.DefaultFilterItem().ForIndex(1, s => s.SetLabel("Not specified")).Generate(3)
                    )
                    .Generate(1)
            )
            .Generate();
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(_statisticsDbContextId))
        {
            ...
            observations.ForEach(observation => Assert.Null(observation.Measures[additionalIndicator.Id]));
        }
    }
    ...
    public async Task ProcessStage3_SpecialFilterItemAndIndicatorValues()
    {
        var subject = _fixture
            ...
            .WithIndicatorGroups(
                _fixture
                    .DefaultIndicatorGroup()
                    .WithIndicators(_fixture.DefaultIndicator().WithLabel("Indicator one").GenerateList(1))
                    .Generate(1)
            )
            .Generate();
