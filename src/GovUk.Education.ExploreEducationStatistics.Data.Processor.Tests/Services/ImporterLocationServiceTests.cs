...
public class ImporterLocationServiceTests
{
    ...
    {
        var location = new Location { GeographicLevel = GeographicLevel.Country, Country = _england };
    ...
    {
        var nonMatchingLocation = new Location { GeographicLevel = GeographicLevel.Institution, Country = _england };
    ...
    public async Task CreateAndCache_NewLocation()
    {
        ...
        guidGenerator.Setup(mock => mock.NewGuid()).Returns(locationId);

        var importerLocationCache = new ImporterLocationCache(Mock.Of<ILogger<ImporterLocationCache>>());

        var service = BuildService(guidGenerator.Object, importerLocationCache);
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(statisticsDbContextId))
        {
            var result = await service.CreateIfNotExistsAndCache(
                statisticsDbContext,
                new Location { GeographicLevel = GeographicLevel.Country, Country = _england }
            );
    ...
    public async Task CreateAndCache_UniqueByName()
    {
        ...
        var guidGenerator = new Mock<IGuidGenerator>();
        guidGenerator.SetupSequence(mock => mock.NewGuid()).Returns(result1Id).Returns(result2Id);
