...
public abstract class DeleteDataSetVersionFunctionTests(
...
) : ProcessorFunctionsIntegrationTest(fixture)
{
    public class DeleteDataSetVersionTests : DeleteDataSetVersionFunctionTests
    {
        ...
        public DeleteDataSetVersionTests(ProcessorFunctionsIntegrationTestFixture fixture)
            : base(fixture)
        {
        ...
        public async Task Success_FirstDataSetVersion(DataSetVersionStatus dataSetVersionStatus)
        {
            ...
            await AddTestData<ContentDbContext>(context =>
                context.ReleaseFiles.Update(releaseFile)
            );
            ...
            Assert.False(
                await publicDataDbContext.FilterOptionMetaLinks.AnyAsync(ml =>
                    dataSetVersion.FilterMetas.Contains(ml.Meta)
                )
            );
            Assert.False(
            ...
            Assert.False(
                await publicDataDbContext.LocationOptionMetaLinks.AnyAsync(ml =>
                    dataSetVersion.LocationMetas.Contains(ml.Meta)
                )
            );
            Assert.False(
            ...
            Assert.Null(
                await publicDataDbContext.DataSetVersionImports.SingleOrDefaultAsync(v =>
                    v.Id == dataSetVersion.Imports.Single().Id
                )
            );

            // Assert that the ReleaseFile has been unassociated with the Public API DataSet
            var updatedReleaseFile = await contentDataDbContext.ReleaseFiles.SingleAsync(rf =>
                rf.Id == releaseFile.Id
            );
        ...
        )
        {
            ...
            DataSetVersion liveDataSetVersion = DataFixture
                ...
                .WithDataSet(dataSet)
                .WithRelease(
                    DataFixture.DefaultDataSetVersionRelease().WithReleaseFileId(liveReleaseFile.Id)
                )
                .WithImports(() =>
                    DataFixture
            ...
            var liveDataSetVersionDirectory = _dataSetVersionPathResolver.DirectoryPath(
                liveDataSetVersion
            );
            ...
            var updatedDataSet = Assert.Single(
                await publicDataDbContext.DataSets.Where(ds => ds.Id == dataSet.Id).ToListAsync()
            );
            ...
            Assert.Equal(
                0,
                await publicDataDbContext
                    .FilterOptionMetaLinks.Where(ml =>
                        draftDataSetVersion.FilterMetas.Contains(ml.Meta)
                    )
                    .CountAsync()
            );
            Assert.Equal(
            ...
            Assert.Equal(
                0,
                await publicDataDbContext
                    .LocationOptionMetaLinks.Where(ml =>
                        draftDataSetVersion.LocationMetas.Contains(ml.Meta)
                    )
                    .CountAsync()
            );
            Assert.Equal(
            ...
            Assert.NotEmpty(
                await publicDataDbContext
                    .FilterOptionMetaLinks.Where(ml =>
                        liveDataSetVersion.FilterMetas.Contains(ml.Meta)
                    )
                    .ToListAsync()
            );
            Assert.NotEmpty(
            ...
            Assert.NotEmpty(
                await publicDataDbContext
                    .LocationOptionMetaLinks.Where(ml =>
                        liveDataSetVersion.LocationMetas.Contains(ml.Meta)
                    )
                    .ToListAsync()
            );
            Assert.NotEmpty(
            ...
            Assert.Null(
                await publicDataDbContext.DataSetVersionImports.SingleOrDefaultAsync(i =>
                    i.Id == draftDataSetVersion.Imports.Single().Id
                )
            );
            ...
            Assert.Single(
                await publicDataDbContext
                    .DataSetVersionImports.Where(i =>
                        i.Id == liveDataSetVersion.Imports.Single().Id
                    )
                    .ToListAsync()
            );

            // Assert that the DRAFT ReleaseFile has been unassociated with the DRAFT Public API Data Set Version
            var updatedDraftReleaseFile = await contentDataDbContext.ReleaseFiles.SingleAsync(f =>
                f.Id == draftReleaseFile.Id
            );
            ...
            var updatedLiveReleaseFile = await contentDataDbContext.ReleaseFiles.SingleAsync(f =>
                f.Id == liveReleaseFile.Id
            );

            Assert.Equal(dataSet.Id, updatedLiveReleaseFile.PublicApiDataSetId);
            Assert.Equal(
                liveDataSetVersion.SemVersion(),
                updatedLiveReleaseFile.PublicApiDataSetVersion
            );
        }
        ...
        public async Task VersionCanNotBeDeleted_Returns400(
            DataSetVersionStatus dataSetVersionStatus
        )
        {
