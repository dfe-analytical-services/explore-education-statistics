    ...
    : ProcessorFunctionsIntegrationTest(fixture)
{
    public class DeleteDataSetVersionTests : DeleteDataSetVersionFunctionTests
    {
        ...
        public async Task Success_FirstDataSetVersion(DataSetVersionStatus dataSetVersionStatus)
        {
            ...
            Assert.Null(
                await publicDataDbContext.DataSetVersions.SingleOrDefaultAsync(dsv => dsv.Id == dataSetVersion.Id)
            );

            // Assert that the Data Set Version metadata is deleted
            Assert.False(
                await publicDataDbContext.FilterMetas.AnyAsync(fm => fm.DataSetVersionId == dataSetVersion.Id)
            );
            Assert.False(
            ...
            Assert.False(
                await publicDataDbContext.LocationMetas.AnyAsync(fm => fm.DataSetVersionId == dataSetVersion.Id)
            );
            Assert.False(
            ...
            Assert.False(
                await publicDataDbContext.IndicatorMetas.AnyAsync(fm => fm.DataSetVersionId == dataSetVersion.Id)
            );
            Assert.False(
                await publicDataDbContext.GeographicLevelMetas.AnyAsync(fm => fm.DataSetVersionId == dataSetVersion.Id)
            );
            Assert.False(
                await publicDataDbContext.TimePeriodMetas.AnyAsync(fm => fm.DataSetVersionId == dataSetVersion.Id)
            );
            ...
            var updatedReleaseFile = await contentDataDbContext.ReleaseFiles.SingleAsync(rf => rf.Id == releaseFile.Id);
        ...
        public async Task Success_SubsequentDataSetVersion(DataSetVersionStatus dataSetVersionStatus)
        {
            ...
            Assert.Null(
                await publicDataDbContext.DataSetVersions.SingleOrDefaultAsync(dsv => dsv.Id == draftDataSetVersion.Id)
            );

            // Assert that the LIVE Data Set Version has NOT been deleted
            Assert.Single(
                await publicDataDbContext.DataSetVersions.Where(dsv => dsv.Id == liveDataSetVersion.Id).ToListAsync()
            );
        ...
        private async Task<IActionResult> DeleteDataSetVersion(Guid dataSetVersionId)
        {
            ...
            return await function.DeleteDataSetVersion(null!, dataSetVersionId, CancellationToken.None);
        }
    }
}
