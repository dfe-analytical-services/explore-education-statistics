    ...
    : ProcessorFunctionsIntegrationTest(fixture)
{
    public class BulkDeleteDataSetVersionsTests : BulkDeleteDataSetVersionsFunctionTests
    {
        ...
        public async Task ReleaseVersionIsLinkedToOldFileWithApiDataSet(DataSetVersionStatus dataSetVersionStatus)
        {
            ...
            var targetReleaseFileWithOldFilePostDelete = await contentDataDbContext.ReleaseFiles.SingleAsync(f =>
            ...
            var previousReleaseFilePostDelete = await contentDataDbContext.ReleaseFiles.SingleAsync(f =>
                f.Id == previousReleaseFile.Id
            );
        ...
        public async Task ReleaseVersionIsLinkedToSubsequentDataSetVersion(DataSetVersionStatus dataSetVersionStatus)
        {
            ...
            var release1Version1ReleaseFilePostDelete = await contentDataDbContext.ReleaseFiles.SingleAsync(f =>
                f.Id == release1Version1ReleaseFile.Id
            );
            var release2Version1ReleaseFilePostDelete = await contentDataDbContext.ReleaseFiles.SingleAsync(f =>
        ...
        {
            Assert.True(await publicDataDbContext.FilterMetas.AnyAsync(fm => fm.DataSetVersionId == dataSetVersion.Id));
            Assert.True(
                await publicDataDbContext.FilterOptionMetaLinks.AnyAsync(foml =>
                    dataSetVersion.FilterMetas.Contains(foml.Meta)
                )
