    ...
    : ProcessorFunctionsIntegrationTest(fixture)
{
        ...
        : ImportDataFunctionTests(fixture)
    {
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public async Task DuckDbDataTable_CorrectDistinctGeographicLevels(
        ...
        public async Task DuckDbDataTable_CorrectDistinctLocationOptions(ProcessorTestData testData)
        {
            ...
            await Assert.AllAsync(
                ...
                async expectedLocation =>
                {
                    ...
                    var actualOptions = (
                        await optionsCommand.QueryAsync<ParquetLocationOption>()
                    ).AsList();
            ...
            await Assert.AllAsync(
                ...
                async expectedFilter =>
                {
                    var optionsCommand = duckDbConnection.SqlBuilder(
                        ...
                        ON {DataTable.Ref().Col(
                            expectedFilter.Column
                        ):raw} = {FilterOptionsTable.Ref().Id:raw}
                        ORDER BY {FilterOptionsTable.Ref().Label:raw}
                        """
                    );

                    var actualOptions = (
                        await optionsCommand.QueryAsync<ParquetFilterOption>()
                    ).AsList();
