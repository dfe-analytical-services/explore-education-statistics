...
const PrototypeLocalAuthorityDataTable = () => {
  return (
    <PrototypePage
      breadcrumbs={[
              ...
              <table className="govuk-table">
                ...
                <tbody>
                  <tr>
                    ...
                    {absenceRateData.map(({ Overall }) => (
                      <td>{`${Overall}%`}</td>
                    ))}
                  </tr>
                  <tr>
                    <td scope="row">Authorised absence rate</td>
                    {absenceRateData.map(({ Authorised }) => (
                      <td>{`${Authorised}%`}</td>
                    ))}
                  </tr>
                  <tr>
                    <td scope="row">Unauthorised absence rate</td>
                    {absenceRateData.map(({ Unauthorised }) => (
                      <td>{`${Unauthorised}%`}</td>
                    ))}
                  </tr>
                  <tr>
                    <td scope="row">Primary permanent exclusions rate</td>
                    {exclusionRateData.map(({ Primary }) => (
                      <td>{`${Primary.toFixed(3)}%`}</td>
                    ))}
                  </tr>
                  <tr>
                    <td scope="row">Secondary permanent exclusions rate</td>
                    {exclusionRateData.map(({ Secondary }) => (
                      <td>{`${Secondary.toFixed(3)}%`}</td>
                    ))}
                  </tr>
                  <tr>
                    <td scope="row">Special permanent exclusions rate</td>
                    {exclusionRateData.map(({ Special }) => (
                      <td>{`${Special.toFixed(3)}%`}</td>
                    ))}
                  </tr>
                  <tr>
                    <td scope="row">Primary permanent exclusions rate</td>
                    {exclusionRateData.map(({ Total }) => (
