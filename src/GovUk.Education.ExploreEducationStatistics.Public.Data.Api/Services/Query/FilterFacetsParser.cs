...
internal class FilterFacetsParser : IFacetsParser
{
    ...
    private readonly Dictionary<string, ParquetFilterOption> _filterOptionsByPublicId;

    public FilterFacetsParser(QueryState queryState, IEnumerable<ParquetFilterOption> filterOptions)
    {
        _queryState = queryState;
        _filterOptionsByPublicId = filterOptions.ToDictionary(o => o.PublicId);
    }
    ...
    public IInterpolatedSql Parse(DataSetQueryCriteriaFacets facets, string path)
    {
        ...
        if (facets.Filters?.In is not null)
        {
            fragments.Add(
                InFragment(
        ...
        if (facets.Filters?.NotIn is not null)
        {
            fragments.Add(
                InFragment(
        ...
        bool negate = false)
    {
        ...
        if (!_filterOptionsByPublicId.TryGetValue(filterOptionId, out var option))
        {
        ...
        builder += $"{DataTable.Ref().Col(option.ColumnName):raw} {(negate ? "!=" : "="):raw} {filterOptionId}";
        ...
        bool negate = false)
    {
        ...
        var options = filterOptionIds
            .Distinct()
            .Select(id => _filterOptionsByPublicId.GetValueOrDefault(id))
            .OfType<ParquetFilterOption>()
            .ToList();
        ...
        return builder
            .AppendLiteral("(")
            .AppendRange(conditions, joinString: " AND ")
            .AppendLiteral(")")
            .Build();
    }
    ...
    private WarningViewModel CreateNotFoundWarning(HashSet<string> filterOptionIds, string path) => new()
    {
        ...
        Detail = new NotFoundItemsErrorDetail<string>(
            filterOptionIds.Where(optionId => !_filterOptionsByPublicId.ContainsKey(optionId))
        )
    };
}
