...
) : IAuthorizationHandlerService
{
    public bool CanAccessUnpublishedData()
    {
        ...
        if (
            ...
            && httpContextAccessor.HttpContext?.Request.Headers.UserAgent.Contains(SecurityConstants.AdminUserAgent)
                == true
        )
        {
            return true;
    ...
    {
        return await previewTokenService.ValidatePreviewTokenForDataSet(dataSetId: dataSet.Id);
    }

    public async Task<bool> RequestHasValidPreviewToken(DataSetVersion dataSetVersion)
    {
        return await previewTokenService.ValidatePreviewTokenForDataSetVersion(dataSetVersionId: dataSetVersion.Id);
    }
}
