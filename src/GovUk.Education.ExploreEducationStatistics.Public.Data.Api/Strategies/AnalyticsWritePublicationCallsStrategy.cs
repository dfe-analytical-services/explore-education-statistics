...
) : IAnalyticsWriteStrategy
{
    ...
    private readonly IWorkflowActor<CapturePublicationCallRequest> _workflowActor = new WorkflowActor(
    ...
    public async Task Report(IAnalyticsCaptureRequest request, CancellationToken cancellationToken)
    {
        if (request is not CapturePublicationCallRequest captureRequest)
        {
            throw new ArgumentException(
                $"Request must be of type {nameof(CapturePublicationCallRequest)}. It is {request.GetType().FullName}",
                nameof(request)
            );
        }
    ...
    private class WorkflowActor(string analyticsPath) : WorkflowActorBase<CapturePublicationCallRequest>(analyticsPath)
    {
