...
public static partial class LocationStringValidators
{
        ...
        return rule.NotEmpty().Custom((value, context) =>
        {
            ...
            if (!HasAllowedProperty(level: level, property: property))
            {
                context.AddFailure(
                    ...
                    detail: new AllowedPropertyErrorDetail(property, GetAllowedProperties(level: level))
                );
                return;
            }
    ...
    public record AllowedPropertyErrorDetail(string Value, IEnumerable<string> AllowedProperties)
