...
) : IParquetLocationRepository
{
    ...
    {
        var locationsByLevel = locations.GroupBy(location => location.ParsedLevel()).ToList();

        var allOptions = new List<ParquetLocationOption>();

        foreach (var levelGroup in locationsByLevel)
        {
            var columnValues = levelGroup.Select(MapLocationToColumnValue).ToList();

            var whereBuilder = new DuckDbSqlBuilder();

            var groups = columnValues.GroupBy(cv => cv.Column).ToList();
            ...
            var optionMetas = await command.QueryAsync<ParquetLocationOption>(cancellationToken: cancellationToken);
    ...
    private static (string Column, string Value) MapLocationToColumnValue(IDataSetQueryLocation location)
    {
        ...
        {
            DataSetQueryLocationCode locationCode => (Column: LocationOptionsTable.Cols.Code, Value: locationCode.Code),

            DataSetQueryLocationId locationId => (Column: LocationOptionsTable.Cols.PublicId, Value: locationId.Id),

            DataSetQueryLocationLocalAuthorityCode localAuthority => (
                Column: LocationOptionsTable.Cols.Code,
                Value: localAuthority.Code
            ),
            DataSetQueryLocationLocalAuthorityOldCode localAuthority => (
                Column: LocationOptionsTable.Cols.OldCode,
                Value: localAuthority.OldCode
            ),

            DataSetQueryLocationProviderUkprn provider => (
                Column: LocationOptionsTable.Cols.Ukprn,
                Value: provider.Ukprn
            ),

            DataSetQueryLocationSchoolUrn school => (Column: LocationOptionsTable.Cols.Urn, Value: school.Urn),
            DataSetQueryLocationSchoolLaEstab { LaEstab: not null } school => (
                Column: LocationOptionsTable.Cols.LaEstab,
                Value: school.LaEstab
            ),
    ...
    )
    {
        ...
        return levels.Select(EnumUtil.GetFromEnumValue<GeographicLevel>).ToHashSet();
    }
}
