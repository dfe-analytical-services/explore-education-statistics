...
public class ParquetLocationOptionRepository(
    IDuckDbConnection duckDbConnection,
    IParquetPathResolver parquetPathResolver)
    : IParquetLocationOptionRepository
{
    public async Task<IEnumerable<ParquetLocationOption>> List(
        DataSetVersion dataSetVersion,
        IEnumerable<int> ids,
        CancellationToken cancellationToken = default)
    {
        return await ListById<ParquetLocationOption>(
            dataSetVersion: dataSetVersion,
            ids: ids.ToList(),
            columns: ["*"],
    ...
    private static (string Column, string Value) MapLocationToColumnValue(DataSetQueryLocation location)
    {
        return location switch
        {
            ...
            DataSetQueryLocationId locationId =>
                (Column: LocationsTable.Cols.Id, Value: locationId.Id),

            DataSetQueryLocationLocalAuthority { Code: not null } localAuthority =>
                (Column: LocationsTable.Cols.Code, Value: localAuthority.Code),
            DataSetQueryLocationLocalAuthority { OldCode: not null } localAuthority =>
                (Column: LocationsTable.Cols.OldCode, Value: localAuthority.OldCode),

            DataSetQueryLocationProvider provider =>
                (Column: LocationsTable.Cols.Ukprn, Value: provider.Ukprn),

            DataSetQueryLocationSchool { Urn: not null } school =>
                (Column: LocationsTable.Cols.Urn, Value: school.Urn),
            DataSetQueryLocationSchool { LaEstab: not null } school =>
