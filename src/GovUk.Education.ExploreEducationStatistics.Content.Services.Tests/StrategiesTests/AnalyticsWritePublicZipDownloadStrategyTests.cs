    ...
    public class ReportTests : AnalyticsWritePublicZipDownloadStrategyTests
    {
        ...
        public async Task ProduceTwoRequestFiles_Success()
        {
            ...
            var files = Directory
                .GetFiles(
                    pathResolver.BuildOutputDirectory(
                        AnalyticsWritePublicZipDownloadStrategy.OutputSubPaths
                    )
                )
                .ToList();
            ...
            var requestFile1 = Assert.Single(
                files.Where(file => file.Contains(releaseVersionId1.ToString())).ToList()
            );
            ...
            var requestFile2 = Assert.Single(
                files.Where(file => file.Contains(releaseVersionId2.ToString())).ToList()
            );
