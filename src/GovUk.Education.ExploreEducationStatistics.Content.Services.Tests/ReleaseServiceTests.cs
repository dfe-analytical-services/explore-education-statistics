...
public class ReleaseServiceTests
{
    ...
    public async Task GetRelease()
    {
        ...
        await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
        {
            ...
            contentDbContext.DataBlockVersions.AddRange(
                Release1DataBlockVersion1,
                Release1DataBlockVersion2
            );
        ...
        await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                "<p>Release 1 summary 2 order 0</p>",
            ...
            Assert.Equal(
                "<p>Release 1 summary 3 order 1</p>",
            ...
            Assert.Equal(
                "<p>Release 1 summary 1 order 2</p>",
            ...
            Assert.Equal(
                "<p>Release 1 section 1 order 0</p>",
                (content[2].Content[0] as HtmlBlockViewModel)?.Body
            );
            Assert.Equal(Release1Section1HtmlContentBlock3.Id, content[2].Content[1].Id);
            Assert.Equal(
                "<p>Release 1 section 1 order 1</p>",
                (content[2].Content[1] as HtmlBlockViewModel)?.Body
            );
            Assert.Equal(Release1Section1HtmlContentBlock1.Id, content[2].Content[2].Id);
            Assert.Equal(
                "<p>Release 1 section 1 order 2</p>",
                (content[2].Content[2] as HtmlBlockViewModel)?.Body
            );
    ...
    public async Task List()
    {
        ...
        await using (var contentDbContext = InMemoryContentDbContext(contextId))
        {
            ...
            Assert.Equal(
                release2Version1.Release.TimePeriodCoverage.GetEnumLabel(),
                releases[0].CoverageTitle
            );
            ...
            Assert.Equal(
                release1Version1.Release.TimePeriodCoverage.GetEnumLabel(),
                releases[1].CoverageTitle
            );
    ...
    public async Task List_FiltersDraftReleases()
    {
        Publication publication = _dataFixture
            .DefaultPublication()
            .WithReleases(
                _dataFixture.DefaultRelease(publishedVersions: 1, draftVersion: true).Generate(2)
            );
