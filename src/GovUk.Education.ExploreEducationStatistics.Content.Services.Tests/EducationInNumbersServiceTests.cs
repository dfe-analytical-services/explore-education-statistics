...
public class EducationInNumbersServiceTests
{
    ...
    public async Task ListSitemapItems_Success()
    {
            ...
            contentDbContext.EducationInNumbersPages.AddRange(
                [
                    ...
                    new EducationInNumbersPage
                    {
                        ...
                        Published = new DateTime(2000, 01, 01),
                        Created = new DateTime(2000, 01, 01),
                        Updated = new DateTime(2002, 01, 01),
                    },
                    // unpublished so won't appear
                    new EducationInNumbersPage { Slug = "test-4", Created = new DateTime(2003, 01, 01) },
                ]
            );
            await contentDbContext.SaveChangesAsync();
        }
        ...
        await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(new DateTime(2000, 1, 1), viewModels[0].LastModified); // Created date because no Updated date
            Assert.Equal("test-2", viewModels[1].Slug);
            Assert.Equal(new DateTime(2001, 1, 1), viewModels[1].LastModified);
            Assert.Equal("test-3", viewModels[2].Slug);
            Assert.Equal(new DateTime(2002, 1, 1), viewModels[2].LastModified);
        }
    }
