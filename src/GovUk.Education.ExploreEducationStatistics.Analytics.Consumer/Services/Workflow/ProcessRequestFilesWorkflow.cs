...
) : IProcessRequestFilesWorkflow
{
    public async Task Process(IWorkflowActor actor)
    {
        ...
        var filesToProcess = fileAccessor.ListFiles(sourceDirectory).Order().ToList();
        ...
        logger.LogInformation("Found {Count} requests for {Processor} to process", filesToProcess.Count, processorName);
        ...
        var temporaryProcessingDirectory = Path.Combine(baseProcessingDirectory, temporaryProcessingDirectoryName);

        var fileBatches = filesToProcess.Batch(batchSize).Select(batch => batch.ToList()).ToList();

        // For each batch of files to process, move them into their own folder
        // under an overarching temporary processing folder.
        fileBatches.ForEach(
            (batchFilenames, index) =>
            {
        ...
        var someFileBatchesProcessedSuccessfully = batchProcessingResults.Result.Any(result => result);
    ...
    )
    {
        ...
        catch (Exception e)
        {
            ...
            var failuresDirectory = Path.Combine(sourceDirectory, "failures", temporaryProcessingDirectoryName);
    ...
    private static string GetBatchDirectoryPath(string temporaryProcessingDirectory, int batchNumber)
    {
        return Path.Combine(temporaryProcessingDirectory, batchNumber.ToString());
    }
