...
internal class ContentApiClientMockBuilder
{
    ...
    public ContentApiClientMockBuilder()
    {
        ...
        _mock
            .Setup(m =>
                m.GetPublicationLatestReleaseSearchableDocument(
                    It.IsAny<string>(),
                    It.IsAny<CancellationToken>()
                )
            )
            .ReturnsAsync(() =>
                _releaseSearchableDocument ?? _releaseSearchableDocumentBuilder.Build()
            );
        ...
        _mock
            .Setup(m =>
                m.GetReleasesForPublication(It.IsAny<string>(), It.IsAny<CancellationToken>())
            )
    ...
    public ContentApiClientMockBuilder WhereReleaseSearchViewModelIs(
        Func<
            ReleaseSearchableDocumentBuilder,
            ReleaseSearchableDocumentBuilder
        > modifyReleaseSearchViewModel
    )
    {
    ...
    public ContentApiClientMockBuilder WhereThemeHasPublications(
        params PublicationInfo[] publications
    )
    {
    ...
    )
    {
        _mock
            .Setup(m =>
                m.GetReleaseSummary(publicationSlug, releaseSlug, It.IsAny<CancellationToken>())
            )
    ...
    public class Asserter(Mock<IContentApiClient> mock)
    {
        ...
        {
            mock.Verify(
                m => m.GetPublicationsForTheme(expectedThemeId, It.IsAny<CancellationToken>()),
                Times.Once
            );
        }
        ...
        {
            mock.Verify(
                m => m.GetReleasesForPublication(publicationSlug, It.IsAny<CancellationToken>()),
                Times.Once
            );
        }

        public void ReleaseSummaryRequestedForPublication(
            string publicationSlug,
            string releaseSlug
        )
        {
