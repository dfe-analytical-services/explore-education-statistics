...
public class ContentApiClientTests(ITestOutputHelper output)
{
    ...
    protected void PrintAsJson<T>(T obj) =>
        output.WriteLine(
            ...
            typeof(T)
                ...
                .Select(propertyInfo =>
                    (Action)(
                        () =>
                            Assert.Equal(
                                propertyInfo.GetValue(expected),
                                propertyInfo.GetValue(actual)
                            )
                    )
                )
        );
    ...
    public abstract class LocalDevelopmentIntegrationTests(ITestOutputHelper output)
        : ContentApiClientTests(output)
    {
        /// <summary>
        /// Ensure ContentAPI is running locally on port 5010
        /// </summary>
        public class CallLocalService(ITestOutputHelper output)
            : LocalDevelopmentIntegrationTests(output)
        {
            ...
            public async Task GetExampleSeedDocument()
            {
                ...
                var sut = GetSut();
                var publicationSlug =
                    "seed-publication-permanent-and-fixed-period-exclusions-in-england";

                // ACT
                var actual = await sut.GetPublicationLatestReleaseSearchableDocument(
                    publicationSlug
                );
                ...
                var expected = new ReleaseSearchableDocument
                {
                    ...
                    PublicationTitle =
                        "Seed publication - Permanent and fixed-period exclusions in England",
                    Summary =
                        "Seed publication - Permanent and fixed-period exclusions in England summary",
        ...
        public class CallUnknownService(ITestOutputHelper output)
            : LocalDevelopmentIntegrationTests(output)
        {
            private IContentApiClient GetSut() =>
                base.GetSut(httpClient =>
            ...
            public async Task UnknownEndpointShouldThrow()
            {
                ...
                var exception = await Record.ExceptionAsync(() =>
                    sut.GetPublicationLatestReleaseSearchableDocument(publicationSlug)
                );

                // ASSERT
                Assert.NotNull(exception);
                var unableToGetPublicationLatestReleaseSearchViewModelException =
                    Assert.IsType<UnableToGetPublicationLatestReleaseSearchViewModelException>(
                        exception
                    );
                Assert.Contains(
                    publicationSlug,
                    unableToGetPublicationLatestReleaseSearchViewModelException.Message
                );
            }
        }
    }
