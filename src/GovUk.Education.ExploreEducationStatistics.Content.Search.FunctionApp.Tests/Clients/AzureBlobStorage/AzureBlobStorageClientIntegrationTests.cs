...
public class AzureBlobStorageClientIntegrationTests
{
    ...
    public class IntegrationTests
    {
        ...
        public class HiveITAzureAccount : AzureBlobStorageClientIntegrationTests
        {
            ...
            private const string StorageAccountAccessKey =
                "-- azure storage account access key here --";
            ...
            private AzureBlobStorageClient GetSut() =>
                base.GetSut(IntegrationTestStorageAccountConnectionString);

            [Fact(
                Skip = "This integration test creates a blob in an Azure Storage Account and retrieves it again."
            )]
            ...
            [Fact(
                Skip = "This integration test gets a non-existent blob from Azure Storage Account."
            )]
            public async Task DownloadBlob_WhenBlobDoesNotExist_ThenThrows()
            {
                ...
                var azureBlobStorageNotFoundException =
                    Assert.IsType<AzureBlobStorageNotFoundException>(actual);
                Assert.Equal(uniqueBlobName, azureBlobStorageNotFoundException.BlobName);
                Assert.Equal(
                    IntegrationTestContainerName,
                    azureBlobStorageNotFoundException.ContainerName
                );
            }

            [Fact(
                Skip = "This integration test deletes a non-existent blob from Azure Storage Account."
            )]
