...
{
    private readonly SearchableDocumentCreatorMockBuilder _searchableDocumentCreatorMockBuilder =
        new();

    private RefreshSearchableDocumentFunction GetSut() =>
        new(_searchableDocumentCreatorMockBuilder.Build(), new TestableCommandHandler());
    ...
    public async Task WhenMessageWithPublicationSlug_ThenDocumentCreatorCalled()
    {
        ...
        var command = new RefreshSearchableDocumentMessageDto { PublicationSlug = publicationSlug };

        // ACT
        await sut.RefreshSearchableDocument(command, new FunctionContextMockBuilder().Build());

        // ASSERT
        _searchableDocumentCreatorMockBuilder.Assert.CreateSearchableDocumentCalledFor(
            publicationSlug
        );
    }
    ...
    public async Task WhenMessageHasNoPublicationSlug_ThenDocumentCreatorNotCalled(
    ...
    public async Task GivenMessageWithPublicationSlug_WhenDocumentCreatorCalled_ThenResponseDataIsReturned()
    {
        ...
        // ACT
        var response = await sut.RefreshSearchableDocument(
            command,
