...
public class SeedDataCommand : ICommand
{
    ...
    )
    {
        ...
        var options = new DbContextOptionsBuilder<PublicDataDbContext>().UseNpgsql(dataSourceBuilder.Build()).Options;
    ...
    private class Seeder
    {
        ...
        public async Task Generate()
        {
            ...
            var columns = _duckDbConnection
                .Query<(string ColumnName, string ColumnType)>($"DESCRIBE SELECT * FROM '{_dataFilePath}'")
            ...
            var dataSetVersion = new DataSetVersion
            {
                ...
                MetaSummary = new DataSetVersionMetaSummary
                {
                    ...
                    Filters = metaFileRows
                        .Where(row =>
                            row.ColType == MetaFileRow.ColumnType.Filter && allowedColumns.Contains(row.ColName)
                        )
                    ...
                    Indicators = metaFileRows
                        .Where(row =>
                            row.ColType == MetaFileRow.ColumnType.Indicator && allowedColumns.Contains(row.ColName)
                        )
                        .OrderBy(row => row.Label)
                        .Select(row => row.Label)
                        .ToList(),
        ...
        private async Task CreateDataSetMeta(IList<MetaFileRow> metaFileRows, IReadOnlySet<string> allowedColumns)
        {
        ...
        private async Task CreateFilterMetas(IEnumerable<MetaFileRow> metaFileRows, IReadOnlySet<string> allowedColumns)
        {
            ...
            foreach (var metaRow in filterMetaRows)
            {
                ...
                var defaultOption = _dbContext
                    ...
                    .FirstOrDefault(option =>
                        metaRow.FilterDefault != null ? option.Label == metaRow.FilterDefault : option.Label == "Total"
                    );
        ...
        private async Task CreateLocationMetas(IReadOnlySet<string> allowedColumns)
        {
            ...
            var metas = levels
                .Select(level => new LocationMeta { DataSetVersionId = _seed.DataSetVersionId, Level = level })
                .ToList();
        ...
        private async Task CreateParquetTimePeriodsTable(DataSetVersion version)
        {
            ...
            var timePeriods = version.TimePeriodMetas.OrderBy(tp => tp.Period).ThenBy(tp => tp.Code);
        ...
        {
            await _duckDbConnection
                .SqlBuilder($"EXPORT DATABASE '{dataSetVersionDirectory:raw}' (FORMAT PARQUET, CODEC ZSTD)")
                .ExecuteAsync(cancellationToken: _cancellationToken);
            ...
            var newLines = (await File.ReadAllLinesAsync(loadSqlFilePath, _cancellationToken)).Select(line =>
