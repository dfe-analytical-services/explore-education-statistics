    ...
    : OptimisedIntegrationTestBase<Startup>(fixture)
{
        ...
        : DataSetVersionMappingControllerTests(fixture)
    {
        ...
        public async Task UserOnPublicationTeam_CanGetLocationMappings_ReturnsLocationMappingPlan(
        ...
        )
        {
            ...
            var response = await GetLocationMappings(nextDataSetVersionId: nextDataSetVersion.Id, user: identityUser);

            // Content correctness of the returned mapping plan is already covered by BauUser_Success.
            // This test only needs to prove that a user with a publication role is not forbidden.
            response.AssertOk<LocationMappingPlan>();
        }
        ...
        public async Task UserWithOnlyPreReleaseRole_Returns403()
        {
            ...
            var releaseVersion = await fixture
                .GetContentDbContext()
                .ReleaseFiles.Where(rf => rf.PublicApiDataSetId == nextDataSetVersion.DataSetId)
            ...
            await fixture
                .GetContentDbContext()
                .AddTestData(context => context.UserPreReleaseRoles.Add(userPreReleaseRole));
        ...
        : DataSetVersionMappingControllerTests(fixture)
    {
        ...
        {
            // CanManagePublicApiDataSets is BAU-only, so a user with a publication role on the
            // mapping's publication - but who is not BAU - should still be forbidden.
            ClaimsPrincipal identityUser = DataFixture.StandardUser();
            User user = DataFixture.DefaultUser().WithId(identityUser.GetUserId());
        ...
        : DataSetVersionMappingControllerTests(fixture)
    {
        ...
        [InlineData(PublicationRole.Approver)]
        [InlineData(PublicationRole.Drafter)]
        public async Task UserOnPublicationTeam_CanGetFilterMappings_ReturnsFilterMappingPlan(
            PublicationRole publicationRole
        )
        {
            ClaimsPrincipal identityUser = DataFixture.StandardUser();
            ...
            var response = await GetFilterMappings(nextDataSetVersionId: nextDataSetVersion.Id, user: identityUser);

            // Content correctness of the returned mapping plan is already covered by BauUser_Success.
            // This test only needs to prove that a user with a publication role is not forbidden.
            response.AssertOk<FilterMappingPlan>();
        }
