...
[Collection(nameof(OptimisedDataSetVersionMappingControllerCollection))]
public class OptimisedGetLocationMappingsTests : IAsyncLifetime
{
...
[Collection(nameof(OptimisedDataSetVersionMappingControllerCollection))]
public class OptimisedApplyBatchLocationMappingUpdatesTests : IAsyncLifetime
{
    private const string BaseUrl = "api/public-data/data-set-versions";
                ...
                GeographicLevel.Country, new LocationLevelMappings
                {
                    Mappings = new Dictionary<string, LocationOptionMapping>
                    {
                        { "source-location-1-key", originalCountryMappingNotUpdated },
                        {
                            ...
                            "source-location-3-key", originalCountryMappingToUpdate with
                            {
                                Type = MappingType.ManualNone,
                                CandidateKey = null
                            }
    ...
    {
        var (initialDataSetVersion, nextDataSetVersion) = await CreateInitialAndNextDataSetVersion(_fixture.GetPublicDataDbContext(), _fixture.GetContentDbContext(), _output);
...
[CollectionDefinition(nameof(OptimisedDataSetVersionMappingControllerCollection))]
public class OptimisedDataSetVersionMappingControllerCollection : ICollectionFixture<OptimisedHttpClientWithPsqlFixture>
{
