    ...
    public class ReleaseControllerTests
    {
        ...
        public async Task AddAncillaryFilesAsync_UploadsTheFiles_Returns_Ok()
        {
            ...
            var controller = new ReleasesController(releaseService.Object, fileStorageService.Object,
                importService.Object, publicationService.Object);
        ...
        public async Task GetAncillaryFilesAsync_Returns_A_List_Of_Files()
        {
            ...
            releaseService.Setup(s => s.GetAsync(It.IsAny<Guid>()))
                .Returns(Task.FromResult(new Release {Id = releaseId}));
            fileStorageService.Setup(s => s.ListFilesAsync(releaseId, ReleaseFileTypes.Ancillary))
        ...
        public async Task GetDataFilesAsync_Returns_A_List_Of_Files()
        {
            ...
            releaseService.Setup(s => s.GetAsync(It.IsAny<Guid>()))
                .Returns(Task.FromResult(new Release {Id = releaseId}));
            fileStorageService.Setup(s => s.ListFilesAsync(releaseId, ReleaseFileTypes.Data))
                .Returns(Task.FromResult(testFiles));
        ...
        public async void Edit_Release_Summary_Returns_Ok()
        {
            ...
            // Method under test
            var okObjectResult = await controller.EditReleaseSummaryAsync(new EditReleaseSummaryViewModel(), releaseId);
            Assert.IsAssignableFrom<OkObjectResult>(okObjectResult.Result);
            var result = (okObjectResult.Result as OkObjectResult)?.Value;
            Assert.IsAssignableFrom<ReleaseViewModel>(result);
            Assert.Equal(releaseId, (result as ReleaseViewModel).Id);
        }
