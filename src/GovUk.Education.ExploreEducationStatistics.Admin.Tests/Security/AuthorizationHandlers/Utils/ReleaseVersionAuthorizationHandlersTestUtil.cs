...
public static class ReleaseVersionAuthorizationHandlersTestUtil
{
    ...
    {
        var inTeamScenarios = CreateUserInProductionTeamScenarios(
            releaseVersion,
            rolesExpectedToSucceed
        );
        var notInTeamScenario = CreateUserNotInProductionTeamScenario(
            releaseVersion,
            rolesExpectedToSucceed
        );
        var allScenarios = new List<ReleaseVersionHandlerTestScenario>(inTeamScenarios)
        {
            notInTeamScenario,
        };
        await allScenarios
            ...
            .ForEachAwaitAsync(scenario =>
                AssertReleaseVersionHandlerHandlesScenarioSuccessfully<TRequirement>(
                    handlerSupplier,
                    scenario
                )
            );
    ...
    public static async Task AssertReleaseVersionHandlerSucceedsWithCorrectPublicationRoles<TRequirement>(
        ...
        where TRequirement : IAuthorizationRequirement
    {
        var inTeamScenarios = CreateUserInPublicationTeamScenarios(
            releaseVersion,
            rolesExpectedToSucceed
        );
        var notInTeamScenario = CreateUserNotInPublicationTeamScenario(
            releaseVersion,
            rolesExpectedToSucceed
        );
        var allScenarios = new List<ReleaseVersionHandlerTestScenario>(inTeamScenarios)
    ...
    private static ReleaseVersionHandlerTestScenario CreateUserNotInProductionTeamScenario(
        ReleaseVersion releaseVersion,
    ...
    )
    {
        ...
        var notInTeamScenario = new ReleaseVersionHandlerTestScenario
        {
            ...
            UnexpectedPassMessage =
                "Expected not having a role on the Release would have made the handler fail",
        };
            ...
            .Select(role =>
            {
                ...
                return new ReleaseVersionHandlerTestScenario
                {
                    ...
                    UnexpectedFailMessage =
                        "Expected role " + role + " to have made the handler succeed",
                    UnexpectedPassMessage =
                        "Expected role " + role + " to have made the handler fail",
                };
            })
            .ToList();
        return scenarios;
    ...
    private static ReleaseVersionHandlerTestScenario CreateUserNotInPublicationTeamScenario(
        ReleaseVersion releaseVersion,
    ...
    )
    {
        ...
        var notInTeamScenario = new ReleaseVersionHandlerTestScenario
        {
            ...
            ExpectedToPass = false,
            UnexpectedPassMessage =
                "Expected not having a role on the Publication would have made the handler fail",
        };
        return notInTeamScenario;
    }
    ...
    public static async Task AssertReleaseVersionHandlerHandlesScenarioSuccessfully<TRequirement>(
        Func<ContentDbContext, IAuthorizationHandler> handlerSupplier,
        ReleaseVersionHandlerTestScenario scenario
    )
        where TRequirement : IAuthorizationRequirement
    {
