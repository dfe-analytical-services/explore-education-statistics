...
public class PublicationServiceTests
{
                ...
                .Setup(s =>
                    s.MatchesPolicy(
                        It.Is<Tuple<Publication, ReleaseRole>>(tuple =>
                            tuple.Item1.Id == publication.Id
                            && tuple.Item2 == ReleaseRole.Contributor
                        ),
    ...
    public async Task UpdatePublication_NotPublished()
    {
        ...
        methodologyService
            .Setup(s =>
                s.PublicationTitleOrSlugChanged(
                    publication.Id,
                    publication.Slug,
                    "New title",
                    "new-title"
        ...
        _publicationCacheServiceMockBuilder.Assert.CacheNotInvalidatedForPublicationAndReleases(
            publication.Slug
        );
    ...
    public async Task UpdatePublication_AlreadyPublished()
    {
        ...
        _publicationCacheServiceMockBuilder.Assert.CacheInvalidatedForPublicationAndReleases(
            publication.Slug
        );
    }
    ...
    )
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            var updatedPublication = await context.Publications.SingleAsync(p =>
                p.Id == publication.Id
            );
    ...
    public async Task UpdatePublication_CacheInvalidatedForSupersededPublications()
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            {
                _publicationCacheServiceMockBuilder.Assert.CacheInvalidatedForPublicationEntry(
                    expected.Slug
                );
            }
    ...
    )
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            publicationCacheService
                .Setup(mock => mock.RemovePublication("old-title"))
                .ReturnsAsync(Unit.Instance);
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            Assert.InRange(
                DateTime.UtcNow.Subtract(updatedPublication.Updated!.Value).Milliseconds,
                0,
                1500
            );
    ...
    public async Task UpdatePublication_RemovesSupersededPublicationCacheBlobs()
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            var updatedPublication = await context.Publications.FirstAsync(p =>
                p.Title == "Test title"
            );
    ...
    public async Task UpdatePublication_CreateRedirectIfLiveSlugChanged()
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            publicationCacheService
                ...
                .ReturnsAsync(new PublicationCacheViewModel());
            publicationCacheService
                .Setup(mock => mock.RemovePublication("current-title"))
                .ReturnsAsync(Unit.Instance);
    ...
    public async Task UpdatePublication_ChangeBackToPreviousLiveSlug()
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            publicationCacheService
                ...
                .ReturnsAsync(new PublicationCacheViewModel());
            publicationCacheService
                .Setup(mock => mock.RemovePublication("title"))
                .ReturnsAsync(Unit.Instance);
    ...
    public async Task UpdateExternalMethodology()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                "http://test.external.methodology/new",
                dbPublication.ExternalMethodology.Url
            );
        }
    }
    ...
    public async Task ListReleaseVersions_Latest_SingleRelease()
    {
        Publication publication = _dataFixture
            .DefaultPublication()
            .WithReleases(
                _dataFixture.DefaultRelease(publishedVersions: 1).WithLabel("initial").Generate(1)
            );
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            Assert.Equal(
                ...
                summaryViewModel.TimePeriodCoverage
            );
            ...
            Assert.Equal(
                ...
                summaryViewModel.PublishScheduled
            );
    ...
    public async Task ListReleaseVersionsPaginated_Latest()
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            foreach (var (page, years) in expectedPagesAndYears)
            {
                ...
                Assert.Equal(
                    expectedLatestReleaseVersionIds,
                    pagedResult.Results.Select(r => r.Id).ToArray()
                );
            }
        }
    }
    ...
    public async Task AddReleaseSeriesLegacyLink()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            var actualPublication = await contentDbContext.Publications.SingleAsync(p =>
                p.Id == publication.Id
            );
            ...
            Assert.Equal(
                legacyLink.LegacyLinkDescription,
                actualReleaseSeries[1].LegacyLinkDescription
            );
    ...
    public async Task UpdateReleaseSeries_InvalidSeriesItem1()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                "LegacyLink details shouldn't be set if ReleaseId is set.",
                exception.Message
            );
        }
    }
    ...
    public async Task UpdateReleaseSeries_InvalidSeriesItem2()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                "LegacyLink details should be set if ReleaseId is null.",
                exception.Message
            );
        }
    }
