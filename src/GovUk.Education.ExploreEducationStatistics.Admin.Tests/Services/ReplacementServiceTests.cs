...
public class ReplacementServiceTests
{
    ...
    public async Task Replace_ReplacementPlanInvalid()
    {
        ...
        var table = new TableBuilderConfiguration
        {
            TableHeaders = new TableHeaders
            {
                ...
                {
                    new() { TableHeader.NewLocationHeader(GeographicLevel.Country, originalLocation.Id.ToString()) },
                },
    ...
    public async Task Replace_ReplacementImportNotComplete()
    {
        ...
        var originalFilterItem1 = new FilterItem { Id = Guid.NewGuid(), Label = "Test filter item - not changing" };

        var originalFilterItem2 = new FilterItem { Id = Guid.NewGuid(), Label = "Test filter item - not changing" };

        var replacementFilterItem1 = new FilterItem { Id = Guid.NewGuid(), Label = "Test filter item - not changing" };

        var replacementFilterItem2 = new FilterItem { Id = Guid.NewGuid(), Label = "Test filter item - not changing" };
        ...
        var dataBlock = new DataBlock
        {
            ...
            Table = new TableBuilderConfiguration
            {
                TableHeaders = new TableHeaders
                {
                    ...
                    Rows = new List<TableHeader> { new(originalIndicator.Id.ToString(), TableHeaderType.Indicator) },
                },
            },
        ...
        var footnoteForFilterGroup = CreateFootnote(
            ...
            filterGroupFootnotes: new List<FilterGroupFootnote> { new() { FilterGroup = originalFilterGroup1 } }
        );

        var footnoteForFilterItem = CreateFootnote(
            statsReleaseVersion,
            "Test footnote for Filter item",
            filterItemFootnotes: new List<FilterItemFootnote> { new() { FilterItem = originalFilterItem1 } }
        );
        ...
        var dataImport = new DataImport { FileId = replacementFile.Id, Status = DataImportStatus.STAGE_2 };
    ...
    public async Task Replace_LinkedFilesForReplacementNotFound()
    {
        ...
        var releaseFileRepository = new Mock<IReleaseFileRepository>(Strict);
        releaseFileRepository
            .Setup(mock => mock.CheckLinkedOriginalAndReplacementReleaseFilesExist(releaseVersion.Id, originalFileId))
    ...
    public async Task Replace()
    {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(statisticsDbContextId))
        {
            ...
            var replacedFootnoteForFilterGroup = await GetFootnoteById(statisticsDbContext, footnoteForFilterGroup.Id);
            ...
            var replacedFootnoteForFilterItem = await GetFootnoteById(statisticsDbContext, footnoteForFilterItem.Id);
            ...
            Assert.Equal(replacementFilterItem1.Id, replacedFootnoteForFilterItem.FilterItems.First().FilterItem.Id);
            Assert.Equal(
            ...
            Assert.Equal(replacementIndicator.Label, replacedFootnoteForIndicator.Indicators.First().Indicator.Label);
            ...
            Assert.Equal(replacementReleaseSubject.SubjectId, replacedFootnoteForSubject.Subjects.First().Subject.Id);
        ...
        var originalFilter1 = new Filter
        {
            ...
            FilterGroups = new List<FilterGroup>
            {
                new()
                {
                    ...
                    {
                        new() { Id = originalFilterItem1Id, Label = "Test filter item - not changing" },
                    },
                },
            },
        };
        ...
        var originalFilter2 = new Filter
        {
            ...
            FilterGroups = new List<FilterGroup>
            {
                new()
                {
                    ...
                    FilterItems = new List<FilterItem>
                    {
                        new() { Id = originalFilterItem2Id, Label = "Test filter item - not changing" },
                    },
                },
            },
        };
                    ...
                    Map = new MapChartConfig
                    {
                        DataSetConfigs = new List<ChartDataSetConfig>
                        {
                            new()
                            {
                                DataSet = new ChartBaseDataSet
                                {
                                    Filters = new List<Guid> { originalFilterItem1.Id, originalFilterItem2.Id },
                                    Indicator = originalIndicator.Id,
                                    Location = new ChartDataSetLocation
                                    {
                                        Level = GeographicLevel.LocalAuthority.ToString().CamelCase(),
    ...
    public async Task Replace_IndicatorSequenceIsReplaced()
    {
        ...
        var statsReleaseVersion = _fixture.DefaultStatsReleaseVersion().WithId(contentReleaseVersion.Id).Generate();
        ...
        var originalReleaseFile = new ReleaseFile { ReleaseVersion = contentReleaseVersion, File = originalFile };
        ...
        releaseFileRepository
            .Setup(mock =>
                mock.CheckLinkedOriginalAndReplacementReleaseFilesExist(contentReleaseVersion.Id, originalFile.Id)
            )
            .ReturnsAsync((originalReleaseFile, replacementReleaseFile));
