...
public class ReleaseInviteServiceTests
{
    ...
    public async Task InviteContributor_CreateInvite()
    {
        ...
        var releaseVersionIds = ListOf(publication.Releases[0].Versions[0].Id, publication.Releases[1].Versions[0].Id);
        ...
        userReleaseInviteRepository
            .Setup(mock => mock.UserHasInvites(releaseVersionIds, Email, ReleaseRole.Contributor))
            .ReturnsAsync(false)
            .Verifiable();
        userReleaseInviteRepository
            .Setup(mock =>
                mock.CreateManyIfNotExists(releaseVersionIds, Email, ReleaseRole.Contributor, true, CreatedById, null)
            )
        ...
        emailService.Verify(s => s.SendEmail(Email, NotifyContributorTemplateId, expectedTemplateValues), Times.Once);

        VerifyAllMocks(emailService, userReleaseInviteRepository);
    ...
    public async Task InviteContributor_ExistingUser()
    {
        ...
        VerifyAllMocks(userRoleService, emailService, userReleaseRoleRepository);

        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        await using (var usersAndRolesDbContext = InMemoryUserAndRolesDbContext(usersAndRolesDbContextId))
        {
            var userInvites = await usersAndRolesDbContext.UserInvites.AsQueryable().ToListAsync();
    ...
    public async Task InviteContributor_UserAlreadyHasReleaseRoles()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var result = await service.InviteContributor(
                email: Email,
                publicationId: publication.Id,
                releaseVersionIds: ListOf(publication.ReleaseVersions[0].Id, publication.ReleaseVersions[1].Id)
            );
    ...
    public async Task InviteContributor_NotAllReleasesBelongToPublication()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var result = await service.InviteContributor(
                ...
                releaseVersionIds: ListOf(publication1.ReleaseVersions[0].Id, publication2.ReleaseVersions[0].Id)
            );
    ...
    public async Task RemoveByPublication()
    {
        ...
        var userReleaseInviteRepository = new Mock<IUserReleaseInviteRepository>(Strict);
        userReleaseInviteRepository
            .Setup(mock => mock.RemoveByPublicationAndEmail(publication.Id, Email, default, ReleaseRole.Contributor))
            .Returns(Task.CompletedTask)
            .Verifiable();
