...
public class ReleaseInviteServiceTests
{
    ...
    public async Task InviteContributor_CreateInvite()
    {
        Publication publication = _dataFixture
            .DefaultPublication()
            .WithReleases(
                _dataFixture.DefaultRelease(publishedVersions: 0, draftVersion: true).Generate(2)
            );
        ...
        emailService
            .Setup(mock =>
                mock.SendEmail(Email, NotifyContributorTemplateId, expectedTemplateValues)
            )
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var result = await service.InviteContributor(
                ...
                releaseVersionIds: ListOf(
                    publication.ReleaseVersions[0].Id,
    ...
    public async Task InviteContributor_NotAllReleasesBelongToPublication()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var result = await service.InviteContributor(
                ...
                releaseVersionIds: ListOf(
                    publication1.ReleaseVersions[0].Id,
    ...
    public async Task RemoveByPublication()
    {
        ...
        userReleaseInviteRepository
            .Setup(mock =>
                mock.RemoveByPublicationAndEmail(
                    publication.Id,
                    Email,
                    default,
                    ReleaseRole.Contributor
    ...
    private static IOptions<NotifyOptions> DefaultNotifyOptions()
    {
        ...
        {
            ContributorTemplateId = NotifyContributorTemplateId,
        }.ToOptionsWrapper();
    }
