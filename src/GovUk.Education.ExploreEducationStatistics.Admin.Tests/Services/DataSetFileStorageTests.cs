...
public class DataSetFileStorageTests
{
    ...
    public async Task UploadDataSetsToTemporaryStorage_ReturnsUploadDetailsAndScreenerProgress()
    {
        ...
        // Assert that with the "enhanced user journey" flag set to true, some initial screener
        // progress is inserted.
        var expectedInitialScreenerProgress = new DataSetScreenerProgress
        {
            Stage = "Started",
            Completed = false,
            Passed = false,
        ...
        // Expect to see an initial Screener progress entry indicating that screening has started but
        // no progress has yet been made.
        Assert.Equal(expectedInitialScreenerProgress, uploadDetails.ScreenerProgress);

        // Expect the initial Screener progress update date to be set to now.
        Assert.Equal(utcNow, uploadDetails.ScreenerProgressLastUpdated);

        // Expect the initial Screener progress checked date to still be null, as we have not yet
        // requested any progress updates for this data set.
