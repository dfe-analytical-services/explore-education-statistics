...
public class ReleaseChecklistServiceTests
{
    ...
    public async Task GetChecklist_AllErrors(DataSetVersionStatus mappingStatus)
    {
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            methodologyVersionRepository
                .Setup(mock =>
                    mock.GetLatestVersionByPublication(releaseVersion.Release.PublicationId)
                )
            ...
            dataGuidanceService
                .Setup(s =>
                    s.ValidateForReleaseChecklist(releaseVersion.Id, CancellationToken.None)
                )
            ...
            dataSetVersionService
                .Setup(s =>
                    s.GetStatusesForReleaseVersion(releaseVersion.Id, CancellationToken.None)
                )
    ...
    public async Task GetChecklist_AllWarningsWithDataFiles()
    {
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            var noFootnotesWarning = Assert.IsType<NoFootnotesOnSubjectsWarning>(
                checklist.Warnings[2]
            );
    ...
    public async Task GetChecklist_AllWarningsWithUnapprovedMethodology()
    {
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            var methodologyMustBeApprovedError = Assert.IsType<MethodologyNotApprovedWarning>(
