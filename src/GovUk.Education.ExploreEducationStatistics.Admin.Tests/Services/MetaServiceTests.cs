...
public class MetaServiceTests
{
    ...
    public void GetTimeIdentifiers()
    {
        ...
        foreach (
            var category in (TimeIdentifierCategory[])Enum.GetValues(typeof(TimeIdentifierCategory))
        )
        {
            ...
            foreach (var identifier in category.GetTimeIdentifiers())
            {
                ...
                Assert.True(
                    timeIdentifiersRetrieved
                        .Single(ti => ti.Category == category)
                        .TimeIdentifiers.Exists(i => i.Identifier == identifier)
                );
            }
        }
    }
