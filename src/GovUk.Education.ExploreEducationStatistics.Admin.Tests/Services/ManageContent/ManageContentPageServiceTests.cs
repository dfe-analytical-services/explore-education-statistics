...
public class ManageContentPageServiceTests
{
    ...
    public async Task GetManageContentPageViewModel()
    {
        ...
        releaseVersion.RelatedInformation.Add(
            new Link { Description = "Related 1", Url = "https://related-1" }
        );
        ...
        var files = _dataFixture
            ...
            .WithFiles(
                [_dataFixture.DefaultFile(Ancillary), _dataFixture.DefaultFile(FileType.Data)]
            )
            .Generate(2)
        ...
        var methodologyVersionRepository = new Mock<IMethodologyVersionRepository>(
            MockBehavior.Strict
        );
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                releaseVersion.Release.TimePeriodCoverage.GetEnumLabel(),
            ...
            var originalKeyStatDataBlock = (
                releaseVersion.KeyStatistics[1] as KeyStatisticDataBlock
            )!;
            ...
            Assert.Equal(
                releaseVersion.RelatedDashboardsSection.Id,
            ...
            var htmlBlockViewModel = Assert.IsType<HtmlBlockViewModel>(
                contentSections[0].Content[0]
            );
            Assert.Equal("Test block 1", htmlBlockViewModel.Body);

            var dataBlockViewModel = Assert.IsType<DataBlockViewModel>(
                contentSections[0].Content[1]
            );
            ...
            Assert.Equal(
                ...
                contentPublicationReleaseSeries[0].ReleaseSlug
            );
            Assert.Equal(
                releaseVersion.Release.Title,
                contentPublicationReleaseSeries[0].Description
            );
    ...
    public async Task GetManageContentPageViewModel_MapsBlocksCorrectly()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            dataBlockService.Verify(
                mock => mock.GetUnattachedDataBlocks(releaseVersion.Id),
                Times.Once
            );

            releaseFileService.Verify(
                mock => mock.ListAll(releaseVersion.Id, Ancillary, FileType.Data),
                Times.Once
            );
            ...
            var summarySectionBlock = Assert.IsType<HtmlBlockViewModel>(
                contentReleaseSummary.Content[0]
            );
            ...
            Assert.Equal(
                DateTime.Parse("2022-03-12T12:00:00Z"),
                summarySectionBlock.Comments[0].Created
            );
            ...
            Assert.Equal(
                DateTime.Parse("2022-03-14T12:00:00Z"),
                summarySectionBlock.Comments[0].Resolved
            );
            ...
            Assert.Equal(
                DateTime.Parse("2022-03-16T12:00:00Z"),
                summarySectionBlock.Comments[1].Created
            );
            ...
            var contentBlock = Assert.IsType<HtmlBlockViewModel>(
                contentReleaseContent[0].Content[0]
            );
