...
public class DataBlockServiceTests
{
    ...
    public async Task GetDeletePlan()
    {
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            Assert.Equal(
                dataBlockVersion.ContentSection!.Heading,
                dependentBlocks[0].ContentSectionHeading
            );
    ...
    public async Task Delete()
    {
        ...
        releaseVersion.FeaturedTables = ListOf(
            new FeaturedTable { DataBlockId = dataBlockVersion.Id }
        );
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            cacheKeyService
                ...
                .ReturnsAsync(
                    new Either<ActionResult, DataBlockTableResultCacheKey>(dataBlockCacheKey)
                );
    ...
    public async Task Delete_WithVersionAlreadyPublished()
    {
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            cacheKeyService
                .Setup(s =>
                    s.CreateCacheKeyForDataBlock(releaseVersion.Id, draftDataBlockVersion.Id)
                )
                .ReturnsAsync(
                    new Either<ActionResult, DataBlockTableResultCacheKey>(dataBlockCacheKey)
                );
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            Assert.Equal(
                publishedDataBlockVersion.Id,
                remainingDataBlockParent.LatestPublishedVersionId
            );
    ...
    public async Task Create()
    {
        ...
        await using (var context = InMemoryContentDbContext(contextId))
        {
            ...
            var savedRelease = await context.ReleaseVersions.FirstOrDefaultAsync(rv =>
