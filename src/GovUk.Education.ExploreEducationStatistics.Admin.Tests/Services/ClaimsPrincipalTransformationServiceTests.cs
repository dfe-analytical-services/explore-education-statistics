...
public class ClaimsPrincipalTransformationServiceTests(TestApplicationFactory testApp)
    : IntegrationTestFixture(testApp)
{
    ...
    public async Task RoleClaims()
    {
        ...
        Assert.Contains(
            new ClaimViewModel(EesClaimTypes.LocalUserId, userId.ToString()),
            claimList
        );

        // Check that the scope Claim is present.
        Assert.Contains(
            new ClaimViewModel(EesClaimTypes.SupportedMsalScope, "access-admin-api"),
            claimList
        );
        ...
        Assert.DoesNotContain(
            new ClaimViewModel(EesClaimTypes.LocalUserId, unrelatedUserId.ToString()),
            claimList
        );
        Assert.DoesNotContain(new ClaimViewModel(EesClaimTypes.Role, "Role 1"), claimList);
        Assert.DoesNotContain(
            new ClaimViewModel("Role 1 Claim 1", "Role 1 Claim 1 value"),
            claimList
        );
        Assert.DoesNotContain(
            new ClaimViewModel("Role 1 Claim 2", "Role 1 Claim 2 value"),
            claimList
        );
    }
    ...
    private class TestController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<string>>> RoleClaims()
        {
            return await Task.FromResult(
                new OkObjectResult(
                    HttpContext
                        .User.Claims.Select(c => new ClaimViewModel(c.Type, c.Value))
                        .ToList()
                )
            );
        }
