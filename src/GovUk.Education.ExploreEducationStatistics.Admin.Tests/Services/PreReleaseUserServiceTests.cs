...
public class PreReleaseUserServiceTests
{
    ...
    public async Task GetPreReleaseUsersInvitePlan()
    {
        ...
        foreach (var email in allEmails)
        {
            ...
            userReleaseRoleRepository
                .Setup(mock =>
                    mock.HasUserReleaseRole(email, releaseVersion.Id, ReleaseRole.PrereleaseViewer)
                )
                .ReturnsAsync(false)
                .Verifiable();
        }
        ...
        foreach (var email in allEmails)
        {
            ...
            if (usersWithExistingReleaseInviteEmails.Contains(email))
            {
                userReleaseInviteRepository
                    .Setup(mock =>
                        mock.UserHasInvite(releaseVersion.Id, email, ReleaseRole.PrereleaseViewer)
                    )
                    .ReturnsAsync(true)
                    .Verifiable();
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        await using (
            var usersAndRolesDbContext = InMemoryUserAndRolesDbContext(userAndRolesDbContextId)
        )
        {
            var service = SetupPreReleaseUserService(
                contentDbContext,
    ...
    public async Task InvitePreReleaseUsers_FailsSendingEmail_ExistingUser_ApprovedRelease()
    {
        ...
        userReleaseRoleRepository
            .Setup(mock =>
                mock.HasUserReleaseRole(user.Email, releaseVersion.Id, ReleaseRole.PrereleaseViewer)
            )
            .ReturnsAsync(false)
            .Verifiable();
        ...
        userReleaseInviteRepository
            .Setup(mock =>
                mock.UserHasInvite(releaseVersion.Id, user.Email, ReleaseRole.PrereleaseViewer)
            )
            .ReturnsAsync(false)
            .Verifiable();
    ...
    public async Task InvitePreReleaseUsers_FailsSendingEmail_NewUser_ApprovedRelease()
    {
        ...
        )
        {
            ...
            var result = await service.InvitePreReleaseUsers(releaseVersion.Id, ListOf(email));
    ...
    public async Task InvitePreReleaseUsers_InvitesMultipleUsers_ApprovedRelease()
    {
        ...
        var existingUsersByEmail = existingEmailsWithNoRolesOrInvites
            .Concat(emailsWithExistingRoles)
            .Select(email => _dataFixture.DefaultUser().WithEmail(email).Generate())
        ...
        {
            if (
                emailsWithExistingRoles.Contains(email)
                || emailsWithExistingReleaseInvites.Contains(email)
            )
            {
                continue;
            }
    ...
    public async Task RemovePreReleaseUser_NoRemainingReleaseInvites_AcceptedInvite()
    {
        ...
        userReleaseRoleRepository
            .Setup(m =>
                m.RemoveForReleaseVersionAndUser(
                    releaseVersion.Id,
                    user.Id,
                    default,
                    ReleaseRole.PrereleaseViewer
        ...
        )
        {
            ...
            var remainingUserInvites = await usersAndRolesDbContext.UserInvites.ToListAsync();
    ...
    )
    {
        preReleaseService
            .Setup(s =>
                s.GetPreReleaseWindow(It.Is<ReleaseVersion>(rv => rv.Id == releaseVersion.Id))
            )
            .Returns(
                new PreReleaseWindow
                {
                    Start = DateTime.Parse(
                        "2020-09-08T08:30:00.00Z",
    ...
    )
    {
        ...
        return new(
            ...
            userReleaseInviteRepository
                ?? Mock.Of<IUserReleaseInviteRepository>(MockBehavior.Strict)
        );
    }
}
