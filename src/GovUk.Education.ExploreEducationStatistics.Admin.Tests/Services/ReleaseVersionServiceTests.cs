...
public abstract class ReleaseVersionServiceTests
{
    ...
    public class GetDeleteDataFilePlanTests : ReleaseVersionServiceTests
    {
        ...
        [Theory]
        [InlineData(DataSetVersionStatus.Mapping, true)]
        [InlineData(DataSetVersionStatus.Draft, true)]
        [InlineData(DataSetVersionStatus.Published, false)]
        [InlineData(DataSetVersionStatus.Processing, true)]
        [InlineData(DataSetVersionStatus.Failed, true)]
        [InlineData(DataSetVersionStatus.Deprecated, false)]
        [InlineData(DataSetVersionStatus.Withdrawn, false)]
        [InlineData(DataSetVersionStatus.Cancelled, true)]
    ...
    public class RemoveDataFilesTests : ReleaseVersionServiceTests
    {
        ...
        public async Task Success()
        {
            ...
            footnoteRepository.Setup(service => service.GetFootnotes(releaseVersion.Id, subject.Id)).ReturnsAsync([]);
        ...
        public async Task FileImporting()
        {
            ...
            var releaseVersionService = BuildService(context, dataImportService: dataImportService.Object);
        ...
        public async Task FileLinkedToDraftReleaseAndPublishedPublicApiDataSet_ThrowsException()
        {
            ...
            // Verify the exception message
            Assert.Contains("A DRAFT release version's file cannot be linked to a PUBLISHED API.", exception.Message);
        }
        ...
        [InlineData(DataSetVersionStatus.Withdrawn, false)]
        [InlineData(DataSetVersionStatus.Deprecated, false)]
        public async Task ValidatesApiLinkedDeletionRules_BasedOnDataSetVersionStatus(
            DataSetVersionStatus dataSetVersionStatus,
        ...
        )
        {
            ...
            testFixture
                .DataSetVersionService.Setup(service =>
                    service.GetDataSetVersion(
                        testFixture.ReleaseFile.PublicApiDataSetId!.Value,
                        testFixture.ReleaseFile.PublicApiDataSetVersion!,
                        It.IsAny<CancellationToken>()
                    )
                )
                .ReturnsAsync(testFixture.TestDataSetVersion);
            var deleteDataBlockPlan = new DeleteDataBlockPlanViewModel();
            var footnote = new Footnote { Id = Guid.NewGuid() };
            testFixture
                .FootnoteRepository.Setup(service =>
                    service.GetFootnotes(testFixture.ReleaseVersion.Id, testFixture.Subject.Id)
                )
                .ReturnsAsync([footnote]);

            if (expectingApiDeletion)
            {
                testFixture.SetUpFootnotesAndDataBlocksMocks();
                testFixture.SetUpDeleteVersionMocks();
            }
            else
            {
                testFixture.SetUpDeleteDataBlockMocks(deleteDataBlockPlan);
            }
            ...
            var releaseVersionService = BuildService(
                ...
                dataSetVersionService: testFixture.DataSetVersionService.Object,
                dataImportService: testFixture.DataImportService.Object,
                dataBlockService: testFixture.DataBlockService.Object,
                footnoteRepository: testFixture.FootnoteRepository.Object,
            ...
            await using (var context = InMemoryApplicationDbContext(contextId))
            {
                ...
                Assert.All(
                    ...
                    expectedOrganisation =>
                        Assert.Contains(viewModel.PublishingOrganisations, ovm => expectedOrganisation.Id == ovm.Id)
                );
            }
    ...
    public class GetReleaseTests : ReleaseVersionServiceTests
    {
        ...
        public async Task Success()
        {
            ...
            var userPrereleaseRoleRepository = new Mock<IUserPrereleaseRoleRepository>(Strict);
            userPrereleaseRoleRepository.SetupQuery(ResourceRoleFilter.AllButExpired, nonPreReleaseUserRole);
    ...
    public class DeleteReleaseVersionTests : ReleaseVersionServiceTests
    {
        ...
        public async Task Success(bool isAmendment)
        {
            ...
            var methodologyScheduledWithRelease = new MethodologyVersion
            {
                ...
                Methodology = new Methodology { OwningPublicationTitle = "Methodology scheduled with this Release" },
            };
            ...
            userPrereleaseRoleRepository.SetupQuery(ResourceRoleFilter.All, [userReleaseRoles[0], userReleaseRoles[1]]);
            ...
            await using (var contentDbContext = InMemoryApplicationDbContext(contextId))
            {
                ...
                dataSetUploadRepository.Verify(mock => mock.DeleteAll(releaseVersion.Id, default), Times.Once);

                releaseSubjectRepository.Verify(
                    mock => mock.DeleteAllReleaseSubjects(releaseVersion.Id, !forceDeleteRelatedData),
                    Times.Once
                );
                ...
                else
                {
                    ...
                    var softDeletedRelease = await contentDbContext.ReleaseVersions.FirstOrDefaultAsync(rv =>
                    ...
                    var publicationWithoutDeletedRelease = await contentDbContext
                        ...
                        .AsNoTracking()
                        .FirstAsync(p => p.Id == releaseVersion.Release.PublicationId);

                    Assert.Single(publicationWithoutDeletedRelease.ReleaseVersions);
                    Assert.Equal(anotherReleaseVersion.Id, publicationWithoutDeletedRelease.ReleaseVersions[0].Id);
                    ...
                    Assert.Equal(anotherReleaseVersion.Id, publicationWithDeletedRelease.ReleaseVersions[1].Id);
                ...
                Assert.InRange(
                    DateTime.UtcNow.Subtract(retrievedMethodologyVersion.Updated!.Value).Milliseconds,
                    0,
                    1500
                );

                // Assert that Methodologies that were scheduled to go out with other Releases remain unaffected
                var unrelatedMethodology = await contentDbContext.MethodologyVersions.SingleAsync(m =>
                    m.Id == methodologyScheduledWithAnotherRelease.Id
                );
        ...
        public async Task ProcessorReturns400_Returns400()
        {
            ...
            await using (var context = InMemoryApplicationDbContext(contextId))
            {
                var releaseVersionService = BuildService(context, processorClient: processorClient.Object);
    ...
    {
        [Fact]
        public async Task WhenReleaseVersionIsPublished_UpdatesPublishedDisplayDate()
        {
            Publication publication = _dataFixture
                .DefaultPublication()
                .WithReleases(_ =>
                    [
                        ...
                        _dataFixture.DefaultRelease(publishedVersions: 2, year: 2025),
                    ]
                );
            ...
            // Capture the original published display dates for the versions that should NOT be altered
            var release2024Version0OriginalDisplayDate = release2024.Versions[0].PublishedDisplayDate;
            var release2025Version0OriginalDisplayDate = release2025.Versions[0].PublishedDisplayDate;

            var request = new ReleaseVersionPublishedDisplayDateUpdateRequest
            {
                PublishedDisplayDate = DateTimeOffset.Parse("2022-01-01T09:30:00.0000000+00:00"),
            };
            ...
            await using (var context = InMemoryApplicationDbContext(contextId))
            {
                ...
                var result = await service.UpdatePublishedDisplayDate(release2025.Versions[1].Id, request);
            ...
            await using (var context = InMemoryApplicationDbContext(contextId))
            {
                // Verify the targeted release version (release 2025, version 1) had its published display date updated
                var updatedReleaseVersion = await context.ReleaseVersions.SingleAsync(rv =>
                    rv.Id == release2025.Versions[1].Id
                );
                Assert.Equal(request.PublishedDisplayDate, updatedReleaseVersion.PublishedDisplayDate);

                // Verify the release 2024 (version 0) was NOT altered
                var release2024Version0 = await context.ReleaseVersions.SingleAsync(rv =>
                    rv.Id == release2024.Versions[0].Id
                );
                Assert.Equal(release2024Version0OriginalDisplayDate, release2024Version0.PublishedDisplayDate);

                // Verify release 2025 version 0 was NOT altered
                var release2025Version0 = await context.ReleaseVersions.SingleAsync(rv =>
                    rv.Id == release2025.Versions[0].Id
                );
                Assert.Equal(release2025Version0OriginalDisplayDate, release2025Version0.PublishedDisplayDate);
            }
        }

        [Fact]
        public async Task WhenReleaseVersionIsNotPublished_ReturnsBadRequest()
        {
            Publication publication = _dataFixture
                .DefaultPublication()
            ...
            await using (var context = InMemoryApplicationDbContext(contextId))
            {
                ...
                result.AssertBadRequest(ValidationErrorMessages.ReleaseVersionNotPublished);
            }
        }

        [Fact]
        public async Task WhenPublishedDisplayDateHasLocalOffset_ConvertsPublishedDisplayDateToUtc()
        {
            Publication publication = _dataFixture
                .DefaultPublication()
                .WithReleases(_ => [_dataFixture.DefaultRelease(publishedVersions: 1)]);
    ...
    public class ListUsersReleasesForApprovalTests : ReleaseVersionServiceTests
    {
        ...
        public async Task UserIsPublicationAndReleaseApprover_NoDuplication()
        {
            ...
            var userPrereleaseRoleRepository = new Mock<IUserPrereleaseRoleRepository>(Strict);
            userPrereleaseRoleRepository.SetupQuery(ResourceRoleFilter.ActiveOnly, [.. approverReleaseRolesForUser]);
