...
public class ReleaseDataFileServiceTests
{
    ...
    public async Task Delete()
    {
        ...
        dataImportService
            .Setup(mock => mock.DeleteImport(releaseDataFile.File.Id))
        ...
        releaseFileService
            .Setup(mock =>
                mock.CheckFileExists(releaseVersion.Id, releaseDataFile.File.Id, FileType.Data)
            )
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            dataImportService.Verify(
                mock => mock.DeleteImport(releaseDataFile.File.Id),
                Times.Once()
            );
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var dbReleaseFile1 = contentDbContext.ReleaseFiles.Single(rf =>
                rf.Id == releaseFile1.Id
            );
            Assert.Equal(0, dbReleaseFile1.Order);

            var dbReleaseFile3 = contentDbContext.ReleaseFiles.Single(rf =>
                rf.Id == releaseFile3.Id
            );
            Assert.Equal(2, dbReleaseFile3.Order);

            var dbReleaseFile4 = contentDbContext.ReleaseFiles.Single(rf =>
                rf.Id == releaseFile4.Id
            );
    ...
    public async Task Delete_DeleteReplacementFiles()
    {
        ...
        dataImportService
            .Setup(mock => mock.DeleteImport(replacementDataFile.Id))
            .Returns(Task.CompletedTask);
        ...
        privateBlobStorageService
            .Setup(mock =>
                mock.DeleteBlob(
                    PrivateReleaseFiles,
                    It.IsIn(
                        replacementDataFile.Path(),
                        replacementMetaFile.Path(),
                        replacementZipFile.Path()
                    )
                )
            )
            .Returns(Task.CompletedTask);
        ...
        releaseFileService
            .Setup(mock =>
                mock.CheckFileExists(releaseVersion.Id, replacementDataFile.Id, FileType.Data)
            )
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            dataImportService.Verify(
                mock => mock.DeleteImport(replacementDataFile.Id),
                Times.Once()
            );
        ...
        {
            Assert.Null(
                await contentDbContext.ReleaseFiles.FindAsync(replacementReleaseDataFile.Id)
            );
            Assert.Null(await contentDbContext.Files.FindAsync(replacementDataFile.Id));

            Assert.Null(
                await contentDbContext.ReleaseFiles.FindAsync(replacementReleaseMetaFile.Id)
            );
    ...
    public async Task DeleteAll()
    {
        ...
        dataImportService
            .Setup(mock => mock.DeleteImport(dataReleaseFile.File.Id))
            .Returns(Task.CompletedTask);
        ...
        releaseFileService
            .Setup(mock =>
                mock.CheckFileExists(releaseVersion.Id, dataReleaseFile.File.Id, FileType.Data)
            )
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            dataImportService.Verify(
                mock => mock.DeleteImport(dataReleaseFile.File.Id),
                Times.Once()
            );
    ...
    public async Task ReorderDataFiles()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            var dbDataFiles = contentDbContext
                ...
                .Where(rf =>
                    rf.ReleaseVersionId == releaseVersion.Id && rf.File.Type == FileType.Data
                )
                .ToList();
    ...
    public async Task ListAll()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                dataReleaseFile1.PublicApiDataSetVersionString,
                files[0].PublicApiDataSetVersion
            );
    ...
    public async Task ListAll_WithReplacement()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                originalReleaseFile.PublicApiDataSetVersionString,
                dataFileInfo.PublicApiDataSetVersion
            );
    ...
    public async Task SaveDataSetsFromTemporaryBlobStorage_Valid_ReturnsSuccess()
    {
        ...
        privateBlobStorageService.SetupCheckBlobExists(
