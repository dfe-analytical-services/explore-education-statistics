    ...
    public async Task CreateReleaseAmendment()
    {
        ...
        var releaseFiles = new List<ReleaseFile>
        {
            new()
            {
                ...
                File = dataFile1,
                FileId = dataFile1.Id,
                FilterSequence = [new FilterSequenceEntry(Guid.NewGuid(), [new(Guid.NewGuid(), [Guid.NewGuid()])])],
                IndicatorSequence = [new IndicatorGroupSequenceEntry(Guid.NewGuid(), [Guid.NewGuid()])],
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var originalFile = releaseFiles.First(f => f.File.Filename == amendmentDataFile.File.Filename);
            AssertAmendedReleaseFileCorrect(originalFile, amendmentDataFile, amendment);

            var amendmentDataFile2 = amendmentDataFiles[1];
            var originalFile2 = releaseFiles.First(f => f.File.Filename == amendmentDataFile2.File.Filename);
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(statisticsDbContextId))
        {
            ...
            // Release amendment.
            var statsReleaseVersionAmendment = statisticsDbContext.ReleaseVersion.SingleOrDefault(rv =>
                rv.Id == amendmentId
            );
