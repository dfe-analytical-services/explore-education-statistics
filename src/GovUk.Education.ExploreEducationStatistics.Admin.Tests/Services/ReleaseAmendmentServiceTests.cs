...
public class ReleaseAmendmentServiceTests
{
    ...
    public async Task CreateReleaseAmendment()
    {
        ...
        ReleaseVersion originalReleaseVersion = _fixture
            ...
            .WithPublishingOrganisations(_fixture.DefaultOrganisation().Generate(2))
            .WithReleaseStatuses(_fixture.DefaultReleaseStatus().Generate(1))
            .WithVersion(2)
                ...
                _fixture
                    ...
                    .ForIndex(
                        ...
                        s =>
                            s.SetContentBlocks(
                                ListOf<ContentBlock>(
                                    _fixture.DefaultHtmlBlock().WithBody("<div></div>")
                                )
                            )
                    )
                    .ForIndex(
            ...
            .WithDataBlockVersions(
                dataBlockParents.Select(dataBlockParent => dataBlockParent.LatestPublishedVersion!)
            )
            .WithKeyStatistics(
                ListOf<KeyStatistic>(
                    new KeyStatisticText { Title = "key stat text" },
        ...
        var releaseFiles = new List<ReleaseFile>
        {
            new()
            {
                ...
                IndicatorSequence =
                [
                    new IndicatorGroupSequenceEntry(Guid.NewGuid(), [Guid.NewGuid()]),
                ],
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(statisticsDbContextId))
        {
            ...
            // Method under test
            var result = await releaseAmendmentService.CreateReleaseAmendment(
                originalReleaseVersion.Id
            );
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            amendmentDataBlockVersions.ForEach(dataBlockVersion =>
            {
                var dataBlockVersionsForParent = contentDbContext
                    .DataBlockVersions.Where(version =>
                        version.DataBlockParentId == dataBlockVersion.DataBlockParentId
                    )
                    .ToList();
                ...
                Assert.NotEqual(
                    dataBlockVersion,
                    dataBlockVersion.DataBlockParent.LatestPublishedVersion
                );
            });

            var amendmentContentBlock1 = GetMatchingDataBlock(
                amendmentDataBlockVersions,
                dataBlock1Parent
            );
            var amendmentContentBlock2 = GetMatchingDataBlock(
                amendmentDataBlockVersions,
                dataBlock2Parent
            );
            var amendmentContentBlock3 = GetMatchingDataBlock(
                amendmentDataBlockVersions,
                dataBlock3Parent
            );
            ...
            Assert.Equal(
                dataBlock3Parent.LatestDraftVersion!.Name,
                amendmentKeyStatDataBlock.DataBlock.Name
            );
            Assert.NotEqual(
                originalReleaseVersion.KeyStatistics[1].Id,
                amendmentKeyStatDataBlock.Id
            );
            Assert.NotEqual(
                dataBlock3Parent.LatestDraftVersion.Id,
            ...
            var originalEmbedBlockLink = Assert.IsType<EmbedBlockLink>(
                originalReleaseVersion.Content[0].Content[2]
            );
            Assert.NotEqual(originalEmbedBlockLink.Id, amendmentEmbedBlockLink.Id);
            Assert.NotEqual(
                originalEmbedBlockLink.EmbedBlockId,
                amendmentEmbedBlockLink.EmbedBlockId
            );
            Assert.Equal(
                originalEmbedBlockLink.EmbedBlock.Title,
                amendmentEmbedBlockLink.EmbedBlock.Title
            );
            Assert.Equal(
                originalEmbedBlockLink.EmbedBlock.Url,
                amendmentEmbedBlockLink.EmbedBlock.Url
            );
            ...
            var approverAmendmentRole = amendmentReleaseRoles.Single(r =>
                r.Role == ReleaseRole.Approver
            );
            AssertAmendedReleaseRoleCorrect(approverReleaseRole, approverAmendmentRole, amendment);

            var contributorAmendmentRole = amendmentReleaseRoles.Single(r =>
                r.Role == ReleaseRole.Contributor
            );
            Assert.NotEqual(contributorReleaseRole.Id, contributorAmendmentRole.Id);
            AssertAmendedReleaseRoleCorrect(
                contributorReleaseRole,
                contributorAmendmentRole,
                amendment
            );
            ...
            amendment.FeaturedTables.ForEach(
                (amendedTable, index) =>
                {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(statisticsDbContextId))
        {
            ...
            // Release amendment.
            var statsReleaseVersionAmendment = statisticsDbContext.ReleaseVersion.SingleOrDefault(
                rv => rv.Id == amendmentId
            );
            Assert.NotNull(statsReleaseVersionAmendment);
            Assert.Equal(
                originalReleaseVersion.PublicationId,
                statsReleaseVersionAmendment.PublicationId
            );
    ...
    private static ReleaseVersion RetrieveAmendment(
    ...
    public async Task FiltersCommentsFromContent()
    {
            ...
            .WithContent(
                _fixture
                    ...
                    .ForIndex(
                        ...
                        s =>
                            s.SetContentBlocks(
                                _fixture.DefaultHtmlBlock().WithBody(htmlBlock3Body).Generate(1)
                            )
                    )
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            ...
            var amendmentContentBlock1 = Assert.IsType<HtmlBlock>(
                amendmentContentSection1.Content[0]
            );
            var amendmentContentBlock2 = Assert.IsType<HtmlBlock>(
                amendmentContentSection1.Content[1]
            );
            var amendmentContentBlock3 = Assert.IsType<HtmlBlock>(
                Assert.Single(amendmentContentSection2.Content)
            );
    ...
    public async Task CopyFootnotes()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contextId))
        {
            ...
            contentDbContext.Users.AddRange(_fixture.DefaultUser().WithId(_userId));
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(contextId))
        {
            ...
            // Method under test
            var result = await releaseAmendmentService.CreateReleaseAmendment(
                originalStatsReleaseVersion.Id
            );
            var viewModel = result.AssertRight();
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(contextId))
        {
            var newFootnotesFromDb = statisticsDbContext
                ...
                .Where(f =>
                    f.Releases.FirstOrDefault(r => r.ReleaseVersionId == amendmentId) != null
                )
                .OrderBy(f => f.Content)
                .ToList();

            Assert.Equal(2, newFootnotesFromDb.Count);
            AssertFootnoteDetailsCopiedCorrectly(
                releaseFootnotes[0].Footnote,
                newFootnotesFromDb[0]
            );
            AssertFootnoteDetailsCopiedCorrectly(
                releaseFootnotes[1].Footnote,
                newFootnotesFromDb[1]
            );
        }
    }

    private void AssertFootnoteDetailsCopiedCorrectly(
        Footnote originalFootnote,
    ...
    )
    {
        ...
        Assert.Equal(
            originalFootnote.Filters.SelectNullSafe(f => f.FilterId),
            newFootnote.Filters.SelectNullSafe(f => f.FilterId)
        );

        Assert.Equal(
            originalFootnote.FilterGroups.SelectNullSafe(f => f.FilterGroupId),
            newFootnote.FilterGroups.SelectNullSafe(f => f.FilterGroupId)
        );

        Assert.Equal(
            originalFootnote.FilterItems.SelectNullSafe(f => f.FilterItemId),
            newFootnote.FilterItems.SelectNullSafe(f => f.FilterItemId)
        );

        Assert.Equal(
            originalFootnote.Subjects.SelectNullSafe(f => f.SubjectId),
            newFootnote.Subjects.SelectNullSafe(f => f.SubjectId)
        );

        Assert.Equal(
            originalFootnote.Indicators.SelectNullSafe(f => f.IndicatorId),
            newFootnote.Indicators.SelectNullSafe(f => f.IndicatorId)
        );
    }
    ...
    private void AssertAmendedUpdateCorrect(
        Update amendedUpdate,
    ...
    )
    {
        ...
        Assert.Equal(
            originalFile.Published!.Value,
            amendmentDataFile.Published!.Value,
        ...
        Assert.Equal(
            ...
            amendmentDataFile.PublicApiDataSetVersion
        );
