...
public class UserRoleServiceTests
{
    ...
    public async Task SetGlobalRole()
    {
        ...
        userManager.Setup(mock => mock.AddToRoleAsync(ItIsUser(user), role.Name)).ReturnsAsync(new IdentityResult());
    ...
    public async Task SetGlobalRole_AlreadyHasAnotherRole()
    {
        ...
        userManager.Setup(mock => mock.AddToRoleAsync(ItIsUser(user), newRole.Name)).ReturnsAsync(new IdentityResult());
    ...
    public async Task AddPublicationRole()
    {
        ...
        VerifyAllMocks(emailTemplateService, userManager, userPublicationRoleRepository);
    }
    ...
    public async Task AddPublicationRole_HasNoRoles_UpgradesRole()
    {
        ...
        userManager.Setup(s => s.GetRolesAsync(ItIsUser(user))).ReturnsAsync(new List<string>()).Verifiable();
    ...
    public async Task AddPublicationRole_HasHigherGlobalRoleThanAnalyst_DoesNotUpgradeRole()
    {
        ...
        userManager.Setup(s => s.GetRolesAsync(ItIsUser(user))).ReturnsAsync(ListOf(RoleNames.BauUser)).Verifiable();
        ...
        VerifyAllMocks(userManager, emailTemplateService);
    ...
    public async Task AddReleaseRole()
    {
        ...
        await using (var userAndRolesDbContext = InMemoryUserAndRolesDbContext(usersAndRolesDbContextId))
        {
            ...
            var result = await service.AddReleaseRole(userId: _user.Id, releaseId: release.Id, ReleaseRole.Contributor);

            result.AssertRight();
        }

        VerifyAllMocks(emailTemplateService, userManager, userReleaseRoleRepository);
    }
    ...
    public async Task AddReleaseRole_HasNoRoles_UpgradesRole()
    {
        ...
        userManager.Setup(s => s.GetRolesAsync(ItIsUser(user))).ReturnsAsync([]).Verifiable();
    ...
    {
        User user1 = _dataFixture.DefaultUser().WithFirstName("User").WithLastName("1").WithEmail("user1@example.com");

        User user2 = _dataFixture.DefaultUser().WithFirstName("User").WithLastName("2").WithEmail("user2@example.com");
    ...
    public async Task RemoveUserPublicationRole()
    {
        ...
        userPublicationRoleRepository
            ...
            .Verifiable();
        userPublicationRoleRepository.Setup(m => m.GetDistinctRolesByUser(user.Id)).ReturnsAsync([]).Verifiable();

        var userReleaseRoleRepository = new Mock<IUserReleaseRoleRepository>(Strict);
        userReleaseRoleRepository.Setup(m => m.GetDistinctRolesByUser(user.Id)).ReturnsAsync([]).Verifiable();
    ...
    public async Task RemoveUserPublicationRole_AnalystRoleStillRequiredForOtherPublications()
    {
        ...
        await using (var contentDbContext = InMemoryApplicationDbContext(contentDbContextId))
        {
            await contentDbContext.UserPublicationRoles.AddRangeAsync(userPublicationRole, anotherUserPublicationRole);
            await contentDbContext.SaveChangesAsync();
        }
    ...
    public async Task RemoveUserReleaseRole_AnalystRoleStillRequiredForOtherPublications()
    {
        ...
        var identityUserRole = new IdentityUserRole<string> { UserId = identityUser.Id, RoleId = analystGlobalRole.Id };
    ...
    public async Task RemoveAllUserResourceRoles()
    {
        ...
        userManager.Setup(s => s.GetRolesAsync(ItIsUser(targetIdentityUser))).ReturnsAsync(ListOf(RoleNames.Analyst));
        userManager
            .Setup(s =>
