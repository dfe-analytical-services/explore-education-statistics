...
public class ReleaseApprovalServiceTests
{
    ...
    public async Task CreateReleaseStatus()
    {
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            Assert.Equal(
                new DateTime(2051, 6, 29, 23, 0, 0, DateTimeKind.Utc),
    ...
    public async Task CreateReleaseStatus_Approved_Scheduled()
    {
        ...
        preReleaseUserService
            .Setup(mock =>
                mock.SendPreReleaseInviteEmail(releaseVersion.Id, existingUser1Invite.Email, false)
            )
            .ReturnsAsync(Unit.Instance);

        preReleaseUserService
            .Setup(mock =>
                mock.SendPreReleaseInviteEmail(
                    releaseVersion.Id,
    ...
    public async Task CreateReleaseStatus_Approved_Amendment()
    {
        ...
        var amendedReleaseVersion = publication
            .Releases[0]
            .Versions.Single(rv => rv is { Published: null });
        ...
        publishingService
            .Setup(s =>
                s.ReleaseChanged(
                    It.Is<ReleasePublishingKey>(key =>
                        key.ReleaseVersionId == amendedReleaseVersion.Id
                    ),
                    false,
                    CancellationToken.None
                )
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            var saved = await context.ReleaseVersions.SingleAsync(rv =>
                rv.Id == amendedReleaseVersion.Id
            );
    ...
    public async Task CreateReleaseStatus_ReleaseHasImages()
    {
        ...
        userReleaseRoleService
            .Setup(mock =>
                mock.ListUserReleaseRolesByPublication(
    ...
    public async Task CreateReleaseStatus_Approved_Scheduled_FailsSendingPreReleaseInviteEmail()
    {
        ...
        preReleaseUserService
            .Setup(mock =>
                mock.MarkInviteEmailAsSent(It.Is<UserReleaseInvite>(i => i.Email == invite2.Email))
            )
            .Returns(Task.CompletedTask);
        ...
        await using (var context = InMemoryApplicationDbContext(contextId))
        {
            ...
            saved.AssertDeepEqualTo(
                releaseVersion,
                ignoreProperties: [rv => rv.Release, rv => rv.Publication]
            );
    ...
    public async Task CreateReleaseStatus_Approved_Scheduled_FailsWhileInformingPublisher()
    {
        ...
        preReleaseUserService
            .Setup(mock =>
                mock.MarkInviteEmailAsSent(It.Is<UserReleaseInvite>(i => i.Email == invite.Email))
            )
            .Returns(Task.CompletedTask);
    ...
    public class ListReleaseStatuses : ReleaseApprovalServiceTests
    {
                    ...
                    r =>
                        r.SetVersions(
                            _fixture
                                .DefaultReleaseVersion()
                                .WithReleaseStatuses(
                                    _fixture.DefaultReleaseStatus().WithCreatedBy(User).Generate(1)
                                )
                                .Generate(1)
                        )
                )
                ...
                Assert.All(
                    ...
                    tuple =>
                    {
                        ...
                        Assert.Multiple(
                            ...
                            () =>
                                Assert.Equal(
                                    releaseStatus.InternalReleaseNote,
                                    viewModel.InternalReleaseNote
                                ),
                            () =>
                                Assert.Equal(
                                    releaseStatus.ApprovalStatus,
                                    viewModel.ApprovalStatus
                                ),
                            () => Assert.Equal(releaseStatus.Created, viewModel.Created),
                            () =>
                                Assert.Equal(
                                    releaseStatus.CreatedBy!.Email,
                                    viewModel.CreatedByEmail
                                ),
                            () =>
                                Assert.Equal(
                                    releaseStatus.ReleaseVersion.Version,
                                    viewModel.ReleaseVersion
                                )
                        );
                    }
                );
            ...
            Release release = _fixture
                ...
                .WithVersions(
                    _fixture
                        .DefaultReleaseVersion()
                        .WithReleaseStatuses(
                            _fixture.DefaultReleaseStatus().WithCreatedBy().Generate(1)
                        )
                        .Generate(1)
                );
        ...
        public async Task NoReleaseStatuses_ReturnsEmpty()
        {
            Release release = _fixture
                .DefaultRelease()
                .WithVersions(_fixture.DefaultReleaseVersion().Generate(1));
