...
public class TableBuilderService : ITableBuilderService
{
                    ...
                    .OnSuccess(subjectMetaViewModel =>
                    {
                        return new TableBuilderResultViewModel
                        {
                            ...
                            Results = observations.Select(observation =>
                                ObservationViewModelBuilder.BuildObservation(
                                    observation,
                                    query.Indicators
                                )
                            ),
    ...
    {
        return await FindLatestPublishedReleaseVersionId(query.SubjectId)
            .OnSuccessVoid(releaseVersionId =>
                QueryToCsvStream(releaseVersionId, query, stream, cancellationToken)
            );
    }
            ...
            .OnSuccess(releaseSubject =>
                ListQueryObservations(query, cancellationToken)
                    .OnSuccessCombineWith(observations =>
                        _subjectCsvMetaService.GetSubjectCsvMeta(
                            releaseSubject,
                            query,
                            observations,
                            cancellationToken
    ...
    )
    {
        ...
        {
            var match = observation.FilterItems.FirstOrDefault(fi =>
                filter.Items.ContainsKey(fi.FilterItemId)
            );
