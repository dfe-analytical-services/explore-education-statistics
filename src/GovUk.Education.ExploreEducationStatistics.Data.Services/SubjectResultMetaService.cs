...
public class SubjectResultMetaService : ISubjectResultMetaService
{
    ...
    {
        return await CheckReleaseSubjectExists(
            ...
            .OnSuccess(async releaseSubject =>
            {
                ...
                _logger.LogTrace(
                    "Got Location attributes in {Time} ms",
                    stopwatch.Elapsed.TotalMilliseconds
                );
                stopwatch.Restart();

                var releaseFile = await _releaseDataFileRepository.GetBySubject(
                    releaseVersionId: releaseVersionId,
                    subjectId: releaseSubject.SubjectId
                );

                var filterItems = await _filterItemRepository.GetFilterItemsFromObservations(
                    observations
                );
                ...
                var publicationId = await _subjectRepository.FindPublicationIdForSubject(
                    releaseSubject.SubjectId
                );
                var publicationTitle = (
                    await _contentDbContext.Publications.FindAsync(publicationId)
                )!.Title;

                var locationViewModels = await _locationService.GetLocationViewModels(
                    locations,
                    _locationOptions.Hierarchies
                );
                _logger.LogTrace(
                    "Got Location view models in {Time} ms",
                    stopwatch.Elapsed.TotalMilliseconds
                );
                stopwatch.Stop();
    ...
    private Task<Either<ActionResult, ReleaseSubject>> CheckReleaseSubjectExists(
        Guid releaseVersionId,
        Guid subjectId
    )
    {
        return _persistenceHelper.CheckEntityExists<ReleaseSubject>(query =>
            query
    ...
    private List<BoundaryLevelViewModel> GetBoundaryLevelViewModels(
    ...
    {
        var footnotes = await _footnoteRepository.GetFilteredFootnotes(
            releaseVersionId: releaseVersionId,
            subjectId: subjectId,
