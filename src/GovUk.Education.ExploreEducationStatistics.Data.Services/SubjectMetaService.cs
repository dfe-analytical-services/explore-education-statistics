...
) : ISubjectMetaService
{
    ...
    public async Task<Either<ActionResult, SubjectMetaViewModel>> GetSubjectMeta(Guid releaseVersionId, Guid subjectId)
    {
    ...
    )
    {
        return await contentDbContext
            ...
            .OnSuccessVoid(async releaseFile =>
            {
                ...
                await InvalidateCachedReleaseSubjectMetadata(releaseVersionId: releaseVersionId, subjectId: subjectId);
            });
    }
    ...
    private async Task<Dictionary<string, LocationsMetaViewModel>> GetLocations(Guid subjectId)
    {
        ...
        return locationViewModels.ToDictionary(
            ...
            pair => new LocationsMetaViewModel { Legend = pair.Key.GetEnumLabel(), Options = pair.Value }
        );
    }

    private static List<List<DataSetFileFilterHierarchyTierViewModel>>? BuildFilterHierarchyViewModel(
        List<DataSetFileFilterHierarchy> filterHierarchies
    )
    {
    ...
    )
    {
        ...
        return AssertCollectionsAreSameIgnoringOrder(
            ...
            .OnSuccess(_ =>
            {
                ...
                return filters
                    .Select(filter => ValidateFilterGroupsForSubject(filter, requestMap[filter.Id].FilterGroups))
