...
AND (@localEnterprisePartnershipCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @localEnterprisePartnershipsList))
AND (@institutionCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @institutionsList))
AND (@matCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @matsList))
AND (@mayoralCombinedAuthorityCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @mayoralCombinedAuthoritiesList))
AND (@opportunityAreaCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @opportunityAreasList))
AND (@parliamentaryConstituencyCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @parliamentaryConstituenciesList))
AND (@providerCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @providersList))
AND (@wardCount = 0 OR l.LocalAuthorityDistrict_Code IN (SELECT id from @wardsList))
GROUP BY o.Id
