...
) : ControllerBase
{
    ...
    )
    {
        var actionResult = await GetLatestPublishedDataBlockVersion(dataBlockParentId)
            .OnSuccessDo(dataBlockVersion =>
                this.CacheWithLastModifiedAndETag(
                    lastModified: dataBlockVersion.Published,
                    ApiVersion
                )
            )
    ...
    [HttpGet(
        "tablebuilder/release/{releaseVersionId:guid}/data-block/{dataBlockParentId:guid}/geojson"
    )]
    public async Task<
    ...
    > GetLocations(DataBlockVersion dataBlockVersion, long boundaryLevelId)
    {
    ...
    )
    {
        return await contextDbContext
            ...
            .ThenInclude(rv => rv.Release)
            .ThenInclude(r => r.Publication)
            .SingleOrNotFoundAsync(p =>
                p.Id == dataBlockVersion.ReleaseVersion.Release.PublicationId
            )
