    ...
    : NotifierFunctionsIntegrationTest(fixture)
{
    ...
    public async Task RequestPendingSubscription_ReturnsValidationProblem_When_Id_Is_Blank()
    {
        ...
        validationProblem.AssertHasNotEmptyError(
            nameof(PendingPublicationSubscriptionCreateRequest.Id).ToLowerFirst()
        );
    }
    ...
    public async Task SendsSubscriptionConfirmationEmail()
    {
        ...
        var tokenService = new Mock<ITokenService>(MockBehavior.Strict);
        tokenService
            .Setup(mock => mock.GetEmailFromToken("verification-code-4"))
    ...
    public async Task Unsubscribes()
    {
        ...
        await using (
            var contentDbContext = ContentDbUtils.InMemoryContentDbContext(contentDbContextId)
        )
        {
        ...
        var tokenService = new Mock<ITokenService>(MockBehavior.Strict);
        tokenService
            .Setup(mock => mock.GetEmailFromToken("unsubscription-code-5"))
