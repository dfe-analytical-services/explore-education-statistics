    ...
    : NotifierFunctionsIntegrationTest(fixture)
{
    ...
    public async Task SendsSubscriptionVerificationEmail()
    {
        ...
        emailService.Setup(mock =>
            mock.SendEmail("test1@test.com", "subscription-verification-id", It.IsAny<Dictionary<string, dynamic>>())
        );
        ...
        // Act
        var result = await notifierFunction.RequestPendingSubscription(
            request,
    ...
    public async Task DoesNotSendEmailAgainIfSubIsPending()
    {
        ...
        var emailService = new Mock<IEmailService>(MockBehavior.Strict);
        emailService.Setup(mock =>
            mock.SendEmail("test2@test.com", "subscription-verification-id", It.IsAny<Dictionary<string, dynamic>>())
        );
    ...
    public async Task SendsConfirmationEmailIfUserAlreadySubscribed()
    {
        ...
        var emailService = new Mock<IEmailService>(MockBehavior.Strict);
        emailService.Setup(mock =>
            mock.SendEmail("test3@test.com", "subscription-confirmation-id", It.IsAny<Dictionary<string, dynamic>>())
        );
    ...
    public async Task RequestPendingSubscription_ReturnsValidationProblem_When_Id_Is_Blank()
    {
        ...
        var emailService = new Mock<IEmailService>(MockBehavior.Strict);
        emailService.Setup(mock =>
            mock.SendEmail("test@test.com", "subscription-verification-id", It.IsAny<Dictionary<string, dynamic>>())
        );
    ...
    public async Task RequestPendingSubscription_ReturnsValidationProblem_When_Title_Is_Blank()
    {
        ...
        validationProblem.AssertHasNotEmptyError(
            nameof(PendingPublicationSubscriptionCreateRequest.Title).ToLowerFirst()
        );
    }
    ...
    public async Task RequestPendingSubscription_ReturnsValidationProblem_When_Email_Is_Blank()
    {
        ...
        var validationProblem = result.AssertBadRequestWithValidationProblem();
        validationProblem.AssertHasNotEmptyError(
            nameof(PendingPublicationSubscriptionCreateRequest.Email).ToLowerFirst()
        );
    }
    ...
    public async Task RequestPendingSubscription_ReturnsValidationProblem_When_Slug_Is_Blank()
    {
        ...
        var validationProblem = result.AssertBadRequestWithValidationProblem();
        validationProblem.AssertHasNotEmptyError(
            nameof(PendingPublicationSubscriptionCreateRequest.Slug).ToLowerFirst()
        );
    }
    ...
    public async Task SendsSubscriptionConfirmationEmail()
    {
        ...
        // Arrange (mocks)
        var tokenService = new Mock<ITokenService>(MockBehavior.Strict);
        tokenService.Setup(mock => mock.GetEmailFromToken("verification-code-4")).Returns("test4@test.com");
        tokenService
            .Setup(mock => mock.GenerateToken("test4@test.com", It.IsAny<DateTime>()))
            .Returns("unsubscription-code-4");

        var emailService = new Mock<IEmailService>(MockBehavior.Strict);
        emailService.Setup(mock =>
            mock.SendEmail("test4@test.com", "subscription-confirmation-id", It.IsAny<Dictionary<string, dynamic>>())
        );
    ...
    public async Task Unsubscribes()
    {
        ...
        {
            contentDbContext.Publications.Add(
                new Publication { Id = supersededPublicationId, SupersededById = publicationId }
            );
            await contentDbContext.SaveChangesAsync();
        }

        var tokenService = new Mock<ITokenService>(MockBehavior.Strict);
        tokenService.Setup(mock => mock.GetEmailFromToken("unsubscription-code-5")).Returns("test5@test.com");
