...
public abstract class DataSetQueryParserTests
{
    ...
    public class ParseCriteriaFiltersTests : DataSetQueryParserTests
    {
        ...
        {
            var filterOptions = _dataFixture.DefaultParquetFilterOption().WithFilterColumn("field_a").GenerateList(1);
        ...
        {
            var filterOptions = _dataFixture.DefaultParquetFilterOption().WithFilterColumn("field_a").GenerateList(3);
    ...
    public class ParseCriteriaGeographicLevelsTests : DataSetQueryParserTests
    {
        ...
        public ParseCriteriaGeographicLevelsTests()
        {
            _dataSetVersion = _dataFixture
                ...
                .WithMetaSummary(_dataFixture.DefaultDataSetVersionMetaSummary().WithGeographicLevels([]));
    ...
    public class ParseCriteriaLocationsTests : DataSetQueryParserTests
    {
        ...
        public async Task AllComparators_SingleOptionNotFound(string comparator, string expectedSql)
        {
            ...
            [
                new DataSetQueryLocationId { Id = "Invalid ID", Level = GeographicLevel.Country.GetEnumValue() },
            ];
    ...
    public class ParseCriteriaTimePeriodsTests : DataSetQueryParserTests
    {
        ...
        public async Task AllComparators_SingleTimePeriodNotFound(string comparator, string expectedSql)
        {
            ...
            [
                new DataSetQueryTimePeriod { Code = TimeIdentifier.AcademicYear.GetEnumValue(), Period = "2023/2024" },
            ];

            _timePeriodRepository.Setup(r => r.List(_dataSetVersion, queryTimePeriods, default)).ReturnsAsync([]);
        ...
        public async Task InComparators_MultipleTimePeriodsNoneFound(string comparator, string expectedSql)
        {
            List<DataSetQueryTimePeriod> queryTimePeriods =
            [
                new DataSetQueryTimePeriod { Code = TimeIdentifier.AcademicYear.GetEnumValue(), Period = "2020/2021" },
                new DataSetQueryTimePeriod { Code = TimeIdentifier.CalendarYear.GetEnumValue(), Period = "2024" },
            ];
    ...
    public class ParseCriteriaMixedFacetsTests : DataSetQueryParserTests
    {
        ...
        public async Task AllFacetsWithMixedComparatorsAndSingleOption()
        {
            ...
            var locationOptions = _dataFixture.DefaultParquetLocationOption(GeographicLevel.Region).GenerateList(1);
    ...
    public class ParseCriteriaPathTests : DataSetQueryParserTests
    {
        ...
        public ParseCriteriaPathTests()
        {
            ...
            _filterRepository
                .Setup(r => r.ListOptions(_dataSetVersion, It.IsAny<IEnumerable<string>>(), default))
                .ReturnsAsync([]);

            _locationRepository
                .Setup(r => r.ListOptions(_dataSetVersion, It.IsAny<IEnumerable<IDataSetQueryLocation>>(), default))
                .ReturnsAsync([]);

            _timePeriodRepository
                .Setup(r => r.List(_dataSetVersion, It.IsAny<IEnumerable<DataSetQueryTimePeriod>>(), default))
                .ReturnsAsync([]);
        }
    ...
    private static IDataSetQueryLocation MapOptionToQueryLocation(ParquetLocationOption option)
    {
        ...
        return level switch
        {
            ...
            {
                { Code: not null } => new DataSetQueryLocationLocalAuthorityCode { Code = option.Code },
                { OldCode: not null } => new DataSetQueryLocationLocalAuthorityOldCode { OldCode = option.OldCode },
            ...
            {
                { Ukprn: not null } => new DataSetQueryLocationProviderUkprn { Ukprn = option.Ukprn! },
                _ => throw new NullReferenceException($"{nameof(option.Ukprn)} cannot both be null"),
            },
            GeographicLevel.RscRegion => new DataSetQueryLocationId { Level = option.Level, Id = option.PublicId },
            GeographicLevel.School => option switch
            {
                { Urn: not null } => new DataSetQueryLocationSchoolUrn { Urn = option.Urn },
                { LaEstab: not null } => new DataSetQueryLocationSchoolLaEstab { LaEstab = option.LaEstab },
                _ => throw new NullReferenceException(
            ...
            _ => option switch
            {
                { Code: not null } => new DataSetQueryLocationCode { Level = option.Level, Code = option.Code },
