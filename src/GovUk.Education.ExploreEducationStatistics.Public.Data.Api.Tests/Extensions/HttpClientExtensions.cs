...
{
    public static HttpClient WithPreviewTokenHeader(this HttpClient client, Guid? previewToken)
    {
        return client.WithOptionalHeader(RequestHeaderNames.PreviewToken, previewToken?.ToString());
    }

    public static HttpClient WithRequestSourceHeader(this HttpClient client, string? requestSource)
    {
