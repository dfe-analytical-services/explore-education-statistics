...
public abstract class DataSetsControllerPostQueryTests(TestApplicationFactory testApp) : IntegrationTestFixture(testApp)
{
    ...
    public class GeographicLevelsValidationTests(TestApplicationFactory testApp) : DataSetsControllerPostQueryTests(testApp)
    {
        [Theory]
 ...
 [Fact]
    ...
    public class ResultsTests(TestApplicationFactory testApp) : DataSetsControllerPostQueryTests(testApp)
    {
        [Fact]
        ...
        public async Task AllFacets_MixOfConditions_Returns200()
        {
            ...
            Assert.Equal(
                "pTSoj :: England (code = E92000001)",
                result.Locations[GeographicLevel.Country.GetEnumValue()]
            );
            Assert.Equal(
                "IzBzg :: Yorkshire and The Humber (code = E12000003)",
                result.Locations[GeographicLevel.Region.GetEnumValue()]
            );
            Assert.Equal(
                "7zXob :: Sheffield (code = E08000019, oldCode = 373)",
                result.Locations[GeographicLevel.LocalAuthority.GetEnumValue()]
            );
            Assert.Equal(
                "HTzLj :: Newfield Secondary School (urn = 140821, laEstab = 3734008)",
                result.Locations[GeographicLevel.School.GetEnumValue()]
            );
