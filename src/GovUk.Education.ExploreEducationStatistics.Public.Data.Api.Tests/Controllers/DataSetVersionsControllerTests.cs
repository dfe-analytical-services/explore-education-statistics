...
public abstract class DataSetVersionsControllerTests(TestApplicationFactory testApp) : IntegrationTestFixture(testApp)
{
    ...
    public class GetDataSetVersionUsingWildcardTests(TestApplicationFactory testApp) : DataSetVersionsControllerTests(testApp)
    {
        [Theory]
        //[InlineData("meta", new[] { "types:Filters,Locations" }, "3.*", false)]
        [InlineData("query", new[] { "indicators:O7CLF" }, "3.*", true)]
        //[InlineData("csv", new[] {""}, "3.*", true)]
        public async Task WildCardVersionsForGetEndpoints_ReturnExpectedServerCodes_MetaQueryCsv(
        string endPoint, string[] queryKeyValues, string dataSetVersion, bool expectedMissingDataFileServerError)
        {
            DataSet dataSet = DataFixture
                    .DefaultDataSet()
                    .WithStatusPublished();
            ...
            var ind = DataFixture.DefaultIndicatorMeta().GenerateList(1);

            var dataSetVersions = DataFixture
                .DefaultDataSetVersion(filters: 1, indicators: 1, locations: 1, timePeriods: 3)
            .WithStatusPublished()
                //.WithIndicatorMetas(() => DataFixture.DefaultIndicatorMeta().GenerateList(1))
                .WithDataSetId(dataSet.Id).WithPreviewTokens(() => [DataFixture.DefaultPreviewToken()])
                .ForIndex(0, dsv => dsv.SetVersionNumber(1, 0))
                .ForIndex(1, dsv => dsv.SetVersionNumber(3, 0))
                .ForIndex(2, dsv => dsv.SetVersionNumber(3, 1))
                .ForIndex(3, dsv => { dsv.SetVersionNumber(3, 2); dsv.Set(a => a.IndicatorMetas, b => ind); })
                .ForIndex(4, dsv => dsv.SetVersionNumber(2, 0))
                .ForIndex(5, dsv => dsv.SetVersionNumber(2, 1))
                .GenerateList();
            ...
            client.AddPreviewTokenHeader(dataSetVersions[0].PreviewTokens[0].Id);


            // Convert the string[] queryKeyValues into a Dictionary<string, string>
            var queryKeys = new Dictionary<string, string>();
            if (!queryKeyValues.FirstOrDefault().IsNullOrEmpty())
                queryKeys = queryKeyValues
                    .Select(kv => kv.Split(':'))
                    .ToDictionary(kv => kv[0], kv => kv[1]);

            queryKeys.Remove("indicators");
            queryKeys.Add("indicators", ind.FirstOrDefault().PublicId); 
            queryKeys.Add("dataSetId", dataSetId.ToString()); 
            queryKeys.Add("dataSetVersion", dataSetVersion);

            var uri = QueryHelpers.AddQueryString($"{BaseUrl}/{dataSetId}/{endPoint}", queryKeys);

            var response = await client.GetAsync(uri);
            // if it returns 500 or 200, it has found the data set version (but may have failed because our tests are missing data set version files or csvs
            // If it returns 404, it has not found the dataset version
            if (expectedMissingDataFileServerError)
