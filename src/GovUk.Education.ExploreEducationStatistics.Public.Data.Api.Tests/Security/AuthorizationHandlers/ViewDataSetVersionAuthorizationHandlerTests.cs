...
public class ViewDataSetVersionAuthorizationHandlerTests
{
    ...
    public async Task Success_PreviewTokenIsActive(DataSetVersionStatus status)
    {
        ...
        PreviewToken previewToken = _dataFixture.DefaultPreviewToken().WithDataSetVersion(dataSetVersion);
    ...
    public async Task Failure_PreviewTokenIsExpiredOrNotActiveYet(bool activated, bool expired)
    {
        DataSetVersion dataSetVersion = _dataFixture.DefaultDataSetVersion().WithStatus(DataSetVersionStatus.Draft);

        PreviewToken previewToken = _dataFixture
            .DefaultPreviewToken(activated: activated, expired: expired)
    ...
    public async Task Failure_PreviewTokenIsForWrongDataSetVersion()
    {
        ...
        PreviewToken previewToken = _dataFixture.DefaultPreviewToken().WithDataSetVersion(dataSetVersion2);
    ...
    public async Task Failure_IncorrectUserAgentHeaderInDevelopment(DataSetVersionStatus status)
    {
        ...
        var handler = BuildHandler(environmentName: Environments.Development, userAgentValue: "Incorrect User Agent");
    ...
    public async Task Success_ClaimsPrincipalWithRole(DataSetVersionStatus status)
    {
        ...
        var handler = BuildHandler(environmentName: Environments.Production, claimsPrincipal: userWithCorrectRole);
    ...
    public async Task Failure_ClaimsPrincipalWithIncorrectRole(DataSetVersionStatus status)
    {
        ...
        var handler = BuildHandler(environmentName: Environments.Production, claimsPrincipal: userWithIncorrectRole);
    ...
    )
    {
        ...
        {
            HttpContext = new DefaultHttpContext { HttpContext = { User = claimsPrincipal ?? new ClaimsPrincipal() } },
        };
        ...
        environment.SetupGet(s => s.EnvironmentName).Returns(environmentName ?? Environments.Production);
