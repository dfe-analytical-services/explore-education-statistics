...
public abstract class AnalyticsWriteDataSetCallsStrategyTests
{
    public class ReportTests : AnalyticsWriteDataSetCallsStrategyTests
    {
        private const string SnapshotPrefix =
            $"{nameof(AnalyticsWriteDataSetCallsStrategyTests)}.{nameof(ReportTests)}";
        ...
        public async Task CallWrittenSuccessfully_WithCoreDataSetDetails()
        {
            ...
            var files = Directory.GetFiles(
                pathResolver.BuildOutputDirectory(AnalyticsWriteDataSetCallsStrategy.OutputSubPaths)
            );

            var filePath = Assert.Single(files);

            var filename = filePath.Split(
                $"{pathResolver.BuildOutputDirectory(AnalyticsWriteDataSetCallsStrategy.OutputSubPaths)}{Path.DirectorySeparatorChar}"
            )[1];
        ...
        )
        {
            return new AnalyticsWriteDataSetCallsStrategy(
                ...
                new CommonAnalyticsWriteStrategyWorkflow<CaptureDataSetCallRequest>(
                    ...
                    Mock.Of<
                        ILogger<CommonAnalyticsWriteStrategyWorkflow<CaptureDataSetCallRequest>>
                    >()
                )
            );
        }
