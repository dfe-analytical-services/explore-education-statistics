...
public class CommonAnalyticsWriteStrategyWorkflowMockBuilder<TAnalyticsCaptureRequest>
    where TAnalyticsCaptureRequest : IAnalyticsCaptureRequest
{
    private readonly Mock<ICommonAnalyticsWriteStrategyWorkflow<TAnalyticsCaptureRequest>> _mock =
        ...
        _mock
            .Setup(m =>
                m.Report(
                    ...
                    It.IsAny<TAnalyticsCaptureRequest>(),
    ...
    public class Asserter(
        CommonAnalyticsWriteStrategyWorkflowMockBuilder<TAnalyticsCaptureRequest> parent
    )
    {
        public void ReportCalled(Func<TAnalyticsCaptureRequest, bool>? requestPredicate = null)
        {
            parent._mock.Verify(
                m =>
                    m.Report(
                        It.IsAny<IWorkflowActor<TAnalyticsCaptureRequest>>(),
                        It.Is<TAnalyticsCaptureRequest>(actual =>
                            requestPredicate == null || requestPredicate(actual)
                        ),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once
        ...
        /// </example>
        /// </summary>
        public void WorkflowActor(Action<IWorkflowActor<TAnalyticsCaptureRequest>> assertion) =>
            assertion(Xunit.Assert.Single(parent._reportCalls).WorkflowActor);
    }
}
