...
public class SubjectMetaServiceTests
{
    ...
    public async Task FilterSubjectMeta_EmptyModelReturned()
    {
        ...
        await using (var statisticsDbContext = InMemoryStatisticsDbContext(statisticsDbContextId))
        {
            ...
            var result = (
                await service.FilterSubjectMeta(
    ...
    public async Task FilterSubjectMeta_LatestRelease_EmptyModelReturned()
    {
        ...
        await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
        {
            ...
            var result = (
                await service.FilterSubjectMeta(null, request, cancellationToken)
            ).AssertRight();
            ...
            filterItemRepository
                .Setup(s =>
                    s.GetFilterItemsFromMatchedObservationIds(
                        ...
                        statisticsDbContext.MatchedObservations
                    )
                )
    ...
    public async Task UpdateSubjectFilters()
    {
                ...
                FilterGroups = filter
                    .FilterGroups.Select(filterGroup => new FilterGroupUpdateViewModel
                    {
                        ...
                        FilterItems = filterGroup
                            .FilterItems.Select(filterItem => filterItem.Id)
                            .ToList(),
                    })
                    .ToList(),
            })
        ...
        filterRepository
            .Setup(mock => mock.GetFiltersIncludingItems(releaseSubject.SubjectId))
        ...
        await using (var contentDbContext = InMemoryContentDbContext(contentDbContextId))
        {
            ...
            Assert.Equal(
                request[0].FilterGroups[0].FilterItems[0],
                savedFilter1Group1.ChildSequence[0]
            );
            Assert.Equal(
                request[0].FilterGroups[0].FilterItems[1],
                savedFilter1Group1.ChildSequence[1]
            );

            Assert.Equal(2, savedFilter1Group2.ChildSequence.Count);
            Assert.Equal(
                request[0].FilterGroups[1].FilterItems[0],
                savedFilter1Group2.ChildSequence[0]
            );
            Assert.Equal(
                request[0].FilterGroups[1].FilterItems[1],
                savedFilter1Group2.ChildSequence[1]
            );
            ...
            Assert.Equal(
                request[1].FilterGroups[0].FilterItems[0],
                savedFilter2Group1.ChildSequence[0]
            );
        }
    }
    ...
    private static List<FilterGroup> CreateFilterGroups(
        Filter filter,
        ...
        return numberOfFilterItemsPerFilterGroup
            .Select(
                (filterItemCount, index) =>
                {
                    ...
                    filterGroup.FilterItems.ForEach(filterItem =>
    ...
    )
    {
        ...
        return new(
            ...
            releaseSubjectService
                ?? new ReleaseSubjectService(statisticsDbContext, contentDbContextInstance),
