...
public class MatchingObservationQueryGeneratorTests
{
    ...
    [Fact]
    public async Task SubjectIdSupplied_AllObservationsForSubjectMatched()
    {
    ...
    [Fact]
    public async Task TimePeriodsSupplied_ObservationsForSubjectAndTimePeriodsMatched()
    {
        var subjectId = Guid.NewGuid();
        await using var context = InMemoryStatisticsDbContext();
    ...
    [InlineData(2)]
    [InlineData(3)]
    public async Task FilterItemsSupplied_LessThanOrHalfItemsChosenForFilter_NotExistsCheck(int numberOfSelectedItems)
    {
        var subjectId = Guid.NewGuid();
        ...
        var filter = new Filter
        {
            Id = Guid.NewGuid(),
            SubjectId = subjectId,
            FilterGroups = CreateFilterGroups(filterItemsPerFilterGroup: 6),
        };
    ...
    [InlineData(4)]
    [InlineData(5)]
    public async Task FilterItemsSupplied_MoreThanHalfItemsChosenForFilter_ExistsCheck(int numberOfSelectedItems)
    {
        var subjectId = Guid.NewGuid();
    ...
    [Fact]
    public async Task FilterItemsSupplied_NoItemsChosenForFilter_FilterStatementExcluded()
    {
        var subjectId = Guid.NewGuid();
        ...
        var filter1 = new Filter
        {
            Id = Guid.NewGuid(),
            SubjectId = subjectId,
            FilterGroups = CreateFilterGroups(filterItemsPerFilterGroup: [5, 5]),
        };
        ...
        var filter2 = new Filter
        {
            Id = Guid.NewGuid(),
            SubjectId = subjectId,
            FilterGroups = CreateFilterGroups(filterItemsPerFilterGroup: 10),
        };
        ...
        await using (var context = InMemoryStatisticsDbContext(contextId))
        {
            ...
            [
                ...
                filter1.FilterGroups[0].FilterItems[2].Id,
                filter1.FilterGroups[1].FilterItems[1].Id,
            ];
            ...
            //
            // Ensure that only the DELETE statement for the Filter with selected Filter Items
            // appears.
            //
            const string expectedSql =
                @"
    ...
    [Fact]
    public async Task FilterItemsSupplied_AllItemsChosenForFilter_FilterStatementExcluded()
    {
        var subjectId = Guid.NewGuid();
        ...
        await using (var context = InMemoryStatisticsDbContext(contextId))
        {
            ...
            Guid[] selectedFilter2ItemIds =
            [
                filter2.FilterGroups[0].FilterItems[0].Id,
                filter2.FilterGroups[0].FilterItems[2].Id,
                filter2.FilterGroups[1].FilterItems[1].Id,
            ];
    ...
    [Fact]
    public async Task MultipleFiltersSupplied_MultipleDeleteClausesAdded_InCorrectPriorityOrder()
    {
        var subjectId = Guid.NewGuid();
        ...
        var filter3 = new Filter
        {
            Id = Guid.NewGuid(),
            SubjectId = subjectId,
            FilterGroups = CreateFilterGroups(filterItemsPerFilterGroup: [20, 20, 20, 20, 20]),
        };
        ...
        var filter4 = new Filter
        {
            Id = Guid.NewGuid(),
            SubjectId = subjectId,
            FilterGroups = CreateFilterGroups(filterItemsPerFilterGroup: 2),
        };
        ...
        var unrelatedFilter = new Filter
        {
            ...
            FilterGroups = CreateFilterGroups(filterItemsPerFilterGroup: 5),
        };
        ...
        await using (var context = InMemoryStatisticsDbContext(contextId))
        {
            ...
            // We are selecting 3 Filter Items out of 10 for Filter 1, making it
            // a 30% ratio.
            Guid[] selectedFilter1FilterItemIds =
            [
                filter1.FilterGroups[0].FilterItems[0].Id,
                filter1.FilterGroups[0].FilterItems[2].Id,
                filter1.FilterGroups[1].FilterItems[1].Id,
            ...
            Guid[] selectedFilter2FilterItemIds =
            [
                filter2.FilterGroups[0].FilterItems[0].Id,
                filter2.FilterGroups[0].FilterItems[2].Id,
                filter2.FilterGroups[0].FilterItems[4].Id,
                filter2.FilterGroups[0].FilterItems[3].Id,
                filter2.FilterGroups[0].FilterItems[7].Id,
                filter2.FilterGroups[0].FilterItems[6].Id,
                filter2.FilterGroups[0].FilterItems[8].Id,
            ];
            ...
            // We are selecting 12 filter items out of 100 for Filter 3,
            // making it a 12% ratio.
            Guid[] selectedFilter3FilterItemIds =
            [
                filter3.FilterGroups[0].FilterItems[0].Id,
                filter3.FilterGroups[0].FilterItems[1].Id,
                filter3.FilterGroups[1].FilterItems[0].Id,
                filter3.FilterGroups[2].FilterItems[1].Id,
                filter3.FilterGroups[3].FilterItems[0].Id,
                filter3.FilterGroups[4].FilterItems[1].Id,
                filter3.FilterGroups[4].FilterItems[2].Id,
                filter3.FilterGroups[4].FilterItems[3].Id,
                filter3.FilterGroups[4].FilterItems[4].Id,
                filter3.FilterGroups[4].FilterItems[5].Id,
                filter3.FilterGroups[4].FilterItems[6].Id,
                filter3.FilterGroups[4].FilterItems[7].Id,
            ];

            // We are selecting 1 filter item out of 2 for Filter 4,
            // making it a 50% ratio.
            Guid[] selectedFilter4FilterItemIds = [filter4.FilterGroups[0].FilterItems[0].Id];

            // An accidental inclusion of a Filter Item Id that doesn't belong to a
            // Filter on this Subject will be ignored.
            Guid[] invalidUnrelatedFilterFilterItemIds = [unrelatedFilter.FilterGroups[0].FilterItems[0].Id];
            ...
            Guid[][] selectedFilterItemsByFilter =
            [
    ...
    [Fact]
    public async Task SubjectIdLocationsTimePeriodsAndFilterItemsSupplied_FullQueryGenerated()
    {
        var subjectId = Guid.NewGuid();
        ...
        await using (var context = InMemoryStatisticsDbContext(contextId))
        {
            ...
            Guid[] selectedFilterItemIds =
            [
                filter.FilterGroups[0].FilterItems[0].Id,
                filter.FilterGroups[0].FilterItems[2].Id,
                filter.FilterGroups[1].FilterItems[1].Id,
            ];
            ...
            Guid[] selectedLocationIds = [Guid.NewGuid(), Guid.NewGuid()];
            ...
            Mock<ITempTableQuery<IdTempTable>>[] tempTableMocks =
            [
    ...
    private static List<FilterGroup> CreateFilterGroups(params int[] filterItemsPerFilterGroup)
    {
        return filterItemsPerFilterGroup
            .Select(filterItemCount => Fixture.DefaultFilterGroup(filterItemCount: filterItemCount).Generate())
            .ToList();
    }
    ...
    )
    {
        ...
        tempTableReference.SetupGet(t => t.Name).Returns($"#Filter{filterOrder}TempTable");
