...
const LocationFiltersForm = (props: Props & InjectedWizardProps) => {
    ...
    const calculateInitialValues = (initial: Dictionary<string[] | undefined> | undefined, opts: PublicationSubjectMeta['locations']): FormValues => {
      ...
      return {
        locations: Object.entries(opts).reduce((acc, [level, levelOptions]) => {
          ...
          return {
            ...acc,
            [level]: initialLevel.filter(initialId => levelOptions.options.find(({value}) => value === initialId) !== undefined)
          };
        }, {}),
      }
    };

    const [formInitialValues, setFormInitialValues] = React.useState(calculateInitialValues(initialValues, options));
    ...
    React.useEffect(() => {
        ...
        if (formikRef.current) {
          ...
          updateLocationLevels(() => mapValuesWithKeys<Dictionary<string[]>, FilterOption[]>(vals.locations, (key: string, value: string[]) => {
