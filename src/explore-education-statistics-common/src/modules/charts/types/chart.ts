...
export type DraftFullChart<T = FullChart> = Omit<T, 'chartConfig'> & {
...
/* Brings chartConfig.type to the top level */
export type WithChartConfigType<T extends { chartConfig: { type: string } }> = {
  type: T['chartConfig']['type'];
} & T;
...
export type RenderableChart =
  ...
  | WithChartConfigType<InfographicChartProps>
  | WithChartConfigType<Omit<MapBlockProps, 'id'>>;
