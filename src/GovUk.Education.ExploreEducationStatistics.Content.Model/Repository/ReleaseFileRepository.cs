...
public class ReleaseFileRepository : IReleaseFileRepository
{
    ...
    public async Task<Either<ActionResult, ReleaseFile>> FindOrNotFound(Guid releaseVersionId, Guid fileId)
    {
    ...
    )
    {
        return await _contentDbContext
            ...
            .Where(releaseFile =>
                releaseFile.ReleaseVersionId == releaseVersionId && types.Contains(releaseFile.File.Type)
            )
    ...
    public async Task<bool> FileIsLinkedToOtherReleases(Guid releaseVersionId, Guid fileId)
    {
        return await _contentDbContext
            ...
            .AnyAsync(releaseFile => releaseFile.ReleaseVersionId != releaseVersionId && releaseFile.FileId == fileId);
    }
    ...
    > CheckLinkedOriginalAndReplacementReleaseFilesExist(Guid releaseVersionId, Guid originalFileId)
    {
        return await _contentDbContext
            ...
            .OnSuccess(releaseFiles =>
                new Tuple<ReleaseFile, ReleaseFile>(releaseFiles.Original, releaseFiles.Replacement).ToValueTuple()
            );
    }
}
