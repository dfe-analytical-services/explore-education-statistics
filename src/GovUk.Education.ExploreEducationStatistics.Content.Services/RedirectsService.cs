...
public class RedirectsService : IRedirectsService
{
    ...
    {
        var publicationRedirects = await _contentDbContext.PublicationRedirects
        ...
        var releaseRedirectsByPublicationSlug = (
            ...
            .GroupBy(rr => rr.PublicationSlug)
            .ToDictionary(
                g => g.Key,
                g => g
                    .Select(a => new RedirectViewModel(FromSlug: a.FromSlug, ToSlug: a.ToSlug))
                    .ToList());
