...
public class PublicationsSearchService(ContentDbContext contentDbContext)
    : IPublicationsSearchService
{
    ...
    )
    {
        ...
        {
            baseQueryable = baseQueryable.Where(p =>
                p.LatestPublishedReleaseVersion!.Type == releaseType.Value
            );
        }
        ...
        var queryable = baseQueryable.JoinFreeText(
            contentDbContext.PublicationsFreeTextTable,
            p => p.Id,
            search
        );
                ...
                ? queryable
                    .OrderBy(result =>
                        DateOnly.FromDateTime(
                            result.Value.LatestPublishedReleaseVersion!.Published!.Value
                        )
                    )
                    .ThenByReleaseType()
                : queryable
