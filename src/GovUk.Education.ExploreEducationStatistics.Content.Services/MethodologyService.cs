...
public class MethodologyService : IMethodologyService
{
    ...
    public async Task<Either<ActionResult, MethodologyVersionViewModel>> GetLatestMethodologyBySlug(
        string slug
    )
    {
            ...
            .CheckEntityExists<Methodology>(query =>
                query
                    ...
                    .Where(m =>
                        ...
                        && slug
                            == (m.LatestPublishedVersion.AlternativeSlug ?? m.OwningPublicationSlug)
                    )
    ...
    private async Task<List<PublicationSummaryViewModel>> GetPublishedPublicationsForMethodology(
        Guid methodologyId
    )
    {
        return await _contentDbContext
            ...
            .Where(pm =>
                pm.MethodologyId == methodologyId
    ...
    private async Task<List<MethodologyVersionSummaryViewModel>> BuildMethodologiesForPublication(
        Guid publicationId
    )
    {
        var latestPublishedMethodologies =
            await _methodologyVersionRepository.GetLatestPublishedVersionByPublication(
                publicationId
            );
