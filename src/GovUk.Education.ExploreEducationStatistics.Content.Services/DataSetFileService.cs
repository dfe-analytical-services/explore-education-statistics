...
) : IDataSetFileService
{
    ...
    )
    {
        ...
        var geogLvlsDict = await contentDbContext
            ...
            .ToDictionaryAsync(
                file => file.Id,
                file =>
                    file.DataSetFileVersionGeographicLevels.Select(gl =>
                            gl.GeographicLevel.GetEnumLabel()
                        )
                        .Order()
                        .ToList(),
    ...
    {
        var latestReleaseVersions = contentDbContext.ReleaseVersions.LatestReleaseVersions(
            publishedOnly: true
        );
    ...
    )
    {
        if (meta == null)
        {
            throw new InvalidDataException(
                $"DataSetMeta should not be null. SubjectId: {subjectId}"
            );
        }
...
internal static class ReleaseFileQueryableExtensions
{
    ...
    {
        return releaseVersionId.HasValue
            ? query.Where(rf => rf.ReleaseVersionId == releaseVersionId.Value)
            : query;
    }
        ...
        return geographicLevel.HasValue
            ? query.Where(rf =>
                rf.File.DataSetFileVersionGeographicLevels.Any(gl =>
                    gl.GeographicLevel == geographicLevel
                )
            )
            : query;
    }
    ...
    )
    {
        ...
        if (latestOnly)
        {
            ...
            return query.Where(rf =>
                rf.ReleaseVersionId
                == rf.ReleaseVersion.Release.Publication.LatestPublishedReleaseVersionId
            );
        }

        // Restrict data set files to be for *any* latest published release version
        return query.Join(
            latestPublishedReleaseVersions,
