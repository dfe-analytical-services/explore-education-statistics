...
) : IAnalyticsWriteStrategy
{
    ...
    private readonly IWorkflowActor<CaptureCsvDownloadRequest> _workflowActor = new WorkflowActor(
    ...
    public async Task Report(IAnalyticsCaptureRequest request, CancellationToken cancellationToken)
    {
        if (request is not CaptureCsvDownloadRequest captureRequest)
        {
            throw new ArgumentException(
                $"Request must be of type {nameof(CaptureCsvDownloadRequest)}. It is {request.GetType().FullName}",
                nameof(request)
            );
        }
    ...
    private class WorkflowActor(string analyticsPath) : WorkflowActorBase<CaptureCsvDownloadRequest>(analyticsPath)
    {
