...
const testReplacementPlanWithLocation: DataReplacementPlan = {
  originalSubjectId: 'subId',
  replacementSubjectId: 'repId',
  valid: false,
  footnotes: [],
...
describe('DataFileReplacementDifferences', () => {
  ...
  const sharedRenderWithLocations = () =>
    render(
      <DataFileReplacementDifferences
        replacementFileId="replacementFileId"
        reloadPlan={jest.fn}
        fileId="fileId"
        releaseVersionId="releaseVersionId"
        plan={testReplacementPlanWithLocation}
      />,
    );
  ...
  test('generates table attributes correctly', async () => {
    sharedRenderWithLocations();
  ...
  test('renders only the tables that have data', async () => {
    sharedRender();
    ...
    const locationsTable = screen.queryByTestId(`${locationsTableId}`);
    expect(locationsTable).not.toBeInTheDocument();
  });
