...
describe('ApiDataSetNewLocationGroupsTable', () => {
  test('renders correctly with options buttons', () => {
    ...
    render(
      <ApiDataSetNewLocationGroupsTable locationGroups={testLocationGroups} />,
    );
    ...
    expect(row1Cells[1]).toHaveTextContent('Local Authority Districts');
    expect(
    ...
    expect(row2Cells[1]).toHaveTextContent('Multi Academy Trusts');
    expect(
  ...
  test('renders correctly without option buttons', () => {
    ...
    expect(row1Cells[1]).toHaveTextContent('Local Authority Districts');
    expect(
      within(row1Cells[1]).queryByRole('button', {
    ...
    expect(row2Cells[1]).toHaveTextContent('Multi Academy Trusts');
    expect(
      within(row2Cells[1]).queryByRole('button', {
        name: /View group options/,
      }),
    ).not.toBeInTheDocument();
  });
