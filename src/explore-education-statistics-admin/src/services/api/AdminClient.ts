...
class AdminClient extends Client {
  ...
  public get<T = unknown>(url: string, config?: AxiosRequestConfig): Promise<T> {
    return AdminClient.addSecurityHeaders(config)
      .then(extendedConfig => super.get(url, extendedConfig));
  }

  public post<T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
    return AdminClient.addSecurityHeaders(config)
      .then(extendedConfig => super.post(url, data, extendedConfig));
  }

  public put<T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
    return AdminClient.addSecurityHeaders(config)
      .then(extendedConfig => super.put(url, data, extendedConfig));
  }

  public patch<T = unknown>(url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
    return AdminClient.addSecurityHeaders(config)
      .then(extendedConfig => super.patch(url, data, extendedConfig));
  }

  public delete<T = unknown>(url: string, config?: AxiosRequestConfig): Promise<T> {
    return AdminClient.addSecurityHeaders(config)
      .then(extendedConfig => super.delete(url, extendedConfig));
  }

  private static async addSecurityHeaders(config?: AxiosRequestConfig): Promise<AxiosRequestConfig> {
