...
internal class ContentApiClient(HttpClient httpClient) : IContentApiClient
{
    ...
    )
    {
        ...
        {
            return new GetResponse<TResponse>.Error(
                $"Error calling {apiEndpoint}: {e.Message}({e.StatusCode})"
            );
        }
    ...
    {
        var apiEndpoint = BuildGetPublicationLatestReleaseSearchViewModelApiEndpoint(
            publicationSlug
        );
    ...
    public async Task<(bool WasSuccesssful, string? ErrorMessage)> Ping(
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<PublicationInfo[]> GetPublicationsForTheme(
        Guid themeId,
        CancellationToken cancellationToken
    )
    {
        ...
        return Process(
            ...
            onError: errorMessage => new UnableToGetPublicationsByThemeException(
                themeId,
                errorMessage
            )
        );
    ...
    )
    {
        ...
        return Process(
            response,
            onSuccess: dtos =>
                dtos.Select(dto => new ReleaseInfo { ReleaseId = dto.ReleaseId }).ToArray(),
            onError: errorMessage => new UnableToGetReleasesForPublicationException(
                publicationSlug,
                errorMessage
            )
        );
    ...
    public async Task<PublicationInfo[]> GetAllLivePublicationInfos(
        CancellationToken cancellationToken
    )
    {
    ...
    private string BuildGetPublicationReleaseSummaryEndpoint(
        string publicationSlug,
        string releaseSlug
    ...
    private string BuildGetPublicationLatestReleaseSearchViewModelApiEndpoint(
        string publicationSlug
    ) =>
