...
internal class ContentApiClient(HttpClient httpClient) : IContentApiClient
{
    ...
    private const string GetPublicationsEndpoint =
        "/api/publicationInfos";

    private const string GetPublicationsByThemeEndpointFormat =
        "/api/publicationInfos?ThemeId={0}";

    private const string GetPublicationReleaseSummaryEndpointFormat =
        "/api/publications/{0}/releases/{1}/summary";

    private const string GetReleasesForPublicationEndpointFormat =
        "api/publications/{0}/releases";

    private const string GetThemesEndpoint =
        "/api/themes";
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Process(
            ...
            onError: errorMessage => new UnableToGetPublicationLatestReleaseSearchViewModelException(publicationSlug, errorMessage));
    }
