...
public class SearchableDocumentChecker(
    ...
    IReleaseSummaryRetriever releaseSummaryRetriever
)
{
    public async Task<CheckSearchableDocumentsReport> RunCheck(
    ...
    )
    {
        ...
        var releaseSummaries = await releaseSummaryRetriever.GetAllPublishedReleaseSummaries(
            cancellationToken
        );
