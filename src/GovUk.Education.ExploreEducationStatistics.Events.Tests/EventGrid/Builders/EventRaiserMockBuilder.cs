...
public class EventRaiserMockBuilder
{
    ...
    public class Asserter(EventRaiserMockBuilder parent)
    {
        public void EventRaised<TEventBuilder>(TEventBuilder expectedEvent)
            where TEventBuilder : IEvent => Xunit.Assert.True(parent._mock.EventWasRaised(expectedEvent));
        ...
        public void NoEventRaised() => Xunit.Assert.False(parent._mock.EventWasRaised());
    }
    ...
    private class MockEventRaiser : IEventRaiser
    {
        ...
        public Task RaiseEvent<TEventBuilder>(TEventBuilder eventBuilder, CancellationToken cancellationToken = default)
            where TEventBuilder : IEvent => AddEvent(eventBuilder);

        public Task RaiseEvents<TEventBuilder>(
            IEnumerable<TEventBuilder> eventBuilders,
            CancellationToken cancellationToken = default
        )
            where TEventBuilder : IEvent => AddEvents(eventBuilders);
    }
}
