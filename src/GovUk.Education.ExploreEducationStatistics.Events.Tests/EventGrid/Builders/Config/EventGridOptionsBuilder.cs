...
public class EventGridOptionsBuilder
{
    ...
    public IOptions<EventGridOptions> Build() =>
        Options.Create(new EventGridOptions { EventTopics = _eventTopicOptions.ToArray() });

    public EventGridOptionsBuilder AddTopicConfig(
        string topicKey,
        string topicEndpoint,
        string? topicAccessKey = null
    )
    {
        _eventTopicOptions.Add(
            new EventTopicOptions
            {
                Key = topicKey,
