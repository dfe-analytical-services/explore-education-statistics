...
public class CancellationTokenTimeoutAspectTests
    : IClassFixture<CancellationTokenTimeoutTestFixture>
{
    private static class TestMethods
    {
        ...
        public static void AddTimeoutWithMisconfiguredConfiguration(
            CancellationToken? token = null
        ) { }
    ...
    public void NoCancellationTokenParameterAvailableForAddTimeout()
    {
        Assert.Throws<ArgumentException>(
            TestMethods.NoCancellationTokenParameterAvailableForAddTimeout
        );
    }
    ...
    {
        Assert.Throws<ArgumentException>(() =>
            TestMethods.AmbiguousCancellationTokenParametersAvailableForAddTimeout(
    ...
    public void TimeoutExceededWithMisconfiguredConfiguration()
    {
        var exception = Assert.Throws<ArgumentException>(() =>
            TestMethods.AddTimeoutWithMisconfiguredConfiguration()
        );
        Assert.Equal(
            $"Timeout configuration setting for key {MisconfiguredConfigurationItemKey} "
                + $"must be an integer",
            exception.Message
        );
    }
    ...
    public void TimeoutExceededWithMissingConfiguration()
    {
        ...
        Assert.Equal(
            $"Could not find timeout configuration setting for "
                + $"key {MissingConfigurationItemKey}",
            exception.Message
        );
    }
    ...
    public async Task MergeWithOriginalCancellationToken()
    {
        ...
        await TestMethods.AddTimeoutAsync(
            providedToken =>
            {
