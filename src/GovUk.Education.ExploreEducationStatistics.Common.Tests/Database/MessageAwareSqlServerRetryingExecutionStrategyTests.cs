...
public class MessageAwareSqlServerRetryingExecutionStrategyTests
{
    ...
    public MessageAwareSqlServerRetryingExecutionStrategyTests()
    {
        ...
        loggerFactory.Setup(s => s.Logger).Returns(Mock.Of<ILogger>());
        ...
        _strategy = new(dependencies, maxRetryCount: 5, maxRetryDelay: TimeSpan.FromSeconds(30));
    }

    [Fact]
    public void Exception_Unhandled()
    {
        var exception = CreateSqlException("A random error message", errorNumber: 0);

        var retry = (bool)
            typeof(MessageAwareSqlServerRetryingExecutionStrategy)
    ...
    public void Exception_HandledByBaseClass()
    {
        ...
        var exception = CreateSqlException("An error known by errorNumber to base class", errorNumber: 10060);
