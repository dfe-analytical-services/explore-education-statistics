...
public class FileStoragePathUtilsTests
{
    ...
    {
        public static TheoryData<string> PublicationData = new()
        {
            "publication-slug",
            "  publication-SLUG  ",
        };
        ...
        public static TheoryData<
            ...
            Guid,
            long
        > PublicationReleaseIdAndBoundaryLevelData = new()
        {
        ...
        public void PublicContentLatestReleasePath(string publicationSlug)
        {
            Assert.Equal(
                ...
                FileStoragePathUtils.PublicContentLatestReleasePath(
        ...
        public void PublicContentDataBlockPath(
        ...
        [Theory]
        [MemberData(nameof(PublicationReleaseAndIdData))]
        public void PublicContentSubjectMetaPath(
            string publicationSlug,
            string releaseSlug,
            Guid subjectId
        )
    ...
    public void FilesPath()
    {
        ...
        Assert.Equal(
            $"{rootPath}/ancillary/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.Ancillary)
        );
        Assert.Equal(
            $"{rootPath}/chart/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.Chart)
        );
        Assert.Equal($"{rootPath}/data/", FileStoragePathUtils.FilesPath(rootPath, FileType.Data));
        Assert.Equal(
            $"{rootPath}/data-zip/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.DataZip)
        );
        Assert.Equal(
            $"{rootPath}/bulk-data-zip/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.BulkDataZip)
        );
        Assert.Equal(
            $"{rootPath}/image/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.Image)
        );
        Assert.Equal(
            $"{rootPath}/data/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.Metadata)
        );
        Assert.Equal(
            $"{rootPath}/zip/",
            FileStoragePathUtils.FilesPath(rootPath, FileType.AllFilesZip)
        );
    }
}
