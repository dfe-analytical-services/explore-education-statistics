...
public static class BlobServiceTestUtils
{
    ...
    )
    {
        ...
        blobClient.SetupGet(client => client.Name).Returns(name);

        blobClient.Setup(client => client.ExistsAsync(default)).ReturnsAsync(Response.FromValue(exists, null!));
        ...
        blobClient
            .Setup(client => client.GetPropertiesAsync(default, default))
            .ReturnsAsync(Response.FromValue(blobProperties, null!));

        return blobClient;
    }

    public static Mock<BlobServiceClient> MockBlobServiceClient(params Mock<BlobContainerClient>[] blobContainerClients)
    {
        ...
        {
            blobServiceClient
                .Setup(client => client.GetBlobContainerClient(blobContainerClient.Object.Name))
    ...
    )
    {
        ...
        blobContainerClient.SetupGet(client => client.Name).Returns(containerName);

        foreach (var blobClient in blobClients)
        {
            blobContainerClient
                .Setup(client => client.GetBlobClient(blobClient.Object.Name))
