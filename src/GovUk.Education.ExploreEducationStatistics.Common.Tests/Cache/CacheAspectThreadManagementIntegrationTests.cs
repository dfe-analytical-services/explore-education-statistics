...
public class CacheAspectThreadManagementIntegrationTests : IntegrationTest<TestStartup>
{
    ...
    public async Task CachingControllerMethod()
    {
        ...
        var asyncActionFilterCompleted = AssertEventMessageEquals(
            events[2],
            $"{nameof(AsyncActionFilter)} OnActionExecutionAsync "
        ...
        var controllerMethodCompleted = AssertEventMessageEquals(
            events[5],
            $"Controller method with key myRequest completed after "
        ...
        Assert.InRange(
            ...
            controllerMethodCalled.Millis
                + AsyncOperationDelayMillis
                - TaskDelayErrorToleranceMillis,
        ...
        var settingCachedItemCompleted = AssertEventMessageEquals(
            events[7],
            $"Set cached item {expectedControllerResult} with "
        ...
        var asyncResultFilterCompleted = AssertEventMessageEquals(
            events[9],
            $"{nameof(AsyncResultFilter)} OnResultExecutionAsync "
    ...
    public async Task CachingControllerMethod_ExistingCachedItem()
    {
        ...
        await blobCacheService.SetItemAsync(
            new TestCacheKey("myRequest"),
            new ControllerResponse("existingItem")
        );
        events.Clear();
        ...
        var existingCachedItemResponse = AssertEventMessageEquals(
            events[3],
            $"Returning cached item {expectedControllerResult} "
    ...
    public async Task NoThreadExhaustion()
    {
        ...
        const int expectedMinimumTimeToRun =
            (AsyncOperationDelayMillis - TaskDelayErrorToleranceMillis) * 3;
    ...
    private class TestBlobCacheService : IBlobCacheService
    {
        ...
        public async Task<object?> GetItemAsync(IBlobCacheKey cacheKey, Type targetType)
        {
            ...
            if (Cache.ContainsKey(key!))
            {
                ...
                _events.Add(
                    new Event($"Returning cached item {result} with key {cacheKey} after delay")
                );
                return result;
            }
    ...
    private class TestController : ControllerBase
    {
        ...
        public async Task<ControllerResponse> CachingMethod([FromQuery] string key)
        {
            ...
            _events.Add(
                new Event(
                    $"Controller method with key {key} completed after delay - returning {resultToCache}"
                )
            );
