...
public static class MockBlobStorageServiceExtensions
{
    ...
    public static IReturnsResult<T> SetupDeleteBlob<T>(this Mock<T> service, IBlobContainer container, string path)
        where T : class, IBlobStorageService
    {
    ...
    {
        return service
            .Setup(s => s.UploadAsJson(container, path, content, contentEncoding, settings, cancellationToken))
