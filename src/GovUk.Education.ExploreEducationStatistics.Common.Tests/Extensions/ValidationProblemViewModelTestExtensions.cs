...
public static class ValidationProblemViewModelTestExtensions
{
    ...
    public static ErrorViewModel AssertHasNotEmptyBodyError(this ValidationProblemViewModel validationProblem)
    {
        return validationProblem.AssertHasError(expectedPath: null, expectedCode: ValidationMessages.NotEmptyBody.Code);
    }
    ...
    public static void AssertHasErrors(List<ErrorViewModel> errors, List<ErrorViewModel> expectedErrors)
    {
        ...
        {
            var notFoundErrorMessages = notFoundErrors.Select(e => e.Message).ToList().JoinToString('\n');
            assertFailMessage += $"Error message(s) not found in expectedErrors:\n{notFoundErrorMessages}\n";
        }

        if (expectedErrors.Count != 0)
        {
            var expectedErrorMessages = expectedErrors.Select(e => e.Message).ToList().JoinToString('\n');
