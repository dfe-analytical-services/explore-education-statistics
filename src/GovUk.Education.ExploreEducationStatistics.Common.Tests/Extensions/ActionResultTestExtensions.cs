...
public static class ActionResultTestUtils
{
    ...
    public static void AssertValidationProblem<T>(
        this ActionResult<T> result,
    ...
    public static ValidationProblemViewModel AssertBadRequestWithValidationProblem(
        this IActionResult result
    )
    {
        var badRequest = Assert.IsAssignableFrom<BadRequestObjectResult>(result);
        var validationProblem = Assert.IsAssignableFrom<ValidationProblemViewModel>(
            badRequest.Value
        );
