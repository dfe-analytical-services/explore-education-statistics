    ...
    where T : class
{
    ...
    public Generator<T> FinishWith(Action<T> action) =>
        FinishWith((instance, _) => action(instance));
    ...
    private record PropertySetter(string Property, Func<Faker, T, SetterContext, object?> Setter)
        : ISetter
    {
        ...
        private static readonly ConcurrentDictionary<string, Action<T, object?>> SetterCache =
            new();
