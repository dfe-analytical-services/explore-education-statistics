    ...
    where T : class
{
    ...
    public Generator(Faker? faker = null, Func<int>? seeder = null, DataFixture? fixture = null)
    {
    ...
    public Generator<T> ForInstance(Action<InstanceSetters<T>> builder)
    {
        builder(
            new InstanceSetters<T>(
                onAction: setter => _setters.Add(new ActionSetter(setter)),
                onProperty: (property, setter) => _setters.Add(new PropertySetter(property, setter))
            )
        );
    ...
    {
        return Enumerable.Range(1, count).Select(i => GenerateWithRange(index: i - 1, length: count));
    }
    ...
    /// <summary>
    /// Generate a tuple of three instances of <see cref="T"/>.
    /// </summary>
    ...
    public List<T> GenerateList()
    {
        ...
        {
            throw new ArgumentException("Cannot infer number of elements to create if no range setters are used");
        }
    ...
    private int GetMaximumIndex()
    {
        ...
        {
            throw new ArgumentException(
                "At least one Range setter must be "
                    + "used in order to calculate the maximum index "
                    + "that setters apply to"
            );
        }

        return _rangeSetters.Count > 0 ? _rangeSetters.Max(rangeSetter => rangeSetter.Range.End.Value) - 1 : 0;
    }
    ...
    private T GenerateWithRange(int index, int length)
    {
        ...
        var instance = GenerateSingle(index: index, fixtureTypeIndex: fixtureTypeIndex, fixtureIndex: fixtureIndex);
        ...
        foreach (var rangeSetter in _rangeSetters)
        {
            ...
            rangeSetter.Builder(
                new InstanceSetters<T>(
                    onAction: setter => setters.Add(new ActionSetter(setter)),
                    onProperty: (property, setter) => setters.Add(new PropertySetter(property, setter))
                )
            );
    ...
    private void InvokeFinalizers(T instance) => _finalizers.ForEach(finalizer => finalizer(instance, _faker));
    ...
    {
        public void Invoke(Faker faker, T instance, SetterContext context) => Action(faker, instance, context);
    }

    private record PropertySetter(string Property, Func<Faker, T, SetterContext, object?> Setter) : ISetter
    {
        // Holds reflection metadata about properties on T
        private static readonly Lazy<Dictionary<string, MemberInfo>> TypeProperties = new(() =>
            new Binder().GetMembers(typeof(T))
        );
